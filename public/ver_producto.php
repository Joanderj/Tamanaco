...
if (!$producto) {
    $producto = [
        'url' => '/images/default-product.png', // Ruta de una imagen por defecto
        'nombre_producto' => 'Producto no encontrado',
        'nombre_marca' => null,
        'nombre_modelo' => null,
        'nombre_tipo' => null,
        'nombre_clasificacion' => null,
        'unidad_medida' => null,
        'id_status' => 0,
        'date_created' => null,
    ];
...
<div class="max-w-4xl mx-auto mt-10 bg-white shadow-md rounded-lg overflow-hidden">
    <div class="grid grid-cols-1 md:grid-cols-2">
        <div class="bg-gray-100 p-6 flex justify-center items-center">
            ...
            <img src="<?php echo htmlspecialchars($producto['url']); ?>" alt="<?php echo htmlspecialchars($producto['nombre_producto']); ?>" class="w-64 h-64 object-contain rounded border">
            <?php else: ?>
            <span>Sin Imagen</span>
            <?php endif; ?>
        </div>
        <div class="p-6">
            <h2 class="text-2xl font-bold mb-4">
                ...
                <?= strtoupper($producto['nombre_producto']) ?>
            </h2>

            <div class="space-y-2 text-sm">
                <p><span class="font-semibold">Marca:</span> <?= $producto['nombre_marca'] ?? '-' ?></p>
                <p><span class="font-semibold">Modelo:</span> <?= $producto['nombre_modelo'] ?? '-' ?></p>
                <p><span class="font-semibold">Tipo:</span> <?= $producto['nombre_tipo'] ?? '-' ?></p>
                <p><span class="font-semibold">Clasificación:</span> <?= $producto['nombre_clasificacion'] ?? '-' ?></p>
                <p><span class="font-semibold">Unidad de medida:</span> <?= $producto['unidad_medida'] ?? '-' ?></p>
                <p><span class="font-semibold">Estatus:</span> 
                    <?= ($producto['id_status'] == 1) ? '<span class="text-green-600 font-semibold">Activo</span>' : '<span class="text-red-600 font-semibold">Inactivo</span>' ?>
                </p>
                <p><span class="font-semibold">Fecha de creación:</span> <?= date('d/m/Y H:i', strtotime($producto['date_created'])) ?></p>
            </div>
