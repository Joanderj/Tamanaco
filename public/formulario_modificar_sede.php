...
$id_sede = $_GET['id_sede'] ?? null; // Obtener el ID de la marca desde la URL
if ($id_sede) {
    // Obtener el nombre de la marca
    $consulta1 = $conexion->query("SELECT nombre_sede FROM sede WHERE id_sede = $id_sede");
    $sede = $consulta1->fetch_assoc();

    // Obtener los modelos asociados con su id_status
    $consulta = $conexion->query("
        SELECT su.id_sucursal, su.nombre, COALESCE(ss.id_status, 0) AS id_status
        FROM sucursal su
        LEFT JOIN sede_sucursal ss ON su.id_sucursal = ss.id_sucursal AND ss.id_sede = $id_sede
    ");
}
?>
...
if ($id_sede) {
    // Consulta para obtener modelos asociados
    $consulta = $conexion->prepare("
        SELECT su.id_sucursal, su.nombre, COALESCE(ss.id_status, 0) AS id_status
        FROM sucursal su
       LEFT JOIN sede_sucursal ss ON su.id_sucursal = ss.id_sucursal AND ss.id_sede = ?
        ORDER BY su.nombre ASC
    ");
    $consulta->bind_param("i", $id_sede);
    $consulta->execute();
...
?>
 ...
 <div class="flex flex-col items-center mb-6">
    <div class="bg-blue-500 text-white w-16 h-16 rounded-full flex items-center justify-center shadow-lg mb-4">
        <i class="fas fa-tags text-3xl"></i> <!-- Ícono de etiquetas -->
    </div>
    <h2 class="text-3xl font-extrabold text-gray-800">Formulario de Sede</h2>
    <p class="text-gray-600 mt-2 text-center">Edita Las Sedes y Sus Vinculos con las Sucursales asociados de forma rápida y organizada.</p>
</div>
    <?php
...
<?php endif; ?>
    ...
    <form action="modificar_marca.php" method="POST" class="space-y-4" id="form-marca">
        <!-- Nombre de la Marca -->
        <div>
        <label for="nombre" class="block font-semibold flex items-center">
        <i class="fas fa-tag text-blue-500 mr-2"></i> <!-- Ícono de marca -->
        Nombre de la Sede: <span class="text-red-600">*</span>
        </label> 
        <input type="text" id="nombre" name="nombre" 
       value="<?php echo htmlspecialchars($sede['nombre_sede'] ?? ''); ?>" 
       placeholder="Ingrese el nombre de la marca"
       class="w-full border border-gray-300 rounded-lg p-2" required
       oninput="convertirMayusculas(this); validarNombre()">
            <!-- Mensajes específicos para el input -->
            <small id="mensaje-error-input" class="text-red-500 hidden">Esta sede ya existe</small>
            <small id="mensaje-exito-input" class="text-green-500 hidden">¡Esta sede está disponible!</small>
            <small class="text-gray-500">Ejemplo: SEDE1</small>
        </div>
        <button type="button" onclick="abrirVentanaEmergente()"
    ...
    Agregar Nuev Sucursal
</button>
      <!-- Vincular -->
<div id="vincular_varios">
    <label class="block font-semibold">
        <i class="fas fa-cube text-blue-500 mr-2"></i> Selecciona las Sucursales <span class="text-red-600">*</span>
    </label>
    ...
    <input type="text" id="searchBox" 
        ...
        placeholder="Buscar Sucursal..." onkeyup="filterTable()">
        ...
        <table class="table-auto w-full border-collapse">
            <thead class="bg-blue-100 text-blue-600">
                <tr>
                    ...
                    <th class="px-4 py-2 text-left font-bold">Nombre las Sucursales</th>
                </tr>
            </thead> 
...
while ($fila = $resultado->fetch_assoc()) {
    echo "<tr class='model-row'>
            <td class='px-4 py-2 border text-center'>
                <input type='checkbox' name='sucursal[" . $fila['id_sucursal'] . "]' value='1' 
                " . ($fila['id_status'] == 1 ? 'checked' : '') . ">
            </td>
            <td class='px-4 py-2 border'>" . htmlspecialchars($fila['nombre'], ENT_QUOTES, 'UTF-8') . "</td>
          </tr>";
}
?>
            </tbody>
...
<script>
    function filterTable() {
        ...
        rows.forEach(row => {
            ...
            row.style.display = modelName.includes(searchTerm) ? "" : "none";
        });
    }
</script>
...
<p class="text-gray-500 text-sm mt-4">Todos los campos marcados con <span class="text-red-600">*</span> son obligatorios.</p>
        ...
        <div class="flex justify-between mt-4 space-x-4">
            <input type="text" name="id_sede" id="id_sede" value="<?php echo htmlspecialchars($id_sede); ?>" class="hidden">
    <!-- Botón Guardar -->
    ...
    <!-- Botón Regresar -->
    <button type="button" onclick="location.href='sede.php';"
...
<script>
    ...
    function validarNombre() {
        ...
        const idsede = document.getElementById('id_sede').value; // Assuming there's a hidden input with the current brand ID
        ...
        if (nombre.trim() !== '') {
            fetch(`validar_sede_modificar.php?nombre=${encodeURIComponent(nombre)}&id=${encodeURIComponent(idsede)}`)
                .then(response => response.json())
                .then(data => {
                    if (data.existe && data.id !== parseInt(idsede)) {
...
<!-- Modal Structure -->
<div id="ventanaEmergenteModelo" class="fixed inset-0 bg-black bg-opacity-50 hidden flex justify-center items-center">
    <div class="bg-white max-w-4xl mx-auto rounded-lg shadow-lg p-6">
        <div class="flex flex-col items-center mb-6">
            <div class="bg-blue-500 text-white w-16 h-16 rounded-full flex items-center justify-center shadow-lg mb-4">
                <i class="fas fa-store text-3xl"></i> <!-- Ícono de modelos -->
            </div>
            <h2 class="text-3xl font-extrabold text-gray-800">Formulario Sucursal</h2>
            <p class="text-gray-600 mt-2 text-center">Registra una Sucursal y vincúlalo a una Sede de forma rápida y organizada.</p>
        </div>
           <!-- Contenedor global para mensajes de error -->
    ...
    <form id="formRegistrarModelo" method="POST" action="guardar_sucursal.php">
            <div class="mb-4">
                <label for="nombre2" class="block text-gray-700 font-bold mb-2">Nombre de la Sucursal <span class="text-red-600">*</span></label>
                <input type="text" id="nombre2" name="nombre2" placeholder="Ingrese el nombre del modelo"
                       class="border border-gray-300 rounded-lg p-2 w-full" required oninput="convertirMayusculas(this); validarNombre2()">
                       <small id="mensaje-error-input-modelo" class="text-red-500 hidden">Esta Sucursal ya existe</small>
                       <small id="mensaje-exito-input-modelo" class="text-green-500 hidden">¡Esta Sucursal está disponible!</small>
                <small class="text-gray-500">Ejemplo: Peltan</small>
            </div>

            <div class="mb-4">
    <div>
                <label for="pais" class="block text-gray-700 mb-2"><i class="fas fa-flag text-blue-500 mr-2"></i>Pais<span class="text-red-600">*</span></label>
                <select 
                  id="pais" 
                  name="pais"
            ...
            <!-- pais -->
            <div>
                <label for="estado" class="block text-gray-700 mb-2"><i class="fas fa-sun text-blue-500 mr-2"></i>Estado<span class="text-red-600">*</span></label>
                <select 
                  id="estado" 
                  name="estado"
                  class="w-full border border-gray-300 rounded p-2 focus:outline-none focus:ring-2 focus:ring-indigo-500"
    ...
    <div class="mb-4">
      <label for="direccion" class="block text-gray-700 font-bold mb-2"><i class="fas fa-house text-blue-500 mr-2"></i>Direccion <span class="text-red-600">*</span></label>
      <input type="text" id="direccion" name="direccion" placeholder="Ingrese la Direccion"
            ...
            <button type="submit" class="bg-blue-500 text-white px-6 py-2 rounded-lg hover:bg-blue-600 transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-blue-300">
                ...
                <button type="button" onclick="cerrarVentanaEmergente()"
...
// Manejo del formulario para guardar 
document.getElementById('formRegistrarModelo').addEventListener('submit', function (event) {
    ...
    fetch('guardar_modelo_marca.php', {
        method: 'POST',
        body: formData,
    })
        ...
        .then(data => {
            ...
            if (data.success) {
                ...
                const tbody = document.querySelector('#vincular_varios tbody'); // Seleccionar cuerpo de la tabla
...
function mostrarMensaje(tipo, mensaje) {
    ...
    // Apply CSS classes and styles to ensure it's circular
    mensajeIcono.style.width = '80px'; // Set explicit width
    mensajeIcono.style.height = '80px'; // Set explicit height
    ...
    mensajeTexto.classList.add('text-gray-700', 'text-center'); // Ensure text is centered
...
<script>
   function validarNombre2() {
    const nombre2 = document.getElementById('nombre2').value.trim(); // Limpiar espacios
    ...
    if (nombre2 !== '') { // Validación dinámica
        fetch(`validar_sucursal.php?nombre=${encodeURIComponent(nombre2)}`)
            .then(response => response.json())
            .then(data => {
                if (data.existe) {
                    // Mostrar mensaje de error
