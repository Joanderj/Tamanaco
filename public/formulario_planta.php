...
<hr>
 ...
 <div class="flex flex-col items-center mb-6">
    <div class="bg-blue-500 text-white w-16 h-16 rounded-full flex items-center justify-center shadow-lg mb-4">
        <i class="fas fa-industry text-3xl"></i> <!-- Ícono de etiquetas -->
    </div>
    <h2 class="text-3xl font-extrabold text-gray-800">Formulario de Planta</h2>
    <p class="text-gray-600 mt-2 text-center">Registra las Planta y vincula los Articulos con la Planta de forma rápida y organizada.</p>
</div>
    <?php
...
<?php endif; ?>
  ...
  <form id="formRegistrarplanta" action="registrar_planta.php" method="POST">
    <!-- Nombre de la planta -->
    <div class="mb-4">
      <label for="planta" class="block text-gray-700 font-bold mb-2"><i class="fas fa-industry text-blue-500 mr-2"></i>Nombre de la Planta<span class="text-red-600">*</span></label>
      <input type="text" id="planta" name="planta" placeholder="Ingrese el nombre de la PLanta"
       ...
       <small id="mensaje-error-input" class="text-red-500 hidden">Esta Planta ya existe</small>
            <small id="mensaje-exito-input" class="text-green-500 hidden">¡Esta Planta está disponible!</small>
            <small class="text-gray-500">Ejemplo: PELOTA </small>
    </div>

    <!-- Seleccionar sucursal principal -->
    <div class="mb-4">
      <label for="sede" class="block text-gray-700 font-bold mb-2">
      <i class="fas fa-map-marker-alt text-blue-500 mr-2"></i>Seleccione la Sede  de la Planta<span class="text-red-600">*</span>
      </label>
    ...
    <div class="mb-4">
      <label for="sede" class="block text-gray-700 font-bold mb-2">
      <i class="fas fa-question-circle text-blue-500 mr-2"></i>Seleccione los Articulo de la Planta<span class="text-red-600">*</span>
  <table class="table-auto border-collapse w-full text-left text-gray-700">
    ...
    <tbody>
      ...
      // Conexión a la base de datos
      $conexion = new mysqli('localhost', 'root', '', 'bd_tamanaco');
      $consulta = "SELECT id_articulo, nombre_articulo FROM articulo";
      $resultado = $conexion->query($consulta);

      while ($fila = $resultado->fetch_assoc()) {
          echo "<tr>
                  <td class='border-b border-gray-300 px-4 py-2'>
                    <input type='checkbox' name='articulo[]' value='{$fila['id_articulo']}' />
                  </td>
                  <td class='border-b border-gray-300 px-4 py-2'>{$fila['id_articulo']}</td>
                  <td class='border-b border-gray-300 px-4 py-2'>{$fila['nombre_articulo']}</td>
                </tr>";
      }
      $conexion->close();
      ?>
...
<p class="text-gray-500 text-sm mt-4">Todos los campos marcados con <span class="text-red-600">*</span> son obligatorios.</p>
    ...
    <!-- Botón Regresar -->
    <button type="button" onclick="location.href='planta.php';"
...
<script>
    function validarNombre() {
        ...
        if (planta.trim() !== '') {
            fetch(`validar_planta.php?nombre=${encodeURIComponent(planta)}`)
                .then(response => response.json())
                .then(data => {
                    if (data.existe) {
                        ...
                        textoMensajeGlobal.textContent = "Esta Planta ya existe. Por favor, ingrese otra.";
