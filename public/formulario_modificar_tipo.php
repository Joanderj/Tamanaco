...
$id_tipo = $_GET['id_tipo'] ?? null; // Obtener el ID del tipo desde la URL

if ($id_tipo) {
    // Obtener el nombre del tipo
    $consulta_tipo = $conexion->prepare("SELECT nombre_tipo FROM tipo WHERE id_tipo = ?");
    $consulta_tipo->bind_param("i", $id_tipo);
    $consulta_tipo->execute();
    $resultado_tipo = $consulta_tipo->get_result();
    $tipo = $resultado_tipo->fetch_assoc();

    // Obtener clasificaciones asociadas con su id_status
    $consulta_clasificaciones = $conexion->prepare("
        SELECT c.id_clasificacion, c.nombre_clasificacion, c.abreviacion_clasificacion, COALESCE(tc.id_status, 0) AS id_status
        FROM clasificacion c
        LEFT JOIN tipo_clasificacion tc ON c.id_clasificacion = tc.id_clasificacion AND tc.id_tipo = ?
        ORDER BY c.nombre_clasificacion ASC
    ");
    $consulta_clasificaciones->bind_param("i", $id_tipo);
    $consulta_clasificaciones->execute();
    $resultado_clasificaciones = $consulta_clasificaciones->get_result();
} else {
...
?>
 ...
 <div class="flex flex-col items-center mb-6">
    ...
    <p class="text-gray-600 mt-2 text-center">Registra los tipos y vincula las clasificaciones asociadas de forma rápida y organizada.</p>
</div>
    <?php
...
<?php endif; ?>
    ...
    <form action="modificar_tipo.php" method="POST" class="space-y-4" id="form-tipo">
    <!-- Nombre del Tipo -->
    <div>
        <label for="nombre_tipo" class="block font-semibold flex items-center">
        ...
        <input type="text" id="nombre_tipo" name="nombre_tipo"
               ...
               value="<?php echo htmlspecialchars($tipo['nombre_tipo'] ?? ''); ?>" 
               oninput="convertirMayusculas(this); validarNombreTipo()"> <!-- Validación en línea -->
        <!-- Mensajes específicos para el input -->
        <small id="mensaje-error-input" class="text-red-500 hidden">Este tipo ya existe</small>
        <small id="mensaje-exito-input" class="text-green-500 hidden">¡Este tipo está disponible!</small>
        <small class="text-gray-500">Ejemplo: ELECTRÓNICO</small>
    </div>
    <button type="button" onclick="abrirVentanaEmergenteClasificacion()"
    ...
    <div >
        ...
        <div id="vincular_varios" class="overflow-auto border border-gray-300 rounded" style="max-height: 300px;">
            <table class="table-auto w-full">
                ...
                <tbody>
                            <?php while ($fila = $resultado_clasificaciones->fetch_assoc()): ?>
                                <tr onclick="toggleRowSelection(this)">
                                    <td class="px-4 py-2 border text-center">
                                        <input type="checkbox" name="id_clasificacion[]" value="<?php echo $fila['id_clasificacion']; ?>" 
                                               onchange="highlightRow(this)" <?php echo ($fila['id_status'] == 1) ? 'checked' : ''; ?>>
                                    </td>
                                    <td class="px-4 py-2 border"><?php echo htmlspecialchars($fila['nombre_clasificacion'], ENT_QUOTES, 'UTF-8'); ?></td>
                                    <td class="px-4 py-2 border"><?php echo htmlspecialchars($fila['abreviacion_clasificacion'], ENT_QUOTES, 'UTF-8'); ?></td>
                                </tr>
                            <?php endwhile; ?>
                        </tbody>
            </table>
    ...
    <input type="text" name="id_tipo" id="id_tipo" value="<?php echo htmlspecialchars($id_tipo); ?>" class="hidden">
    ...
    <div class="flex justify-between mt-4 space-x-4">
        ...
        <!-- Botón Regresar -->
        <button type="button" onclick="location.href='tipo.php';"
...
<script>
    ...
    function validarNombreTipo() {
        const nombreTipo = document.getElementById('nombre_tipo').value.trim();
        const idTipo = document.getElementById('id_tipo').value; // Assuming there's a hidden input with the current type ID
        ...
        if (nombreTipo !== '') {
            fetch(`validar_tipo_modificar.php?nombre=${encodeURIComponent(nombreTipo)}&id_tipo=${encodeURIComponent(idTipo)}`)
                .then(response => {
                    if (!response.ok) {
                ...
                .then(data => {
                    if (data.existe && data.id_tipo !== parseInt(idTipo)) {
                        ...
                        textoMensajeGlobal.textContent = "Este tipo ya existe. Por favor, ingrese otro.";
                ...
                .catch(error => {
                    ...
                    // Ocultar mensajes y deshabilitar el botón en caso de error
                    mensajeErrorInput.classList.add('hidden');
                    mensajeExitoInput.classList.add('hidden');
                    mensajeGlobal.classList.add('hidden');
                    botonGuardar.disabled = true;
...
<!-- Modal Structure -->
<div id="ventanaEmergenteClasificacion" class="fixed inset-0 bg-black bg-opacity-50 hidden flex justify-center items-center">
    <div class="bg-white max-w-4xl mx-auto rounded-lg shadow-lg p-6">
        <div class="flex flex-col items-center mb-6">
            ...
            <p class="text-gray-600 mt-2 text-center">Registra una clasificación de forma rápida y organizada.</p>
        </div>
        <!-- Contenedor global para mensajes de error -->
        <div id="mensaje-global-clasificacion" class="hidden bg-red-100 text-red-700 p-4 rounded-lg mb-4">
            <strong id="tipo-mensaje-global-clasificacion"></strong> <span id="texto-mensaje-global-clasificacion"></span>
        </div>

        <form id="formRegistrarClasificacion" method="POST" action="guardar_clasificacion_tipo.php">
            <!-- Nombre de la Clasificación -->
            <div class="mb-4">
                <label for="nombre_clasificacion" class="block text-gray-700 font-bold mb-2">
                ...
                <!-- Mensajes específicos para el input -->
                <small id="mensaje-error-input-clasificacion" class="text-red-500 hidden">Esta clasificación ya existe</small>
                <small id="mensaje-exito-input-clasificacion" class="text-green-500 hidden">¡Esta clasificación está disponible!</small>
                <small class="text-gray-500">Ejemplo: ELECTRÓNICO</small>
            </div>

            <!-- Abreviatura de la Clasificación -->
            <div class="mb-4">
                <label for="abreviatura_clasificacion" class="block text-gray-700 font-bold mb-2">
            ...
            <div class="flex justify-between mt-4 space-x-4">
                ...
                <button type="button" onclick="cerrarVentanaEmergenteClasificacion()"
...
<script>
// Función para abrir la ventana emergente de clasificación
function abrirVentanaEmergenteClasificacion() {
    const ventana = document.getElementById('ventanaEmergenteClasificacion');
    if (ventana) {
        ventana.classList.remove('hidden');
    } else {
        console.error('Elemento con ID "ventanaEmergenteClasificacion" no encontrado.');
    }
}

// Función para cerrar la ventana emergente de clasificación
function cerrarVentanaEmergenteClasificacion() {
    const ventana = document.getElementById('ventanaEmergenteClasificacion');
    if (ventana) {
...
// Manejo del formulario para guardar la clasificación
document.getElementById('formRegistrarClasificacion').addEventListener('submit', function (event) {
    event.preventDefault();

    const formData = new FormData(this);

    fetch('guardar_clasificacion_tipo.php', {
        method: 'POST',
        body: formData,
    })
    ...
    .then(data => {
        ...
        if (data.success) {
            ...
            mostrarMensaje('success', '¡Clasificación guardada exitosamente!');
            cerrarVentanaEmergenteClasificacion();
        } else {
    ...
    .catch(error => {
        ...
        mostrarMensaje('error', 'Ocurrió un error al intentar guardar la clasificación.');
    });
});
...
function actualizarTabla(data) {
    ...
    if (tbody) {
        ...
        row.innerHTML = `
            <td class='px-4 py-2'>
                <input type='checkbox' name='clasificaciones[]' value='${data.id_clasificacion}' onchange='highlightRow(this)'>
            </td>
            <td class='px-4 py-2 text-gray-700'>${data.nombre_clasificacion}</td>
            <td class='px-4 py-2 text-gray-700'>${data.abreviacion_clasificacion}</td>
        `;
        tbody.appendChild(row);
    } else {
        console.error('Elemento de tabla no encontrado. Asegúrate de que el selector #vincular_varios tbody es correcto.');
    }
}
        ...
        fetch(`validar_clasificacion.php?nombre=${encodeURIComponent(nombreClasificacion)}`)
            ...
            .then(data => {
                if (data.existe) {
                    ...
                    if (!isAbreviaturaValid && document.getElementById('abreviatura_clasificacion').value.trim() !== '') {
                        tipoMensajeGlobal.textContent = "Error:";
