...
<div class="container mx-auto px-4 py-6">
    <div class="max-w-5xl mx-auto bg-white rounded-lg shadow-lg p-6">
        <div class="flex flex-col items-center mb-6">
            ...
            <h2 class="text-3xl font-extrabold text-gray-800">Actualizar Inventario de Herramientas</h2>
            <p class="text-gray-600 mt-2 text-center">Realiza movimientos de entrada, salida, traslado o retiro.</p>
        </div>

        <form action="actualizar_herramienta.php" method="POST" id="form-actualizar-herramienta" class="space-y-6">
            <!-- Seleccionar Herramienta -->
            <div>
                <label for="id_herramienta" class="block font-semibold">
                    Herramienta <span class="text-red-600">*</span>
                </label>
                <select name="id_herramienta" id="id_herramienta" required class="w-full border border-gray-300 rounded-lg p-2">
                    <?php
                    $conexion = new mysqli("localhost", "root", "", "bd_tamanaco");
                    $resultado = $conexion->query("SELECT id_herramienta, nombre_herramienta FROM herramientas");
                    while ($fila = $resultado->fetch_assoc()) {
                        echo "<option value='{$fila['id_herramienta']}'>{$fila['nombre_herramienta']}</option>";
                    }
                    $conexion->close();
                    ?>
                </select>
            ...
            <div id="campos_entrada" class="hidden space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <small id="error-cantidad_entrada" class="text-red-500 hidden">Ingrese una cantidad válida</small>
                    </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label for="stock_min" class="font-semibold">Stock Mínimo <span class="text-red-600">*</span></label>
                        <input type="number" name="stock_min" id="stock_min" class="w-full border border-gray-300 rounded-lg p-2" oninput="calcularPuntoReorden()">
                    </div>
                    <div>
                        <label for="stock_max" class="font-semibold">Stock Máximo <span class="text-red-600">*</span></label>
                        <input type="number" name="stock_max" id="stock_max" class="w-full border border-gray-300 rounded-lg p-2" oninput="calcularPuntoReorden()">
                    </div>
                    
                </div>

                <div>
                    <label for="punto_reorden" class="font-semibold">Punto de Reorden</label>
                    <input type="number" name="punto_reorden" id="punto_reorden" class="w-full border border-gray-300 rounded-lg p-2 bg-gray-100" readonly>
                </div>
            </div>
            ...
            <div id="campos_salida" class="hidden space-y-4">
                ...
                <div>
                    <label for="destino_salida" class="block font-semibold">Destino (Sede o Almacén) <span class="text-red-600">*</span></label>
                    <select name="destino_salida" id="destino_salida" class="w-full border border-gray-300 rounded-lg p-2">
                        <?php
                        $conexion = new mysqli("localhost", "root", "", "bd_tamanaco");
                        $resultado = $conexion->query("SELECT id_almacen, nombre FROM almacen WHERE id_status = 1");
                        while ($fila = $resultado->fetch_assoc()) {
                ...
                <div>
                    ...
                    <small id="error-cantidad_salida" class="text-red-500 hidden">Cantidad inválida</small>
                </div>
                <div>
            ...
            <div id="campo_hora_traslado" class="hidden">
                <div>
                    ...
                    <input type="number" name="cantidad_traslado" id="cantidad_traslado" class="w-full border border-gray-300 rounded-lg p-2" min="1" required>
                </div>
                <div>
            ...
            <div id="campos_retirado" class="hidden space-y-4">
                <div>
                    ...
                    <input type="number" name="cantidad_retirada" id="cantidad_retirada" class="w-full border border-gray-300 rounded-lg p-2" min="1" required>
                </div>
                <div>
            ...
            <div class="flex justify-between mt-6">
                ...
                <button type="button" onclick="location.href='inventario.php';"
...
<script>
    function mostrarCamposEspeciales() {
        ...
        document.getElementById('campos_entrada').classList.toggle('hidden', tipo !== 'entrada');
        document.getElementById('campos_salida').classList.toggle('hidden', tipo !== 'salida');
        document.getElementById('campo_hora_traslado').classList.toggle('hidden', tipo !== 'traslado');
        document.getElementById('campos_retirado').classList.toggle('hidden', tipo !== 'retirado');
    }
    ...
    function calcularPuntoReorden() {
        ...
        const campoReorden = document.getElementById('punto_reorden');

        stockMinInput.addEventListener('blur', () => {
            ...
            if (!isNaN(stockMax)) {
                if (stockMin > stockMax) {
                    ...
                    campoReorden.value = Math.round((stockMin + stockMax) / 2);
                }
            } else {
                campoReorden.value = '';
            }
