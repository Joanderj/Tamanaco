...
<hr>
    <!-- Formulario Actualizar Inventario de repuesto -->
<div class="container mx-auto px-4 py-6">
    <div class="max-w-5xl mx-auto bg-white rounded-lg shadow-lg p-6">
        <div class="flex flex-col items-center mb-6">
            ...
            <h2 class="text-3xl font-extrabold text-gray-800">Actualizar Inventario de repuesto</h2>
...
?>
    ...
    <!-- Formulario General -->
<form id="formRegistrarsucursal" action="actualizar_repuesto_traslado.php" method="POST" onsubmit="return validarAlmacenes()">
    <!-- Nombre de la sucursal -->
    <div class="mb-4">
        <div>
            <label for="repuesto" class="block text-gray-700 mb-2">
                <i class="fas fa-box text-blue-500 mr-2"></i>repuesto<span class="text-red-600">*</span>
            </label>
            <select 
                id="repuesto" 
                name="repuesto"
            ...
            >
                ...
                $query = "SELECT id_repuesto, nombre_repuesto FROM repuesto ORDER BY nombre_repuesto ASC";
                $result = $conexion->query($query);
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        $id = htmlspecialchars($row["id_repuesto"]);
                        $nombre = htmlspecialchars($row["nombre_repuesto"]);
                        echo "<option value='$id'>$nombre</option>";
                    }
                } else {
                    echo "<option value=''>No hay repuesto disponibles</option>";
                }
                $conexion->close();
                ?>
            </select>
...
function verificarCantidad() {
    const repuestoId = document.getElementById('repuesto').value;
    const almacenId = document.getElementById('almacen').value;
    const cantidadIngresada = document.getElementById('cantidad').value;
    const mensajeError = document.getElementById('mensajeError');

    if (repuestoId && almacenId) {
        fetch(`verificar_inventario_repuesto.php?repuesto=${repuestoId}&almacen=${almacenId}`)
