...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $telefono = !empty($_POST['telefono']) ? $_POST['telefono'] : $perfil["telefono"];
    ...
    try {
        ...
        $stmt = $conexion->prepare("UPDATE personas SET telefono=? WHERE cedula=?");
        if (!$stmt) {
            throw new Exception("Error al preparar la consulta de actualización: " . $conexion->error);
        }

        // Vincular parámetros
        $stmt->bind_param("si", $telefono, $cedula);
