...
// Submenú actual: clasificacion (id_submenu = 14)
$submenu_actual = 14;
...
$nombreClasificacion = isset($_GET['nombreClasificacion']) ? $_GET['nombreClasificacion'] : ''; // Filtro por nombre de la clasificación
$tipoSeleccionado = isset($_GET['tipo']) ? $_GET['tipo'] : ''; // Filtro por tipo relacionado
...
if (isset($_GET['sort'])) {
    switch ($_GET['sort']) {
        ...
        case 'fecha_desc':
            ...
            break;
        case 'nombre_asc':
            $orderBy = 'nombre_clasificacion ASC';
            break;
        case 'nombre_desc':
            $orderBy = 'nombre_clasificacion DESC';
            break;
        case 'numero_asc':
            $orderBy = 'id_clasificacion ASC';
            break;
        case 'numero_desc':
            $orderBy = 'id_clasificacion DESC';
            break;
    }
}
...
$totalQuery = "
    SELECT COUNT(DISTINCT c.id_clasificacion) 
    FROM clasificacion c
    INNER JOIN tipo_clasificacion tc ON c.id_clasificacion = tc.id_clasificacion
    WHERE 1=1
";
$params = [];
...
if (!empty($tipoSeleccionado)) {
    $totalQuery .= " AND tc.id_tipo = ?";
    $params[] = $tipoSeleccionado;
}
if (!empty($nombreClasificacion)) {
    $totalQuery .= " AND c.nombre_clasificacion LIKE ?";
    $params[] = '%' . $nombreClasificacion . '%';
}
if (!empty($status)) {
    $totalQuery .= " AND c.id_status = ?";
    $params[] = $status;
}
if (!empty($fechaInicio)) {
    $totalQuery .= " AND c.fecha_creacion >= ?";
    $params[] = $fechaInicio;
}
if (!empty($fechaFinal)) {
    $totalQuery .= " AND c.fecha_creacion <= ?";
...
// Consulta principal de datos (clasificaciones únicas con paginación)
$query = "
    SELECT DISTINCT c.id_clasificacion, c.nombre_clasificacion, c.fecha_creacion, c.id_status
    FROM clasificacion c
    INNER JOIN tipo_clasificacion tc ON c.id_clasificacion = tc.id_clasificacion
    WHERE 1=1
";
...
// Agregar filtros
if (!empty($tipoSeleccionado)) {
    $query .= " AND tc.id_tipo = ?";
}
if (!empty($nombreClasificacion)) {
    $query .= " AND c.nombre_clasificacion LIKE ?";
}
if (!empty($status)) {
    $query .= " AND c.id_status = ?";
}
if (!empty($fechaInicio)) {
...
$clasificaciones = $result->fetch_all(MYSQLI_ASSOC);
...
<!-- Modal para mostrar información de la Clasificación -->
<div id="modalClasificacion" class="fixed inset-0 bg-gray-900 bg-opacity-75 hidden justify-center items-center z-50">
    <div class="bg-white rounded-xl shadow-lg w-full max-w-lg overflow-hidden">
        ...
        <div class="px-6 py-4 flex justify-between items-center bg-gradient-to-r from-indigo-500 via-purple-500 to-indigo-500 text-white">
            <h2 class="text-xl font-semibold flex items-center">
                <i class="fas fa-layer-group mr-2"></i> Detalles de la Clasificación
            </h2>
        ...
        <div class="px-6 py-4">
            <div class="flex items-center space-x-4">
                ...
                <p class="text-lg font-medium text-gray-800">
                    <strong>Nombre de la Clasificación:</strong> <span id="clasificacionNombre"></span>
                </p>
            </div>
            <div class="flex items-center space-x-4 mt-4">
                <i class="fas fa-calendar-alt text-indigo-500 text-2xl"></i>
                <p class="text-lg font-medium text-gray-800">
                    <strong>Fecha de Creación:</strong> <span id="clasificacionFecha"></span>
                </p>
            </div>

            <p class="mt-4 text-lg font-semibold text-gray-700">Tipos Relacionados:</p>
            <ul id="tiposRelacionados" class="grid grid-cols-2 gap-4 mt-4">
                <!-- Tipos dinámicos -->
            </ul>
        </div>
...
<div class="flex justify-between items-center mb-4">
    ...
    <div class="space-x-2">
        <?php if (in_array(2, $permisos)): ?>
            <button 
                onclick="window.location.href='formulario_guardar_clasificacion.php'" 
        ...
        <?php if (in_array(4, $permisos)): ?>
            <button 
                onclick="window.location.href='imprimir_clasificaciones.php'" 
...
<div id="filterForm" class="hidden mt-2 bg-white border border-gray-300 rounded shadow-lg p-4">
   ...
   <div id="savedFilters" class="mt-4">
    ...
    <!-- Campo: Nombre de la Clasificación -->
    <div class="mb-4">
        <label for="nombreClasificacion" class="block text-gray-700 mb-2">Nombre de la Clasificación</label>
        <input 
          type="text" 
          ...
          placeholder="Escribe el nombre de la clasificación..."
        />
    </div>

    <!-- Campo: Status y Tipos Juntos -->
    <div class="grid grid-cols-2 gap-4 mb-4">
        ...
        <!-- Tipos -->
        <div>
            <label for="tipo" class="block text-gray-700 mb-2">Tipos</label>
            <select 
                id="tipo" 
                name="tipo"
                class="w-full border border-gray-300 rounded p-2 focus:outline-none focus:ring-2 focus:ring-indigo-500"
...
<div class="border border-gray-300 rounded-lg shadow-md p-4">
    ...
    <div class="flex justify-between items-center mb-4 border-b border-gray-300 pb-2">
        <h1 class="text-xl font-semibold text-gray-700">Clasificacion</h1>
        <div class="relative">
        ...
        <tr>
            <th class="px-6 py-3 text-left font-medium">
                <span class="flex items-center space-x-2">
                    ...
                    <span>ID Clasificación</span>
                </span>
            </th>
            <th class="px-6 py-3 text-left font-medium">
                <span class="flex items-center space-x-2">
                    <i class="fas fa-tags"></i>
                    <span>Nombre Clasificación</span>
                </span>
            </th>
            <th class="px-6 py-3 text-left font-medium">
                <span class="flex items-center space-x-2">
    ...
    <tbody class="bg-white divide-y divide-gray-200" id="dataTable">
        ...
        <tr class="hover:bg-gray-100">
            <td class="px-6 py-4"><?php echo htmlspecialchars($clasificacion['id_clasificacion']); ?></td>
            <td class="px-6 py-4"><?php echo htmlspecialchars($clasificacion['nombre_clasificacion']); ?></td>
            <td class="px-6 py-4"><?php echo htmlspecialchars($clasificacion['fecha_creacion']); ?></td>
            <td class="px-6 py-4 flex space-x-2">
                <?php
                include_once 'includes/permisos.php';

                $idPerfil = $_SESSION['id_perfil'];
                $idSubmenu = 14; // ID del submenú correspondiente
                $permisos = obtenerPermisos($idPerfil, $idSubmenu);
                ?>

                <?php if ($clasificacion['id_status'] == 1): ?>
                    
                    <?php if (in_array(1, $permisos)): ?>
                    <button 
                        class="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600"
                        onclick="showModal(<?php echo $clasificacion['id_clasificacion']; ?>)">
                    ...
                    <button 
                        ...
                        onclick="window.location.href='formulario_modificar_clasificacion.php?id_clasificacion=<?php echo $clasificacion['id_clasificacion']; ?>'">
  ...
  <script>
    ...
    function applyFilters() {
        const nombreClasificacion = document.getElementById('nombreClasificacion').value; // Filtro por nombre de la clasificación
        const tipo = document.getElementById('tipo').value; // Filtro por tipo relacionado
        const status = document.getElementById('status').value; // Filtro por estado
        const fechaInicio = document.getElementById('fechaInicio').value;
        const fechaFinal = document.getElementById('fechaFinal').value;

        // Validar que se seleccionó al menos un filtro
        if (!nombreClasificacion && !tipo && !status && !fechaInicio && !fechaFinal) {
            alert("Por favor, selecciona al menos un filtro.");
            return;
        }

        // Redirigir a la misma página con los filtros aplicados
        window.location.href = `?nombreClasificacion=${encodeURIComponent(nombreClasificacion)}&tipo=${tipo}&status=${status}&fechaInicio=${fechaInicio}&fechaFinal=${fechaFinal}`;
    }
...
<script>
    // Función para mostrar el modal con los detalles de la clasificación y sus tipos relacionados
    function showModal(idClasificacion) {
        fetch(`getClasificacionTipos.php?id_clasificacion=${idClasificacion}`) // Ruta dinámica para obtener datos
            .then(response => response.json())
            .then(data => {
                // Mostrar el nombre de la clasificación y la fecha de creación
                document.getElementById('clasificacionNombre').textContent = data.nombreClasificacion;
                document.getElementById('clasificacionFecha').textContent = data.fechaCreacion;

                // Contenedor para los tipos relacionados
                const tiposRelacionados = document.getElementById('tiposRelacionados');
                tiposRelacionados.innerHTML = ''; // Limpiar contenido previo

                // Generar lista de tipos relacionados
                data.tipos.forEach((tipo, index) => {
                    const liTipo = document.createElement('li');
                    liTipo.className = "flex items-center space-x-2 bg-gray-100 p-2 rounded shadow-md";

                    const numeroTipo = document.createElement('span');
                    numeroTipo.className = "text-indigo-500 font-bold";
                    numeroTipo.textContent = `${index + 1}.`;

                    const iconoTipo = document.createElement('i');
                    iconoTipo.className = "fas fa-layer-group text-indigo-500";

                    const nombreTipo = document.createElement('span');
                    nombreTipo.className = "text-gray-700 font-medium";
                    nombreTipo.textContent = tipo;

                    liTipo.appendChild(numeroTipo);
                    liTipo.appendChild(iconoTipo);
                    liTipo.appendChild(nombreTipo);

                    tiposRelacionados.appendChild(liTipo);
                });
