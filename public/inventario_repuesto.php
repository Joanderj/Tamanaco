...
if (isset($_GET['sort'])) {
    switch ($_GET['sort']) {
        ...
        case 'fecha_desc':
            ...
            break;
        case 'nombre_asc':
            $orderBy = 'p.nombre_repuesto ASC';
            break;
        case 'nombre_desc':
            $orderBy = 'p.nombre_repuesto DESC';
            break;
        case 'numero_asc':
...
// Calcular si hay repuesto con stock bajo o valores nulos
$hayStockBajo = false;
$cantidadStockBajo = 0;
$cantidadNulos = 0;
...
<head>
    ...
    <title>Inventario de repuesto</title>
...
<div class="space-x-2">
    <button onclick="location.href='entrega_repuesto.php'" class="bg-blue-500 hover:bg-blue-600 text-white px-3 py-2 rounded-md shadow text-sm">
...
<div id="lowStockAlert" class="<?= $hayStockBajo || $cantidadNulos > 0 ? 'block' : 'hidden' ?> fixed bottom-4 right-4 bg-red-100 border-l-4 border-red-500 text-red-700 p-4 rounded-lg shadow-lg z-50" style="max-width: 400px;">
    <div class="flex items-start">
        ...
        <div class="ml-3">
            ...
            <p class="text-sm">Tienes <?= $cantidadStockBajo ?> repuesto(s) con stock por debajo del mínimo y <?= $cantidadNulos ?> repuesto(s) con valores nulos en stock mínimo o máximo.</p>
            <div class="mt-2">
...
<div id="lowReorderPointAlert" class="<?= $hayPuntoReordenBajo ? 'block' : 'hidden' ?> fixed bottom-16 left-4 bg-yellow-100 border-l-4 border-yellow-500 text-yellow-700 p-4 rounded-lg shadow-lg z-50" style="max-width: 400px; cursor: move;" onmousedown="dragStart(event, 'lowReorderPointAlert')">
    <div class="flex items-start">
        ...
        <div class="ml-3">
            <p class="font-bold">¡Atención!</p>
            <p class="text-sm">Tienes <?= $cantidadPuntoReordenBajo ?> repuesto(s) con punto de reorden menor que la cantidad actual.</p>
            <div class="mt-2">
...
function showLowReorderPointItems() {
    ...
    showFilterAppliedMessage('Mostrando solo repuesto con punto de reorden bajo');
}
...
<div class="border border-gray-300 rounded-lg shadow-md p-4">
    ...
    <div class="flex justify-between items-center mb-4 border-b border-gray-300 pb-2">
        <h1 class="text-xl font-semibold text-gray-700">Lista de repuesto en el Inventario</h1>
        <div class="relative">
            <select id="clasificacion" 
                ...
                <option value="nombre_asc">Por repuesto (A-Z)</option>
                <option value="nombre_desc">Por repuesto (Z-A)</option>
...
<div class="overflow-x-auto">
    ...
    <tr class="hover:bg-gray-100 <?= ($item['cantidad'] < $item['stock_minimo'] || is_null($item['stock_minimo']) || is_null($item['stock_maximo'])) ? 'bg-red-50' : '' ?>">
        ...
        <td class="px-6 py-4 whitespace-nowrap editable-cell" 
            data-id="<?= $item['id_inventario_repuesto'] ?>"
        ...
        <td class="px-6 py-4 whitespace-nowrap editable-cell" 
            data-id2="<?= $item['id_inventario_repuesto'] ?>"
...
<td class="px-6 py-4 flex space-x-2">
    <button onclick="verrepuesto(<?= $item['id_repuesto'] ?>)" 
...
function saveChange(id, field, value, cell) {
    ...
    fetch('actualizar_stock_repuesto.php', {
        method: 'POST',
        headers: {
