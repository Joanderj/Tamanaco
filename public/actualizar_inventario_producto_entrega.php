...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $query_destino = "SELECT cantidad FROM inventario_producto WHERE id_producto = ? AND id_almacen = ?";
    $stmt_destino = $conexion->prepare($query_destino);
    $stmt_destino->bind_param("si", $id_producto, $id_almacen_destino_traslado);
    ...
    if ($row_destino) {
        ...
        $update_query_destino = "UPDATE inventario_producto SET cantidad = ? WHERE id_producto = ? AND id_almacen = ?";
        $update_stmt_destino = $conexion->prepare($update_query_destino);
        $update_stmt_destino->bind_param("isi", $nueva_cantidad_destino, $id_producto, $id_almacen_destino_traslado);
        $update_stmt_destino->execute();
    } else {
        // Si no existe la producto en el almacén de destino, insertarlo
        $insert_query = "INSERT INTO inventario_producto (id_producto, cantidad, id_almacen) VALUES (?, ?, ?)";
        $insert_stmt = $conexion->prepare($insert_query);
        $insert_stmt->bind_param("isi", $id_producto, $cantidad, $id_almacen_destino_traslado);
        $insert_stmt->execute();
    }
    ...
    // Redirigir a inventario_producto
