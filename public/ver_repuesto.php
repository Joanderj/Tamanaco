...
$query = "SELECT r.*, 
          ...
          FROM repuesto r
          LEFT JOIN marca m ON m.id_marca = r.id_marca
          LEFT JOIN modelo mo ON mo.id_modelo = r.id_modelo
          LEFT JOIN tipo t ON t.id_tipo = r.id_tipo
          LEFT JOIN inventario_repuesto inv ON inv.id_repuesto = r.id_repuesto
...
if (!$repuesto) {
    $repuesto = [
        'url' => '/images/default-repuesto.png',
        'nombre_repuesto' => 'Repuesto no encontrado',
        'nombre_marca' => null,
        'nombre_modelo' => null,
        'nombre_tipo' => null,
        'descripcion' => null,
    ...
    <div class="grid grid-cols-1 md:grid-cols-2">
        <div class="bg-gray-100 p-6 flex justify-center items-center">
            <?php if (!empty($repuesto['url'])): ?>
                <img src="<?= htmlspecialchars($repuesto['url']) ?>" alt="<?= htmlspecialchars($repuesto['nombre_repuesto']) ?>" class="w-64 h-64 object-contain rounded border">
            <?php else: ?>
        ...
        <div class="p-6">
            <h2 class="text-2xl font-bold mb-2">
                ...
                <?= strtoupper(htmlspecialchars($repuesto['nombre_repuesto'])) ?>
            </h2>

            <?php if (!empty($repuesto['descripcion'])): ?>
                <p class="text-gray-600 mb-4 italic"><?= nl2br(htmlspecialchars($repuesto['descripcion'])) ?></p>
            <?php endif; ?>

            <div class="space-y-2 text-sm">
                <p><span class="font-semibold">Marca:</span> <?= htmlspecialchars($repuesto['nombre_marca'] ?? '-') ?></p>
                <p><span class="font-semibold">Modelo:</span> <?= htmlspecialchars($repuesto['nombre_modelo'] ?? '-') ?></p>
                <p><span class="font-semibold">Tipo:</span> <?= htmlspecialchars($repuesto['nombre_tipo'] ?? '-') ?></p>
                <p><span class="font-semibold">Ubicación:</span> <?= htmlspecialchars($repuesto['ubicacion'] ?? '-') ?></p>
                <p><span class="font-semibold">Cantidad disponible:</span> <?= $repuesto['cantidad'] ?? '-' ?></p>
                <p><span class="font-semibold">Stock máximo:</span> <?= $repuesto['stock_maximo'] ?? '-' ?></p>
                <p><span class="font-semibold">Punto de reorden:</span> <?= $repuesto['punto_reorden'] ?? '-' ?></p>
                <p><span class="font-semibold">Última reposición:</span> <?= $repuesto['fecha_ultima_reposicion'] ? date('d/m/Y', strtotime($repuesto['fecha_ultima_reposicion'])) : '-' ?></p>
                <p><span class="font-semibold">Estatus:</span> 
                    <?= ($repuesto['id_status'] == 1) ? '<span class="text-green-600 font-semibold">Activo</span>' : '<span class="text-red-600 font-semibold">Inactivo</span>' ?>
                </p>
                <p><span class="font-semibold">Fecha de creación:</span> 
                    <?= $repuesto['date_created'] ? date('d/m/Y H:i', strtotime($repuesto['date_created'])) : '-' ?>
                </p>
            </div>

            <div class="mt-6">
                <a href="javascript:window.close();" class="inline-block bg-yellow-600 text-white px-4 py-2 rounded hover:bg-yellow-700">
