...
?>
    ...
    <!-- Formulario General -->
<form id="formRegistrarsucursal" action="actualizar_herramienta_traslado.php" method="POST" onsubmit="return validarAlmacenes()">
    <!-- Nombre de la sucursal -->
    <div class="mb-4">
        <div>
            <label for="herramienta" class="block text-gray-700 mb-2">
                <i class="fas fa-box text-blue-500 mr-2"></i>herramienta<span class="text-red-600">*</span>
            </label>
            <select 
                id="herramienta" 
                name="herramienta"
                class="w-full border border-gray-300 rounded p-2 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                onchange="verificarCantidad()"
            >
                ...
                $query = "SELECT id_herramienta, nombre_herramienta FROM herramientas ORDER BY nombre_herramienta ASC";
                $result = $conexion->query($query);
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        $id = htmlspecialchars($row["id_herramienta"]);
                        $nombre = htmlspecialchars($row["nombre_herramienta"]);
                        echo "<option value='$id'>$nombre</option>";
                    }
                } else {
                    echo "<option value=''>No hay herramientas disponibles</option>";
                }
                $conexion->close();
                ?>
            </select>
        ...
        <div>
            <label for="almacen" class="block font-semibold">
                <i class="fas fa-warehouse text-blue-500 mr-2"></i>Almacén de Origen <span class="text-red-600">*</span>
            </label>
            <select name="almacen" id="almacen" class="w-full border border-gray-300 rounded-lg p-2">
                <?php
                $conexion = new mysqli("localhost", "root", "", "bd_tamanaco");
        ...
        <div class="mb-4">
            <label for="cantidad" class="block text-gray-700 font-bold mb-2">
                <i class="fas fa-boxes text-blue-500 mr-2"></i>Cantidad <span class="text-red-600">*</span>
            </label>
            <input type="number" id="cantidad" name="cantidad" placeholder="Ingrese la Cantidad"
                   class="border border-gray-300 rounded p-2 w-full" required min="0" oninput="verificarCantidad()">
            <span id="mensajeError" class="text-red-600 hidden"></span>
        </div>

        <div>
            <label for="almacen_destino_traslado" class="block font-semibold">
                <i class="fas fa-warehouse text-blue-500 mr-2"></i>Almacén de Destino <span class="text-red-600">*</span>
            </label>
        ...
        <div>
            <label for="motivo_traslado" class="block font-semibold">
                <i class="fas fa-comment-dots text-blue-500 mr-2"></i>Motivo del Traslado
            </label>
...
<script>
    function validarAlmacenes() {
        const almacenOrigen = document.getElementById('almacen').value;
        const almacenDestino = document.getElementById('almacen_destino_traslado').value;
        const mensajeError = document.getElementById('mensajeError');

        if (almacenOrigen === almacenDestino) {
            mensajeError.textContent = "El almacén de origen y el almacén de destino no pueden ser iguales.";
...
function verificarCantidad() {
    const herramientaId = document.getElementById('herramienta').value;
    const almacenId = document.getElementById('almacen').value;
    const cantidadIngresada = document.getElementById('cantidad').value;
    const mensajeError = document.getElementById('mensajeError');

    if (herramientaId && almacenId) {
        fetch(`verificar_inventario_herramienta.php?herramienta=${herramientaId}&almacen=${almacenId}`)
            .then(response => response.json())
            .then(data => {
                const cantidadDisponible = data.cantidad;
                if (cantidadIngresada > cantidadDisponible) {
                    mensajeError.textContent = `La cantidad ingresada (${cantidadIngresada}) supera la cantidad disponible (${cantidadDisponible}).`;
                    mensajeError.classList.remove('hidden');
