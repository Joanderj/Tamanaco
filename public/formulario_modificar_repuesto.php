...
<script>
    ...
    <option value="modelo_fabricante">Modelo del Fabricante</option>
    <option value="numero_parte">Número de Parte</option>
    <option value="anio_fabricacion">Año de Fabricación</option>
  ...
  <optgroup label="Características Técnicas">
    ...
    <option value="temperatura_maxima">Temperatura Máxima</option>
    <option value="presion_maxima">Presión Máxima</option>
    <option value="presion_operativa">Presión Operativa</option>
    <option value="nivel_filtracion">Nivel de Filtración</option>
    <option value="eficiencia_energetica">Eficiencia Energética</option>
    <option value="ciclo_vida">Ciclo de Vida (horas)</option>
    <option value="potencia">Potencia</option>
    <option value="velocidad">Velocidad</option>
    <option value="rpm">RPM (Revoluciones por Minuto)</option>
    <option value="nivel_ruido">Nivel de Ruido (dB)</option>
  </optgroup>
  ...
  <optgroup label="Compatibilidad">
    ...
    <option value="uso_especifico">Uso Específico</option>
    <option value="intercambiabilidad">Intercambiabilidad</option>
  </optgroup>
  ...
  <optgroup label="Uso Industrial">
    ...
    <option value="tipo_aplicacion">Tipo de Aplicación Industrial</option>
    <option value="resistencia_quimica">Resistencia Química</option>
    <option value="resistencia_impacto">Resistencia al Impacto</option>
    <option value="ignifugo">Ignífugo</option>
    <option value="resistencia_uv">Resistencia a rayos UV</option>
  </optgroup>

  <optgroup label="Estética y Marca">
    <option value="color">Color</option>
    <option value="acabado">Acabado</option>
    <option value="marca_visible">Marca Visible</option>
    <option value="presentacion">Presentación del Producto</option>
    <option value="logotipo_impreso">Logotipo Impreso</option>
  </optgroup>
  ...
  <optgroup label="Funcionalidad">
    ...
    <option value="respuesta_mecanica">Respuesta Mecánica</option>
    <option value="tiempo_respuesta">Tiempo de Respuesta</option>
    <option value="ajustabilidad">Ajustabilidad</option>
  </optgroup>

  <optgroup label="Instalación y Montaje">
    <option value="tipo_conexion">Tipo de Conexión</option>
    <option value="accesorios_incluidos">Accesorios Incluidos</option>
    <option value="tipo_ensamble">Tipo de Ensamble</option>
    <option value="herramientas_requeridas">Herramientas Requeridas</option>
    <option value="tiempo_instalacion">Tiempo Estimado de Instalación</option>
    <option value="reversible">Montaje Reversible</option>
  </optgroup>

  <optgroup label="Logística y Almacenaje">
    <option value="vida_util">Vida Útil en Almacenaje</option>
    <option value="recomendaciones_almacenaje">Recomendaciones de Almacenaje</option>
    <option value="embalaje">Tipo de Embalaje</option>
    <option value="condiciones_transporte">Condiciones de Transporte</option>
  </optgroup>

  <optgroup label="Seguridad y Riesgos">
    <option value="riesgos_asociados">Riesgos Asociados</option>
    <option value="instrucciones_seguridad">Instrucciones de Seguridad</option>
    <option value="manejo_residuos">Manejo de Residuos</option>
    <option value="inflamabilidad">Inflamabilidad</option>
    <option value="precauciones_mantenimiento">Precauciones de Mantenimiento</option>
  </optgroup>

  <optgroup label="Fabricación y Control de Calidad">
    <option value="fabricante">Fabricante</option>
    <option value="proceso_fabricacion">Proceso de Fabricación</option>
    <option value="fecha_control_calidad">Fecha de Control de Calidad</option>
    <option value="codigo_lote">Código de Lote</option>
    <option value="responsable_fabricacion">Responsable de Fabricación</option>
  </optgroup>
                    </select>
                </div>
                <div class="mb-2">
...
if ($id_repuesto <= 0) {
    die("ID de repuesto no válido.");
}

// Consulta del repuesto (clasificación eliminada)
$consulta = $conexion->prepare("
    ...
    WHERE r.id_repuesto = ?
");
$consulta->bind_param("i", $id_repuesto);
$consulta->execute();
...
$especificaciones = [];
$especQuery = $conexion->prepare("SELECT id_especificacion, detalle_especificacion, valor_especificacion FROM especificaciones_repuestos WHERE id_repuesto = ?");
$especQuery->bind_param("i", $id_repuesto);
$especQuery->execute();
...
?>
<form action="modificar_repuesto.php" method="post" enctype="multipart/form-data" class="space-y-6">
    ...
    <div class="text-center mb-6">
        <label for="imagen" class="relative w-64 h-64 mx-auto border-2 border-dashed border-blue-500 rounded-lg flex justify-center items-center cursor-pointer group overflow-hidden">
            ...
            <img id="imagen-preview" src="<?= !empty($repuesto['url']) ? $repuesto['url'] : '' ?>" alt="Imagen actual" class="absolute inset-0 w-full h-full object-cover rounded-lg <?= !empty($repuesto['url']) ? '' : 'hidden' ?>" />
            <div id="imagen-placeholder" class="text-center absolute inset-0 flex flex-col items-center justify-center text-blue-500 bg-white/60 group-hover:bg-white/70 transition <?= empty($repuesto['url']) ? '' : 'hidden' ?>">
    ...
    <!-- Nombre -->
    <div>
        <label class="font-semibold">Nombre del Repuesto <span class="text-red-600">*</span></label>
        <input type="text" name="nombre_repuesto" required value="<?= htmlspecialchars($repuesto['nombre_repuesto']) ?>" class="w-full border p-2 rounded-lg" />
    </div>

    <!-- Marca -->
    <div>
        <label class="font-semibold">Marca <span class="text-red-600">*</span></label>
        <select name="marca" id="marca" onchange="cargarModelos()" class="w-full border p-2 rounded-lg" required>
            <option value="">Seleccione una marca</option>
            <?php
            $marcas = $conexion->query("SELECT * FROM marca");
            while ($m = $marcas->fetch_assoc()) {
                $selected = $m['id_marca'] == $repuesto['id_marca'] ? 'selected' : '';
                echo "<option value='{$m['id_marca']}' $selected>{$m['nombre_marca']}</option>";
            }
            ?>
        </select>
    </div>
    ...
    <!-- Modelo -->
    <div>
        <label class="font-semibold">Modelo <span class="text-red-600">*</span></label>
        <select name="modelo" id="modelo" class="w-full border p-2 rounded-lg" required>
            <option value="<?= $repuesto['id_modelo'] ?>"><?= $repuesto['nombre_modelo'] ?></option>
        </select>
    </div>
    ...
    <div>
        ...
        <select name="tipo" class="w-full border p-2 rounded-lg">
            <option value="">Seleccionar</option>
            <?php
            $tipos = $conexion->query("SELECT * FROM tipo WHERE id_status = 1");
            while ($t = $tipos->fetch_assoc()) {
                $selected = $t['id_tipo'] == $repuesto['id_tipo'] ? 'selected' : '';
                echo "<option value='{$t['id_tipo']}' $selected>{$t['nombre_tipo']}</option>";
            }
            ?>
        </select>
    </div>
    ...
    <div>
        <label class="font-semibold">Sugerencia de Mantenimiento</label>
        <select name="sugerencia_mantenimiento" class="w-full border p-2 rounded-lg">
            <?php
            ...
            foreach ($frecuencias as $f) {
                $sel = ($f == $repuesto['sugerencia_mantenimiento']) ? 'selected' : '';
    ...
    <!-- Especificaciones -->
    <div id="contenedor-especificaciones" class="space-y-2">
        <?php foreach ($especificaciones as $i => $esp): ?>
        <div class="flex gap-2">
            <input type="hidden" name="id_especificacion[]" value="<?= $esp['id_especificacion'] ?>">
            
            <input type="text" name="detalle_especificacion[]" value="<?= htmlspecialchars($esp['detalle_especificacion']) ?>" class="w-1/2 border p-2 rounded-lg" placeholder="Detalle" required>
            <input type="text" name="valor_especificacion[]" value="<?= htmlspecialchars($esp['valor_especificacion']) ?>" class="w-1/2 border p-2 rounded-lg" placeholder="Valor" required>
        </div>
        <?php endforeach; ?>
    </div>
         <!-- Contenedor para especificaciones dinámicas -->
                <div id="contenedor-especificaciones"></div>
                <button type="button" onclick="agregarEspecificacion()" class="bg-blue-500 text-white py-2 px-4 rounded-lg hover:bg-blue-600 transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-blue-300">Agregar Especificación</button>

    <!-- Botones -->
    <div class="flex gap-4">
        <button type="submit" class="bg-green-600 text-white px-6 py-2 rounded hover:bg-green-700"><i class="fas fa-save mr-2"></i>Guardar Cambios</button>
        <a href="repuesto.php" class="bg-gray-400 text-white px-6 py-2 rounded hover:bg-gray-500"><i class="fas fa-arrow-left mr-2"></i>Regresar</a>
    </div>
</form>
...
function cargarModelos() {
    ...
    fetch('cargar_modelos.php?id_marca=' + idMarca)
        ...
        .then(data => {
            ...
            data.forEach(m => {
                modeloSelect.innerHTML += `<option value="${m.id_modelo}">${m.nombre_modelo}</option>`;
            });
        })
        .catch(err => {
