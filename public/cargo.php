...
$nombreCargo = isset($_GET['nombreCargo']) ? $_GET['nombreCargo'] : '';
...
$totalQuery = "SELECT COUNT(*) FROM cargo WHERE 1=1";
$params = [];

if (!empty($nombreCargo)) {
    $totalQuery .= " AND nombre_cargo LIKE ?";
    $params[] = '%' . $nombreCargo . '%'; // Agregar wildcard para búsqueda
}
...
// Consulta de datos
$query = "SELECT * FROM cargo WHERE 1=1"; // 1=1 para facilitar la concatenación de condiciones

if (!empty($nombreCargo)) {
    $query .= " AND nombre_cargo LIKE ?";
}
...
<!-- Modal para mostrar información del Cargo -->
<div id="modalCargo" class="fixed inset-0 bg-gray-900 bg-opacity-75 hidden justify-center items-center z-50">
    <div class="bg-white rounded-xl shadow-lg w-full max-w-lg overflow-hidden">
        ...
        <div class="px-6 py-4 flex justify-between items-center bg-gradient-to-r from-indigo-500 via-purple-500 to-indigo-500 text-white">
            <h2 class="text-xl font-semibold flex items-center">
                <i class="fas fa-briefcase mr-2"></i> Detalles del Cargo
            </h2>
            <button onclick="closeModal()" class="text-white text-lg hover:text-red-300 focus:outline-none">
                <i class="fas fa-times"></i>
            </button>
        </div>
        ...
        <div class="px-6 py-4">
            ...
            <div class="flex items-center space-x-4">
                ...
                <p class="text-lg font-medium text-gray-800">
                    <strong>Nombre del Cargo:</strong> <span id="cargoNombre">Cargando...</span>
                </p>
            </div>
            ...
            <div class="flex items-center space-x-4 mt-4">
                ...
                <p class="text-lg font-medium text-gray-800">
                    <strong>Descripción del Cargo:</strong> <span id="cargoDescripcion">Cargando...</span>
                </p>
            </div>

            <!-- Lista de Personas Relacionadas -->
            <p class="mt-4 text-lg font-semibold text-gray-700">Personas Relacionadas:</p>
            <ul id="personasRelacionadas" class="grid grid-cols-2 gap-4 mt-4">
                <!-- Personas dinámicas -->
            </ul>
        </div>

        <!-- Pie del modal -->
        <div class="px-6 py-4 bg-gray-100 flex justify-center rounded-lg shadow-sm">
...
<div class="flex justify-between items-center mb-4">
    ...
    <div class="space-x-2">
        <?php if (in_array(2, $permisos)): ?>
            <button 
                onclick="window.location.href='formulario_guardar_cargo.php'" 
        ...
        <?php if (in_array(4, $permisos)): ?>
            <button 
                onclick="window.location.href='imprimir_cargos.php'" 
...
<div class="border border-gray-300 rounded-lg shadow-md p-4">
    ...
    <div class="flex justify-between items-center mb-4 border-b border-gray-300 pb-2">
        <h1 class="text-xl font-semibold text-gray-700">Cargos</h1>
        <div class="relative">
    ...
    <tbody id="dataTable" class="bg-white divide-y divide-gray-200">
        ...
        <tr class="hover:bg-gray-100">
            <!-- ID del Cargo -->
            <td class="px-6 py-4 text-gray-800 font-medium"><?php echo $cargo['id_cargo']; ?></td>
            <!-- Nombre del Cargo -->
            <td class="px-6 py-4 text-gray-600"><?php echo $cargo['nombre_cargo']; ?></td>
            <!-- Botones de Acción -->
           <?php
include_once 'includes/permisos.php';

$idPerfil = $_SESSION['id_perfil'];
$idSubmenu = 20; // ID del submenú correspondiente
$permisos = obtenerPermisos($idPerfil, $idSubmenu);
?>
...
<td class="px-6 py-4 flex space-x-2">
    <?php if ($cargo['status'] == 1): ?>
        ...
        <button 
            ...
            onclick="showModal(<?php echo $cargo['id_cargo']; ?>)">
        ...
        <button 
            ...
            onclick="window.location.href='formulario_modificar_cargo.php?id_cargo=<?php echo $cargo['id_cargo']; ?>'">
...
<script>
    ...
    function applyFilters() {
        ...
        // Redirigir con los parámetros aplicados
    ...
    // Función para guardar filtros (puedes adaptarla con un modal)
    function showSaveFilterModal() {
        ...
        alert(`Guardando filtros:\nNombre: ${nombreCargo}\nEstado: ${status}\nFecha Inicio: ${fechaInicio}\nFecha Final: ${fechaFinal}`);
        // Aquí puedes implementar el guardado real con un modal o petición AJAX
    }
</script>

<script>
    // Función para mostrar el modal con los detalles del cargo
    function showModal(idCargo) {
        fetch(`getCargoPersonas.php?id_cargo=${idCargo}`)
            .then((response) => response.json())
            .then((data) => {
                // Actualizar los datos en el modal
                document.getElementById('cargoNombre').textContent = data.nombreCargo || 'Sin información';
                document.getElementById('cargoDescripcion').textContent = data.descripcion || 'Sin descripción disponible';

                // Generar la lista de personas relacionadas
                const personasRelacionadas = document.getElementById('personasRelacionadas');
                personasRelacionadas.innerHTML = ''; // Limpiar contenido previo

                if (data.personas && data.personas.length > 0) {
                    data.personas.forEach((persona, index) => {
                        const liPersona = document.createElement('li');
                        liPersona.className = 'flex items-center space-x-4 bg-gray-100 p-3 rounded shadow-md';

                        const numeroPersona = document.createElement('span');
                        numeroPersona.className = 'text-indigo-500 font-bold';
                        numeroPersona.textContent = `${index + 1}.`;

                        const nombrePersona = document.createElement('span');
                        nombrePersona.className = 'text-gray-700 font-medium';
                        nombrePersona.textContent = `${persona.primer_nombre} ${persona.primer_apellido}`;

                        liPersona.appendChild(numeroPersona);
                        liPersona.appendChild(nombrePersona);
                        personasRelacionadas.appendChild(liPersona);
                    });
                } else {
                    // Si no hay personas relacionadas
                    const noPersonas = document.createElement('p');
                    noPersonas.className = 'text-gray-500 italic';
                    noPersonas.textContent = 'No hay personas relacionadas con este cargo.';
                    personasRelacionadas.appendChild(noPersonas);
                }
            ...
            .catch((error) => {
                console.error('Error al obtener los datos del cargo:', error);
            });
    }
