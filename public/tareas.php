...
$tipoMantenimiento = isset($_GET['tipo_mantenimiento']) ? $_GET['tipo_mantenimiento'] : '';
...
if (!empty($sede)) {
    $countQuery .= " AND mu.id_sede = ?";
    $params[] = $sede;
}
if (!empty($tipoMantenimiento)) {
    $countQuery .= " AND t.tipo_mantenimiento_id = ?";
    $params[] = $tipoMantenimiento;
}
if (!empty($fechaInicio)) {
    $countQuery .= " AND t.fecha_inicio >= ?";
    $params[] = $fechaInicio;
}
if (!empty($fechaFin)) {
    $countQuery .= " AND t.fecha_inicio <= ?";
...
if (!empty($sede)) {
    $query .= " AND mu.id_sede = ?";
    $whereParams[] = $sede;
}
if (!empty($tipoMantenimiento)) {
    $query .= " AND t.tipo_mantenimiento_id = ?";
    $whereParams[] = $tipoMantenimiento;
}
if (!empty($fechaInicio)) {
    $query .= " AND t.fecha_inicio >= ?";
    $whereParams[] = $fechaInicio;
}
if (!empty($fechaFin)) {
    $query .= " AND t.fecha_inicio <= ?";
    $whereParams[] = $fechaFin;
}
...
<table class="table-auto w-full border border-gray-300 shadow-lg rounded-lg">
    ...
    <?= htmlspecialchars($fila['titulo_tarea']) ?>
        ...
        <!-- Tiempo programado con icono de reloj -->
        ...
        <!-- Tipo de mantenimiento con icono de etiqueta -->
        <span class="text-lg flex items-center py-3">
            ...
            <!-- Acciones -->
            <td class="px-6 py-4 text-center flex space-x-3">
    <!-- Botón para ver tarea -->
    <button 
        class="bg-blue-500 hidden text-white px-4 py-2 rounded hover:bg-blue-600 transition-all"
