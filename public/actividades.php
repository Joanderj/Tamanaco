...
$nombreActividad = isset($_GET['nombreActividad']) ? trim($_GET['nombreActividad']) : '';
...
// Clasificación
$orderBy = 'a.id_actividad ASC'; // Orden por defecto
if (isset($_GET['sort'])) {
    switch ($_GET['sort']) {
        case 'fecha_asc':
            $orderBy = 'a.fecha_realizar ASC';
            break;
        case 'fecha_desc':
            $orderBy = 'a.fecha_realizar DESC';
            break;
        case 'nombre_asc':
            $orderBy = 'a.descripcion_actividad ASC';
            break;
        case 'nombre_desc':
            $orderBy = 'a.descripcion_actividad DESC';
            break;
        case 'numero_asc':
...
$totalQuery = "
    SELECT COUNT(*) 
    FROM actividades a
    LEFT JOIN tareas t ON a.tarea_id = t.id_tarea
...
if (!empty($nombreActividad)) {
    $totalQuery .= " AND a.descripcion_actividad LIKE ?";
...
if (!empty($status)) {
    $totalQuery .= " AND a.id_status = ?";
    $params[] = $status;
    $types .= "i";
}
if (!empty($fechaInicio)) {
    $totalQuery .= " AND a.fecha_realizar >= ?";
    $params[] = $fechaInicio;
    $types .= "s";
}
if (!empty($fechaFinal)) {
...
$dataTypes = "";
if (!empty($nombreActividad)) {
    $query .= " AND a.descripcion_actividad LIKE ?";
    $dataParams[] = '%' . $nombreActividad . '%';
    $dataTypes .= "s";
}
if (!empty($idMaquina)) {
...
<div class="flex justify-start items-center space-x-4 mb-4">
 ...
 <div class="container mx-auto px-4 py-6">
    ...
    <div class="flex justify-between items-center mb-4">
      ...
      <div class="space-x-2">
        <button  onclick="window.location.href='formulario_guardar_tarea.php'" class="bg-indigo-500 text-white px-4 py-2 rounded hover:bg-indigo-600">
...
?>
<div class="overflow-x-auto mt-8 rounded-lg shadow-lg border border-gray-200">
    <table class="min-w-full bg-white divide-y divide-gray-200 text-sm">
        <thead class="bg-indigo-600 text-white">
            <tr>
                ...
                <th class="px-6 py-3 text-left">Tiempo Invertido</th>
                <th class="px-6 py-3 text-left">Hora de Finalización</th>
            ...
            <?php foreach ($actividades as $act): ?>
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-6 py-4 flex items-center space-x-3">
                        <?php if (!empty($act['url_maquina'])): ?>
                            <img src="<?= htmlspecialchars($act['url_maquina']) ?>" alt="Imagen máquina" class="w-12 h-12 rounded object-cover">
                        <?php else: ?>
                            <div class="w-12 h-12 bg-gray-200 rounded flex items-center justify-center text-gray-500">N/A</div>
                        <?php endif; ?>
                        <div>
                            <div class="font-semibold"><?= htmlspecialchars($act['nombre_maquina']) ?></div>
                            <div class="text-xs text-gray-500"><?= htmlspecialchars($act['nombre_marca']) ?> | <?= htmlspecialchars($act['nombre_modelo']) ?></div>
                        </div>
                    </td>

                    <!-- Tarea -->
                    <td class="px-6 py-4 font-medium text-gray-800">
                        <?= htmlspecialchars($act['titulo_tarea']) ?>
                    </td>

                    <!-- Actividad -->
                    <td class="px-6 py-4 text-gray-700">
                        <?= htmlspecialchars($act['descripcion_actividad']) ?>
                    </td>

                    <!-- Fecha -->
                    <td class="px-6 py-4">
                        <i class="fas fa-calendar-alt text-blue-500 mr-2"></i>
                        <?= htmlspecialchars($act['fecha_realizar']) ?>
                    </td>

                    <!-- Tiempo Invertido -->
                    <td class="px-6 py-4">
                        <i class="fas fa-clock text-green-500 mr-2"></i>
                        <?= htmlspecialchars($act['tiempo_invertido']) ?> (<?= htmlspecialchars($act['minutos_invertidos']) ?> min)
                    </td>

                    <!-- Hora Finalización -->
                    <td class="px-6 py-4">
                        <i class="fas fa-stopwatch text-red-500 mr-2"></i>
                        <?= htmlspecialchars($act['hora_finalizacion']) ?>
                    </td>

                    <!-- Acciones -->
                    <td class="px-6 py-4 text-center space-x-2">
                        <a href="formulario_guardar_actividad.php?id=<?= $act['id_tarea'] ?>" class="text-yellow-500 hover:text-yellow-600">
