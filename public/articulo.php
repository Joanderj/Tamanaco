...
// Submenú actual: Sede (id_submenu = 10)
$submenu_actual = 10;
...
$totalQuery = "
    SELECT COUNT(DISTINCT a.id_articulo) 
    FROM articulo a 
    JOIN planta_articulo pl ON a.id_articulo = pl.id_articulo 
    WHERE 1=1
";
$params = [];

if (!empty($nombre)) {
    $totalQuery .= " AND a.nombre_articulo LIKE ?"; // Cualificado con 'a.'
    $params[] = '%' . $nombre . '%'; // Agregar wildcard para búsqueda
}

if (!empty($descripcion)) {
    $totalQuery .= " AND a.descripcion_articulo = ?"; // Cualificado con 'a.'
    $params[] = $descripcion;
}

if (!empty($status)) {
    $totalQuery .= " AND a.status_id_status = ?"; // Cualificado con 'a.'
    $params[] = $status;
}

if (!empty($planta)) {
    $totalQuery .= " AND pl.id_planta = ?"; // Cualificado con 'pl.'
    $params[] = $planta;
}

// Agregar filtro por fechas
if (!empty($fechaInicio)) {
    $totalQuery .= " AND a.fecha_ingreso >= ?"; // Cualificado con 'a.'
    $params[] = $fechaInicio;
}
if (!empty($fechaFinal)) {
    $totalQuery .= " AND a.fecha_ingreso <= ?"; // Cualificado con 'a.'
...
// Consulta de datos
$query = "
    SELECT DISTINCT a.id_articulo,a.nombre_articulo,a.descripcion_articulo
    FROM articulo a 
    JOIN planta_articulo pl ON a.id_articulo = pl.id_articulo 
    WHERE 1=1
...
if (!empty($nombre)) {
    $query .= " AND a.nombre_articulo LIKE ?"; // Cualificado con 'a.'
}

if (!empty($descripcion)) {
    $query .= " AND a.descripcion_articulo = ?"; // Cualificado con 'a.'
}

if (!empty($status)) {
    $query .= " AND a.status_id_status = ?"; // Cualificado con 'a.'
}

if (!empty($planta)) {
    $query .= " AND pl.id_planta = ?"; // Cualificado con 'pl.'
}

// Agregar filtro por fechas
if (!empty($fechaInicio)) {
    $query .= " AND a.fecha_ingreso >= ?"; // Cualificado con 'a.'
}
...
$params[] = $offset; // Agregar offset
$params[] = $itemsPerPage; // Agregar límite

// Preparar y ejecutar la consulta
$stmt = $conexion->prepare($query);
if (!empty($params)) {
    // Cambiar el tipo de los parámetros de 's' a 's' o 'i' según corresponda
    $types = str_repeat('s', count($params) - 2) . 'ii'; // Asumiendo que los dos últimos son enteros (offset y itemsPerPage)
...
<div id="modalver" class="fixed inset-0 bg-gray-900 bg-opacity-75 hidden justify-center items-center z-50">
    <div class="bg-white rounded-xl shadow-lg w-full max-w-lg overflow-hidden">
        ...
        <div class="px-6 py-4 flex justify-between items-center bg-gradient-to-r from-indigo-500 via-purple-500 to-indigo-500 text-white">
            <h2 class="text-xl font-semibold flex items-center">
                <i class="fas fa-tags mr-2"></i> Detalles del Articulo
            </h2>
        ...
        <div class="px-6 py-4">
            <div class="flex items-center space-x-4">
                ...
                <p class="text-lg font-medium text-gray-800">
                    <strong>Nombre del Articulo:</strong> <span id="planta"></span>
                </p>
            </div>
            <div class="flex items-center space-x-4">
 ...
 <div class="container mx-auto px-4 py-6">
    ...
    <div class="flex justify-between items-center mb-4">
      ...
      <div class="space-x-2">
        <button  onclick="window.location.href='formulario_articulo.php'" class="bg-indigo-500 text-white px-4 py-2 rounded hover:bg-indigo-600">
...
<div id="filterForm" class="hidden mt-2 bg-white border border-gray-300 rounded shadow-lg p-4">
   ...
   <div id="savedFilters" class="mt-4">
    ...
    <form id="filterOptionsForm">
        <!-- Campo: Nombre de Articulo -->
        <div class="mb-4">
            <label for="nombre" class="block text-gray-700 mb-2">Nombre de Articulo</label>
            <input 
              type="text" 
              id="nombre" 
        ...
        <div class="grid grid-cols-2 gap-4 mb-4">
            ...
            <!-- planta -->
            <div>
                <label for="planta" class="block text-gray-700 mb-2">Planta</label>
                <select 
                  id="planta" 
                  name="planta"
                  class="w-full border border-gray-300 rounded p-2 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                >
                    ...
                    $query = "SELECT id_planta, nombre FROM planta";
                    $result = $conexion->query($query);

                    // Generar opciones dinámicamente
                    if ($result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                            $id_planta = htmlspecialchars($row["id_planta"]);
                            $nombre_planta = htmlspecialchars($row["nombre"]);
                            echo "<option value='$id_planta'>$nombre_planta</option>";
                        }
                    } else {
                        echo "<option value=''>No hay planta disponibles</option>";
                    }
...
<div class="border border-gray-300 rounded-lg shadow-md p-4">
    ...
    <div class="flex justify-between items-center mb-4 border-b border-gray-300 pb-2">
        <h1 class="text-xl font-semibold text-gray-700">Articulos</h1>
        <div class="relative">
        ...
        <table class="table-auto w-full border-collapse border border-gray-300 shadow-lg rounded-lg">
            ...
            <tbody id="dataTable" class="bg-white divide-y divide-gray-200">
                ...
                <tr class="hover:bg-gray-100">
                    <td class="px-6 py-4"><?php echo $articulo['id_articulo']; ?></td>
                    <td class="px-6 py-4"><?php echo $articulo['nombre_articulo']; ?></td>
                    <td class="px-6 py-4"><?php echo $articulo['descripcion_articulo']; ?></td>
                    <td class="px-6 py-4 flex space-x-2">
                    <button 
        class="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600"
        onclick="showModal(<?php echo $articulo['id_articulo']; ?>)">
...
function applyFilters() {
    ...
    window.location.href = `?nombre=${encodeURIComponent(nombre)}&status=${status}&planta=${planta}&fechaInicio=${fechaInicio}&fechaFinal=${fechaFinal}`;
}
...
document.getElementById('searchInput').addEventListener('keyup', filterTable);
  ...
  <script>
    function showModal(valor) {
        fetch(`getarticulo_planta.php?id_articulo=${valor}`)
            .then(response => response.json())
            .then(data => {
                document.getElementById('planta').textContent = data.nombre;
                document.getElementById('fecha').textContent = data.fecha;

                const Relacionados = document.getElementById('Relacionados');
                Relacionados.innerHTML = ''; // Limpiar contenido previo

                data.articulo.forEach((modelo, index) => {
                    ...
                    icono.className = "fas fa-store text-indigo-500";
