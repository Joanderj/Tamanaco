...
$sql_solicitudes = "
    ...
    FROM solicitudes so
    JOIN movimiento_herramientas ma ON ma.id_solicitud = so.id_solicitud
            ...
            <tr>
                ...
                <td class="py-2 px-4 border">
                    <button class="bg-green-500 text-white p-2 rounded" 
                            onclick="verDetalles(<?php echo htmlspecialchars($solicitud['herramienta_id']); ?>, <?php echo htmlspecialchars($solicitud['id_solicitud']); ?>)">
                        Ver Entrega
                    </button>
                    
                    <!-- Formulario para actualizar inventario -->
                    <?php if (isset($id_status) && $id_status == 1): // Solo mostrar si el estado es "Entregando" ?>
                    <form action="actualizar_inventario_herramienta_entrega.php" method="POST" class="inline-block">
                        <input type="hidden" name="herramienta" value="<?php echo htmlspecialchars($solicitud['herramienta_id']); ?>">
...
<!-- Botón Volver -->
<div class="mt-4">
    <a href="inventario_herramienta.php" class="bg-gray-500 text-white p-2 rounded">Volver</a>
</div>
...
<script>
function verDetalles(id_herramienta, id_solicitud) {
    // Realizar una petición AJAX para obtener los datos
    fetch(`ver_detalles_entrega_herramienta.php?id_solicitud=${id_solicitud}&id_herramienta=${id_herramienta}`)
        ...
        .then(data => {
            ...
            if (data.length > 0) {
                const contenido = `
                    ...
                    <p>Nombre de herramienta: ${data[0].nombre_herramienta}</p>
