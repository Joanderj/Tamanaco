...
// Submenú actual: Sede (id_submenu = 9)
$submenu_actual = 9;
...
$totalQuery = "
    SELECT COUNT(DISTINCT p.id_planta) 
    FROM planta p 
    INNER JOIN planta_articulo pl ON p.id_planta = pl.id_planta 
    INNER JOIN status st ON p.id_status = st.id_status
    INNER JOIN sede se ON p.id_sede = se.id_sede
    WHERE 1=1
";
$params = [];

if (!empty($nombre)) {
    $totalQuery .= " AND p.nombre LIKE ?"; // Cualificado con 'p=planta.'
...
if (!empty($sede)) {
    $totalQuery .= " AND p.id_sede = ?";
    $params[] = $sede;
}

if (!empty($articulo)) {
    $totalQuery .= " AND pl.id_articulo = ?"; // Cualificado con 'pl.'
    $params[] = $articulo;
}

// Agregar filtro por fechas
if (!empty($fechaInicio)) {
    $totalQuery .= " AND p.fecha_creacion >= ?";
    $params[] = $fechaInicio;
}
if (!empty($fechaFinal)) {
    $totalQuery .= " AND p.fecha_creacion <= ?";
...
// Consulta de datos
$query = "
    SELECT  DISTINCT p.id_planta, p.nombre,st.nombre_status,se.nombre_sede
    FROM planta p 
    INNER JOIN planta_articulo pl ON p.id_planta = pl.id_planta
    INNER JOIN status st ON p.id_status = st.id_status
    INNER JOIN sede se ON p.id_sede = se.id_sede
...
if (!empty($nombre)) {
    $query .= " AND p.nombre LIKE ?"; // Cualificado con 'p=planta.'
}
...
if (!empty($articulo)) {
    $query .= " AND pl.id_articulo = ?"; // Cualificado con 'pl=planta y articulo.'
}
...
<div id="modalver" class="fixed inset-0 bg-gray-900 bg-opacity-75 hidden justify-center items-center z-50">
    <div class="bg-white rounded-xl shadow-lg w-full max-w-lg overflow-hidden">
        ...
        <div class="px-6 py-4 flex justify-between items-center bg-gradient-to-r from-indigo-500 via-purple-500 to-indigo-500 text-white">
            <h2 class="text-xl font-semibold flex items-center">
                <i class="fas fa-tags mr-2"></i> Detalles de la Planta
            </h2>
        ...
        <div class="px-6 py-4">
            <div class="flex items-center space-x-4">
                ...
                <p class="text-lg font-medium text-gray-800">
                    <strong>Nombre de la Planta:</strong> <span id="planta"></span>
                </p>
            </div>
            <div class="flex items-center space-x-4">
                <i class="fas fa-map-marker-alt text-indigo-500 text-2xl"></i>
                <p class="text-lg font-medium text-gray-800">
                    <strong>Nombre de la Sede:</strong> <span id="sede"></span>
                </p>
            </div>
            <div class="flex items-center space-x-4">
            ...
            <div class="flex items-center space-x-4">
                ...
                <p class="text-lg font-medium text-gray-800">
                    <strong>Estado Actual:</strong> <span id="status"></span>
                </p>
            </div>
 ...
 <div class="container mx-auto px-4 py-6">
    ...
    <div class="flex justify-between items-center mb-4">
      ...
      <div class="space-x-2">
        <button  onclick="window.location.href='formulario_planta.php'" class="bg-indigo-500 text-white px-4 py-2 rounded hover:bg-indigo-600">
...
<div id="filterForm" class="hidden mt-2 bg-white border border-gray-300 rounded shadow-lg p-4">
   ...
   <div id="savedFilters" class="mt-4">
    ...
    <form id="filterOptionsForm">
        ...
        <div class="mb-4">
            <label for="nombre" class="block text-gray-700 mb-2">Nombre de Planta</label>
            <input 
              type="text" 
              id="nombre" 
        ...
        <div>
            <!-- Campo: articulo -->
                <label for="articulo" class="block text-gray-700 mb-2">Articulo</label>
                <select 
                  id="articulo" 
                  name="articulo"
                  class="w-full border border-gray-300 rounded p-2 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                >
                    <option value="">Selecciona una Articulo</option>
                    <?php
                    // Conexión a la base de datos
                    $conexion = new mysqli("localhost", "root", "", "bd_tamanaco");
                    ...
                    // Consultar las articulo
                    $query = "SELECT id_articulo, nombre_articulo FROM articulo";
                    $result = $conexion->query($query);

                    // Generar opciones dinámicamente
                    if ($result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                            $idarticulo = htmlspecialchars($row["id_articulo"]);
                            $nombrearticulo = htmlspecialchars($row["nombre_articulo"]);
                            echo "<option value='$idarticulo'>$nombrearticulo</option>";
                        }
                    } else {
                        echo "<option value=''>No hay articulo disponibles</option>";
                    }
...
<div class="border border-gray-300 rounded-lg shadow-md p-4">
    ...
    <div class="flex justify-between items-center mb-4 border-b border-gray-300 pb-2">
        <h1 class="text-xl font-semibold text-gray-700">Plantas</h1>
        <div class="relative">
        ...
        <table class="table-auto w-full border-collapse border border-gray-300 shadow-lg rounded-lg">
            ...
            <tbody id="dataTable" class="bg-white divide-y divide-gray-200">
                ...
                <tr class="hover:bg-gray-100">
                    <td class="px-6 py-4"><?php echo $planta['id_planta']; ?></td>
                    <td class="px-6 py-4"><?php echo $planta['nombre']; ?></td>
                    <td class="px-6 py-4"><?php echo $planta['nombre_sede']; ?></td>
                    <td class="px-6 py-4"><?php echo $planta['nombre_status']; ?></td>
                    <td class="px-6 py-4 flex space-x-2">
                       <button 
        class="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600"
        onclick="showModal(<?php echo $planta['id_planta']; ?>)">
...
function applyFilters() {
    ...
    window.location.href = `?nombre=${encodeURIComponent(nombre)}&status=${status}&articulo=${articulo}&sede=${sede}&fechaInicio=${fechaInicio}&fechaFinal=${fechaFinal}`;
}
...
document.getElementById('searchInput').addEventListener('keyup', filterTable);
  ...
  <script>
    function showModal(valor) {
        fetch(`getplanta_articulo.php?id_planta=${valor}`)
            ...
            .then(data => {
                ...
                data.articulo.forEach((modelo, index) => {
                    ...
                    icono.className = "fas fa-box-open text-indigo-500";
