...
<div id="modalProducto" class="fixed inset-0 bg-gray-900 bg-opacity-75 hidden justify-center items-center z-50">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden">
        <!-- Encabezado del modal -->
        <div class="px-6 py-4 flex justify-between items-center bg-gradient-to-r from-indigo-600 to-purple-600 text-white">
            <h2 class="text-xl font-semibold flex items-center">
                <i class="fas fa-box mr-2"></i> Detalles del Producto
            </h2>
            <button onclick="closeModal()" class="text-white text-lg hover:text-red-300 transition duration-200">
                <i class="fas fa-times"></i>
            </button>
        </div>
        ...
        <div class="px-6 py-6">
            ...
            <div class="flex justify-center mb-6">
                <img id="productoImagen" src="" alt="Imagen del Producto" class="w-40 h-40 object-contain rounded-lg shadow-md border border-gray-200">
            </div>

            <!-- Nombre y Fecha -->
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-4">
                <div class="flex items-start space-x-3">
                    <i class="fas fa-tags text-indigo-500 text-xl mt-1"></i>
                    <div>
                        <p class="text-sm text-gray-500">Nombre del Producto</p>
                        <p id="productoNombre" class="text-lg font-semibold text-gray-800"></p>
                    </div>
                </div>
                <div class="flex items-start space-x-3">
                    ...
                    <div>
                        ...
                        <p id="productoFecha" class="text-lg font-semibold text-gray-800"></p>
                    </div>
                </div>
            </div>
            ...
            <div class="mt-4">
                <h3 class="text-base font-semibold text-gray-700 mb-2">Detalles Relacionados</h3>
                <ul id="detallesRelacionados" class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <!-- Detalles se insertan dinámicamente aquí -->
                </ul>
            </div>
        </div>

        <!-- Pie del modal (animación o decorativo) -->
        <div class="px-6 py-4 bg-gray-50 flex justify-center rounded-b-2xl">
            <div class="flex gap-2">
                <div class="w-3 h-3 rounded-full bg-blue-500 animate-bounce delay-75"></div>
                <div class="w-3 h-3 rounded-full bg-green-500 animate-bounce delay-150"></div>
                <div class="w-3 h-3 rounded-full bg-yellow-500 animate-bounce delay-300"></div>
            </div>
        </div>
    </div>
</div>
...
<div class="flex justify-between items-center mb-4">
    ...
    <div class="space-x-2">
        <?php if (in_array(2, $permisos)): ?>
            <button 
                onclick="window.location.href='formulario_guardar_producto.php'" 
        ...
        <?php if (in_array(4, $permisos)): ?>
            <button 
                onclick="window.location.href='imprimir_producto.php'" 
...
<div id="filterForm" class="hidden mt-2 bg-white border border-gray-300 rounded shadow-lg p-4">
   ...
   <div id="savedFilters" class="mt-4">
    ...
    <div class="mb-4">
        <label for="nombreProducto" class="block text-gray-700 mb-2">Nombre del Producto</label>
        <input 
            type="text" 
            id="nombreProducto" 
    ...
    <!-- Campo: Tipo y Clasificación -->
    <div class="grid grid-cols-2 gap-4 mb-4">
        <!-- Tipo -->
        <div>
            <label for="idTipo" class="block text-gray-700 mb-2">Tipo</label>
            <select 
                id="idTipo" 
                class="w-full border border-gray-300 rounded p-2 focus:outline-none focus:ring-2 focus:ring-indigo-500"
            >
                ...
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        ...
                        $nombreTipo = htmlspecialchars($row["nombre_tipo"]);
        ...
        <!-- Clasificación -->
        <div>
            <label for="idClasificacion" class="block text-gray-700 mb-2">Clasificación</label>
            <select 
                id="idClasificacion" 
                class="w-full border border-gray-300 rounded p-2 focus:outline-none focus:ring-2 focus:ring-indigo-500"
            >
                ...
                $query = "SELECT id_clasificacion, nombre_clasificacion FROM clasificacion";
                $result = $conexion->query($query);

                // Generar opciones dinámicamente
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        $idClasificacion = htmlspecialchars($row["id_clasificacion"]);
                        $nombreClasificacion = htmlspecialchars($row["nombre_clasificacion"]);
                        echo "<option value='$idClasificacion'>$nombreClasificacion</option>";
                    }
                } else {
...
<div class="border border-gray-300 rounded-lg shadow-md p-4">
    ...
    <div class="flex justify-between items-center mb-4 border-b border-gray-300 pb-2">
        <h1 class="text-xl font-semibold text-gray-700">Productos</h1>
        <div class="relative">
        ...
        <tr class="hover:bg-gray-100">
            ...
            <td class="px-6 py-4">
                <?php if (!empty($producto['url'])): ?>
                    <img src="<?php echo htmlspecialchars($producto['url']); ?>" alt="<?php echo htmlspecialchars($producto['nombre_imagen']); ?>" class="w-32 h-32 object-cover rounded-md">
                <?php else: ?>
                    <span>Sin Imagen</span>
                <?php endif; ?>
            </td>
    ...
    <li class="flex items-center space-x-2">
        ...
        <strong>ID:</strong> <?php echo htmlspecialchars($producto['id_producto']); ?>
    </li>
    <li class="flex items-center space-x-2">
    ...
    <li class="flex items-center space-x-2">
        ...
        <strong>Marca:</strong> <?php echo htmlspecialchars($producto['nombre_marca']); ?>
    </li>
    <li class="flex items-center space-x-2">
        <i class="fas fa-cube text-yellow-500"></i>
        <strong>Modelo:</strong> <?php echo htmlspecialchars($producto['nombre_modelo']); ?>
    </li>
    <li class="flex items-center space-x-2">
        <i class="fas fa-boxes text-orange-500"></i>
        <strong>Tipo:</strong> <?php echo htmlspecialchars($producto['nombre_tipo']); ?>
    </li>
    <li class="flex items-center space-x-2">
        <i class="fas fa-layer-group text-purple-500"></i>
        <strong>Clasificación:</strong> <?php echo htmlspecialchars($producto['nombre_clasificacion']); ?>
    </li>
    <li class="flex items-center space-x-2">
        <i class="fas fa-check-circle text-teal-500"></i>
        <strong>Estado:</strong> <?php echo htmlspecialchars($producto['nombre_status']); ?>
    </li>
    <li class="flex items-center space-x-2">
        <i class="fas fa-calendar-alt text-pink-500"></i>
        <strong>Fecha:</strong> <?php echo htmlspecialchars($producto['date_created']); ?>
    </li>
</ul>
            </td>
...
$idPerfil = $_SESSION['id_perfil'];
$idSubmenu = 15; // ID del submenú correspondiente
$permisos = obtenerPermisos($idPerfil, $idSubmenu);
?>

<td class="px-6 py-4 flex space-x-2">
    <?php if ($producto['nombre_status'] == 'Activo'): ?>
        
        <?php if (in_array(1, $permisos)): ?>
        <button 
            class="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600"
            onclick="showModal(<?php echo $producto['id_producto']; ?>)">
        ...
        <button 
            ...
            onclick="window.location.href='formulario_modificar_producto.php?id_producto=<?php echo $producto['id_producto']; ?>'">
        ...
        <button 
            class="bg-green-500 text-white px-4 py-2 rounded hover:bg-green-600" 
            onclick="window.location.href='imprimir_producto_individual.php?id_producto=<?php echo $producto['id_producto']; ?>'">
...
function applyFilters() {
    ...
    const idClasificacion = document.getElementById('idClasificacion').value;
    const fechaInicio = document.getElementById('fechaInicio').value;
    const fechaFinal = document.getElementById('fechaFinal').value;

    // Redirigir a la misma página con los filtros aplicados como parámetros
    window.location.href = `?nombreProducto=${encodeURIComponent(nombreProducto)}&idMarca=${idMarca}&idModelo=${idModelo}&idTipo=${idTipo}&idClasificacion=${idClasificacion}&fechaInicio=${fechaInicio}&fechaFinal=${fechaFinal}`;
}
...
document.getElementById('searchInput').addEventListener('keypress', function(event) {
    if (event.key === 'Enter') {
        ...
        window.location.href = window.location.pathname + `?nombreProducto=${encodeURIComponent(searchValue)}`;
    }
});
</script>
  <script>
...
<script>
    // Función para mostrar el modal con los detalles del producto y sus relaciones
    function showModal(idProducto) {
        fetch(`getProductoDetalles.php?id_producto=${encodeURIComponent(idProducto)}`)
            .then(response => {
                if (!response.ok) {
            ...
            .then(data => {
                if (!data || !data.nombreProducto || !data.fechaCreacion || !data.detalles) {
                ...
                const detallesRelacionados = document.getElementById('detallesRelacionados');
                detallesRelacionados.innerHTML = '';

                let imagenSeteada = false;

                data.detalles.forEach((detalle, index) => {
                    // Si el detalle contiene la imagen
                    if (detalle.startsWith("Imagen:")) {
                        const urlImagen = detalle.replace("Imagen: ", "").trim();
                        document.getElementById('productoImagen').src = urlImagen;
                        imagenSeteada = true;
                        return; // No mostrar este detalle como texto
                    }

                    // Crear ítem de lista para detalles normales
                    const li = document.createElement('li');
                    li.className = "flex items-start space-x-2 bg-gray-100 p-2 rounded shadow-sm";
                    ...
                    icono.className = "fas fa-check-circle text-indigo-400 mt-0.5";

                    const texto = document.createElement('span');
                    texto.className = "text-gray-800 text-sm";
                    ...
                    detallesRelacionados.appendChild(li);
                });

                // Si no se encontró imagen en los detalles, usar imagen por defecto
                if (!imagenSeteada) {
                    document.getElementById('productoImagen').src = 'ruta_a_imagenes/default.png';
                }
