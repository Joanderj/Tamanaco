...
$nombreProveedor = isset($_GET['nombreProveedor']) ? $_GET['nombreProveedor'] : ''; // Nombre del proveedor
$status = isset($_GET['status']) ? $_GET['status'] : ''; // Estado del proveedor
$fechaInicio = isset($_GET['fechaInicio']) ? $_GET['fechaInicio'] : ''; // Fecha de inicio del filtro
$fechaFinal = isset($_GET['fechaFinal']) ? $_GET['fechaFinal'] : ''; // Fecha final del filtro

// Clasificación
$orderBy = 'proveedor.id_proveedor'; // Campo por defecto para ordenar
if (isset($_GET['sort'])) {
    switch ($_GET['sort']) {
        case 'fecha_asc':
            $orderBy = 'proveedor.date_created ASC';
            break;
        case 'fecha_desc':
            $orderBy = 'proveedor.date_created DESC';
            break;
        case 'nombre_asc':
            $orderBy = 'proveedor.nombre_proveedor ASC';
            break;
        case 'nombre_desc':
            $orderBy = 'proveedor.nombre_proveedor DESC';
            break;
    }
}

// Consulta total de elementos (proveedores únicos con filtros)
$totalQuery = "
    SELECT COUNT(proveedor.id_proveedor) 
    FROM proveedor
    WHERE 1=1
";
$params = [];
...
if (!empty($nombreProveedor)) {
    $totalQuery .= " AND proveedor.nombre_proveedor LIKE ?";
    $params[] = '%' . $nombreProveedor . '%';
}
if (!empty($status)) {
    $totalQuery .= " AND proveedor.id_status = ?";
    $params[] = $status;
}
if (!empty($fechaInicio)) {
    $totalQuery .= " AND proveedor.date_created >= ?";
    $params[] = $fechaInicio;
}
if (!empty($fechaFinal)) {
    $totalQuery .= " AND proveedor.date_created <= ?";
...
// Consulta principal de datos (proveedores con paginación)
$query = "
    SELECT proveedor.id_proveedor, proveedor.nombre_proveedor, proveedor.telefono, proveedor.email, proveedor.direccion, proveedor.date_created, proveedor.id_status
    FROM proveedor
    WHERE 1=1
";
...
// Agregar filtros
if (!empty($nombreProveedor)) {
    $query .= " AND proveedor.nombre_proveedor LIKE ?";
}
if (!empty($status)) {
    $query .= " AND proveedor.id_status = ?";
}
if (!empty($fechaInicio)) {
    $query .= " AND proveedor.date_created >= ?";
}
if (!empty($fechaFinal)) {
    $query .= " AND proveedor.date_created <= ?";
}
...
<div id="modalProveedorInfo" class="hidden fixed inset-0 bg-black bg-opacity-50 justify-center items-center z-50">
    <div class="bg-white p-6 rounded-lg shadow-lg w-full max-w-lg">
        <div class="flex justify-between items-center mb-4">
            <h2 class="text-xl font-bold text-indigo-700">Información del Proveedor</h2>
            <button onclick="closeProveedorInfo()" class="text-red-500 text-2xl font-bold hover:text-red-700">&times;</button>
        </div>
        <div class="space-y-2">
            <p><strong class="text-gray-600">Nombre:</strong> <span id="infoNombre" class="text-gray-800"></span></p>
            <p><strong class="text-gray-600">Teléfono:</strong> <span id="infoTelefono" class="text-gray-800"></span></p>
            <p><strong class="text-gray-600">Email:</strong> <span id="infoEmail" class="text-gray-800"></span></p>
            <p><strong class="text-gray-600">Dirección:</strong> <span id="infoDireccion" class="text-gray-800"></span></p>
            <p><strong class="text-gray-600">Fecha de Registro:</strong> <span id="infoFecha" class="text-gray-800"></span></p>
        </div>
    </div>
</div>
...
<div class="flex justify-between items-center mb-4">
    ...
    <div class="space-x-2">
        <?php if (in_array(2, $permisos)): ?>
            <button 
                onclick="window.location.href='formulario_guardar_proveedor.php'" 
        ...
        <?php if (in_array(4, $permisos)): ?>
            <button 
                onclick="window.location.href='imprimir_proveedores.php'" 
...
<div id="filterForm" class="hidden mt-2 bg-white border border-gray-300 rounded shadow-lg p-4">
   ...
   <div id="savedFilters" class="mt-4">
    ...
    <form id="filterOptionsForm" method="get" action="proveedor.php" class="space-y-6">
    <!-- Campo: Nombre del Proveedor -->
    <div class="mb-4">
        <label for="nombreProveedor" class="block text-gray-700 font-semibold mb-2">Nombre del Proveedor</label>
        <input 
            type="text" 
            id="nombreProveedor" 
...
<div class="border border-gray-300 rounded-lg shadow-md p-4">
    ...
    <div class="flex justify-between items-center mb-4 border-b border-gray-300 pb-2">
        <h1 class="text-xl font-semibold text-gray-700">Proveedores</h1>
        <div class="relative">
    ...
    <tbody id="dataTable" class="bg-white divide-y divide-gray-200">
        ...
        <tr class="hover:bg-gray-100">
            <!-- ID del Proveedor -->
            <td class="px-6 py-4 text-gray-800 font-medium"><?php echo htmlspecialchars($proveedor['id_proveedor']); ?></td>
            <!-- Nombre del Proveedor -->
            <td class="px-6 py-4 text-gray-800"><?php echo htmlspecialchars($proveedor['nombre_proveedor']); ?></td>
            <!-- Teléfono -->
            <td class="px-6 py-4 text-gray-800"><?php echo htmlspecialchars($proveedor['telefono']); ?></td>
            <!-- Email -->
            <td class="px-6 py-4 text-gray-800"><?php echo htmlspecialchars($proveedor['email']); ?></td>
            <!-- Estado -->
            <td class="px-6 py-4 text-gray-800 font-medium">
                <?php echo ($proveedor['id_status'] == 1) ? "Activo" : "Inactivo"; ?>
            </td>
            <!-- Botones de Acción -->
           <?php
...
$idPerfil = $_SESSION['id_perfil'];
$idSubmenu = 18; // ID del submenú correspondiente
$permisos = obtenerPermisos($idPerfil, $idSubmenu);
?>

<td class="px-6 py-4 flex space-x-2">
    <?php if ($proveedor['id_status'] == 1): ?>
        
        <?php if (in_array(1, $permisos)): ?>
        <button 
    class="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600"
    onclick="showProveedorInfo(<?php echo $proveedor['id_proveedor']; ?>)">
    <i class="fas fa-eye"></i> Ver
        ...
        <button 
            ...
            onclick="window.location.href='formulario_modificar_proveedor.php?id_proveedor=<?php echo $proveedor['id_proveedor']; ?>'">
...
<script>
    function showProveedorInfo(idProveedor) {
        fetch(`getProveedorInfo.php?id_proveedor=${idProveedor}`)
            ...
            .then(data => {
                ...
                document.getElementById('infoNombre').textContent = data.nombre_proveedor || '-';
                document.getElementById('infoTelefono').textContent = data.telefono || '-';
                document.getElementById('infoEmail').textContent = data.email || '-';
                document.getElementById('infoDireccion').textContent = data.direccion || '-';
                document.getElementById('infoFecha').textContent = data.date_created || '-';

                document.getElementById('modalProveedorInfo').classList.remove('hidden');
                document.getElementById('modalProveedorInfo').classList.add('flex');
            })
            .catch(error => {
    ...
    function closeProveedorInfo() {
        const modal = document.getElementById('modalProveedorInfo');
