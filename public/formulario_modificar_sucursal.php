...
$id_sucursal = $_GET['id_sucursal'] ?? null; // Obtener el ID 
if ($id_sucursal) {
    $consulta1 = $conexion->query("SELECT nombre,direccion,pais_id_pais,estado_id_estado FROM sucursal WHERE id_sucursal = $id_sucursal");
    $sucursal = $consulta1->fetch_assoc();
}
?>
...
<hr>
 ...
 <div class="flex flex-col items-center mb-6">
    <div class="bg-blue-500 text-white w-16 h-16 rounded-full flex items-center justify-center shadow-lg mb-4">
        <i class="fas fa-store text-3xl"></i> <!-- Ícono de etiquetas -->
    </div>
    <h2 class="text-3xl font-extrabold text-gray-800">Formulario de Sucursal</h2>
    <p class="text-gray-600 mt-2 text-center">Edita Las Sucursales y Sus Vinculo con la Sede asociado de forma rápida y organizada.</p>
</div>
    <?php
...
<?php endif; ?>
    ...
    <form action="modificar_sucursal.php" method="POST" class="space-y-4" id="form">
        ...
        Nombre de la Sucursal: <span class="text-red-600">*</span>
        </label> 
       ...
       oninput="convertirMayusculas(this); validarNombre()">
            ...
            <small id="mensaje-error-input" class="text-red-500 hidden">Esta Sucursal ya existe</small>
            <small id="mensaje-exito-input" class="text-green-500 hidden">¡Esta Sucursa está disponible!</small>
...
<label for="sede" class="block text-gray-700 mb-2"><i class="fas fa-map-marker-alt text-blue-500 mr-2"></i>Sede<span class="text-red-600">*</span></label>
    <tbody>
    
      <select id="sede" name="sede" class="border border-gray-300 rounded p-2 w-full">
      <option value="" disabled selected>Seleccione una Sede</option>
      <?php
      // Conexión a la base de datos
      $conexion = new mysqli('localhost', 'root', '', 'bd_tamanaco');
      $consulta = "SELECT id_sede, nombre_sede FROM sede WHERE id_status = 1";
...
<p class="text-gray-500 text-sm mt-4">Todos los campos marcados con <span class="text-red-600">*</span> son obligatorios.</p>
        ...
        <div class="flex justify-between mt-4 space-x-4">
            <input type="text" name="id_sede" id="id_sede" value="<?php echo htmlspecialchars($id_sucursal); ?>" class="hidden">
    <!-- Botón Guardar -->
