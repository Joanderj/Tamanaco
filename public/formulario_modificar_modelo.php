...
$id_modelo = $_GET['id_modelo'] ?? null; // Obtener el ID del modelo desde la URL
if ($id_modelo) {
    // Obtener el nombre del modelo
    $consulta_modelo = $conexion->query("SELECT nombre_modelo FROM modelo WHERE id_modelo = $id_modelo");
    $modelo = $consulta_modelo->fetch_assoc();

    // Obtener las marcas asociadas con su id_status
    $consulta_marcas = $conexion->query("
        SELECT ma.id_marca, ma.nombre_marca, COALESCE(mm.id_status, 0) AS id_status
        FROM marca ma
        LEFT JOIN marca_modelo mm ON ma.id_marca = mm.id_marca AND mm.id_modelo = $id_modelo
    ");
}
?>
...
if ($id_modelo) {
    // Consulta para obtener marcas asociadas
    $consulta_marcas = $conexion->prepare("
        SELECT ma.id_marca, ma.nombre_marca, COALESCE(mm.id_status, 0) AS id_status
        FROM marca ma
        LEFT JOIN marca_modelo mm ON ma.id_marca = mm.id_marca AND mm.id_modelo = ?
        ORDER BY ma.nombre_marca ASC
    ");
    $consulta_marcas->bind_param("i", $id_modelo);
    $consulta_marcas->execute();
    $resultado = $consulta_marcas->get_result();
    ...
    $consulta_total = $conexion->prepare("
        ...
        LEFT JOIN marca_modelo mm ON ma.id_marca = mm.id_marca AND mm.id_modelo = ?
    ");
    $consulta_total->bind_param("i", $id_modelo);
    $consulta_total->execute();
    $total_resultados = $consulta_total->get_result()->fetch_assoc()['total'];
} else {
...
?>
 ...
 <div class="flex flex-col items-center mb-6">
    ...
    <p class="text-gray-600 mt-2 text-center">Registra los modelos y vincúlalos a marcas existentes de forma rápida y organizada.</p>
</div>
    <?php
...
<?php endif; ?>
    ...
    <form action="modificar_modelo.php" method="POST" class="space-y-4" id="form-modelo">
        <!-- Nombre del Modelo -->
        <div>
        <label for="nombre_modelo" class="block font-semibold flex items-center">
    <i class="fas fa-cube text-blue-500 mr-2"></i> <!-- Ícono de modelo -->
    Nombre del Modelo: <span class="text-red-600">*</span>
</label>
            <input type="text" id="nombre_modelo" name="nombre_modelo"
       placeholder="Ingrese el nombre del modelo"
       class="w-full border border-gray-300 rounded-lg p-2" value="<?php echo htmlspecialchars($modelo['nombre_modelo'] ?? ''); ?>"  required
       oninput="convertirMayusculas(this); validarNombreModelo()">
            <!-- Mensajes específicos para el input -->
            <small id="mensaje-error-input" class="text-red-500 hidden">Este modelo ya existe</small>
            <small id="mensaje-exito-input" class="text-green-500 hidden">¡Este modelo está disponible!</small>
            <small class="text-gray-500">Ejemplo: SEDAN, SUV, CAMIONETA</small>
        </div>
        <button type="button"  onclick="abrirVentanaEmergenteMarca()"
...
<script>
    function toggleMarcaSelection() {
        const selectOption = document.getElementById('vincular_opcion').value;
        document.getElementById('vincular_una').classList.add('hidden');
        document.getElementById('vincular_varias').classList.add('hidden');

        if (selectOption === 'una') {
            document.getElementById('vincular_una').classList.remove('hidden');
        } else if (selectOption === 'varias') {
            document.getElementById('vincular_varias').classList.remove('hidden');
        }
    }
</script>
...
<div id="vincular_varias">
    ...
    <input type="text" id="searchBox" 
        ...
        placeholder="Buscar marca..." onkeyup="filterTable()">
    ...
    <div class="max-h-60 overflow-y-auto border border-gray-300 rounded-lg shadow-md mt-3">
        <table class="table-auto w-full border-collapse">
            ...
            $resultado = $conexion->query("
                SELECT ma.id_marca, ma.nombre_marca, 
                       COALESCE(mm.id_status, 0) AS id_status
                FROM marca ma
                LEFT JOIN marca_modelo mm ON ma.id_marca = mm.id_marca AND mm.id_modelo = $id_modelo
                ORDER BY ma.nombre_marca ASC
            ");
            ...
            while ($fila = $resultado->fetch_assoc()) {
                echo "<tr class='marca-row'>
                        <td class='px-4 py-2 border text-center'>
                            <input type='checkbox' name='marcas[" . $fila['id_marca'] . "]' value='1' 
                            " . ($fila['id_status'] == 1 ? 'checked' : '') . ">
                        </td>
                        <td class='px-4 py-2 border'>" . htmlspecialchars($fila['nombre_marca'], ENT_QUOTES, 'UTF-8') . "</td>
                      </tr>";
            }
            ?>
            </tbody>
...
<input type="hidden" name="id_modelo" value="<?php echo htmlspecialchars($id_modelo); ?>">
    <!-- Botón Regresar -->
    <button type="button" onclick="location.href='modelo.php';"
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        botonGuardar.disabled = false; // Enable the button initially
    ...
    function validarNombreModelo() {
        ...
        const idModelo = document.getElementById('id_modelo').value; // Assuming there's a hidden input with the current model ID
        ...
        if (nombreModelo !== '') {
            fetch(`validar_modelo_modificar.php?nombre=${encodeURIComponent(nombreModelo)}&id_modelo=${encodeURIComponent(idModelo)}`)
                .then(response => response.json())
                .then(data => {
                    if (data.existe && data.id_modelo !== parseInt(idModelo)) {
                        ...
                        textoMensajeGlobal.textContent = "Este modelo ya existe. Por favor, ingrese otro.";
...
<!-- Modal Structure -->
<div id="ventanaEmergenteMarca" class="fixed inset-0 bg-black bg-opacity-50 hidden flex justify-center items-center">
    <div class="bg-white max-w-4xl mx-auto rounded-lg shadow-lg p-6">
        <div class="flex flex-col items-center mb-6">
            <div class="bg-blue-500 text-white w-16 h-16 rounded-full flex items-center justify-center shadow-lg mb-4">
                <i class="fas fa-tags text-3xl"></i> <!-- Ícono de marca -->
            </div>
            <h2 class="text-3xl font-extrabold text-gray-800">Formulario de Marca</h2>
            <p class="text-gray-600 mt-2 text-center">Registra una marca de forma rápida y organizada.</p>
        </div>
        <!-- Contenedor global para mensajes de error -->
        <div id="mensaje-global-marca" class="hidden bg-red-100 text-red-700 p-4 rounded-lg mb-4">
            <strong id="tipo-mensaje-global-marca"></strong> <span id="texto-mensaje-global-marca"></span>
        </div>

        <form id="formRegistrarMarca" method="POST" action="guardar_marca_modelo.php">
            <div class="mb-4">
                <label for="nombre_marca" class="block text-gray-700 font-bold mb-2">Nombre de la Marca <span class="text-red-600">*</span></label>
                <input type="text" id="nombre_marca" name="nombre_marca" placeholder="Ingrese el nombre de la marca"
                       class="border border-gray-300 rounded-lg p-2 w-full" required oninput="convertirMayusculas(this); validarNombreMarca()">
                ...
                <small class="text-gray-500">Ejemplo: Toyota, Nike, Apple</small>
            </div>
            <p class="text-gray-500 text-sm mt-4">Todos los campos marcados con <span class="text-red-600">*</span> son obligatorios.</p>
            <div class="flex justify-between mt-4 space-x-4">
...
<script>
// Function to open the modal for brand registration
function abrirVentanaEmergenteMarca() {
    const ventana = document.getElementById('ventanaEmergenteMarca');
    if (ventana) {
        ventana.classList.remove('hidden'); // Show the modal
    } else {
        console.error('Elemento con ID "ventanaEmergenteMarca" no encontrado.');
    }
}
...
function cerrarVentanaEmergenteMarca() {
    const ventana = document.getElementById('ventanaEmergenteMarca');
    if (ventana) {
        ventana.classList.add('hidden'); // Hide the modal
        const form = document.getElementById('formRegistrarMarca');
        if (form) {
...
// Function to show styled messages with content centered
function mostrarMensaje(tipo, mensaje) {
    ...
    // Create the message icon container
    ...
    icono.classList.add(tipo === 'error' ? 'fa-exclamation-triangle' : 'fa-check-circle');
    icono.classList.add(tipo === 'error' ? 'text-red-500' : 'text-green-500');

    // Append the icon to the message icon container
...
// Handle form submission for brand registration
document.getElementById('formRegistrarMarca').addEventListener('submit', function (event) {
    event.preventDefault(); // Prevent page reload

    const formData = new FormData(this); // Create a FormData object with form data

    // Fetch request to the server
    fetch('guardar_marca_modelo.php', {
        method: 'POST',
        body: formData,
    })
    .then(response => response.json()) // Parse the response to JSON
    .then(data => {
        console.log(data); // For debugging: log the response to the console

        if (data.success) {
            // Update the table dynamically
            const tbody = document.querySelector('#vincular_varias tbody'); // Select table body
            const row = document.createElement('tr');
            row.classList.add('hover:bg-blue-50');
            row.innerHTML = `
                <td class='px-4 py-2'>
                    <input type='checkbox' name='marcas[]' value='${data.id_marca}' onchange='highlightRow(this)'>
                </td>
                <td class='px-4 py-2 text-gray-700'>${data.nombre_marca}</td>
            `;
            tbody.appendChild(row); // Add the new row

            // Update the select dynamically
            const selectMarca = document.getElementById('marca_una');
            const nuevaOpcion = document.createElement('option');
            nuevaOpcion.value = data.id_marca;
            nuevaOpcion.textContent = data.nombre_marca;
            selectMarca.appendChild(nuevaOpcion); // Add to select

            // Show success message
            mostrarMensaje('success', '¡Marca se ha guardado exitosamente!');

            // Close the modal
            cerrarVentanaEmergenteMarca();
        } else {
            // Show error message
    ...
    .catch(error => {
        ...
        mostrarMensaje('error', 'Ocurrió un error al intentar guardar la Marca.');
    });
});
</script>
...
<script>
   function validarNombreMarca() {
    const nombreMarca = document.getElementById('nombre_marca').value.trim(); // Limpiar espacios
    const mensajeErrorInputMarca = document.getElementById('mensaje-error-input-marca'); // Mensaje de error
    const mensajeExitoInputMarca = document.getElementById('mensaje-exito-input-marca'); // Mensaje de éxito
    const mensajeGlobalMarca = document.getElementById('mensaje-global-marca'); // Contenedor del mensaje global
    const tipoMensajeGlobalMarca = document.getElementById('tipo-mensaje-global-marca'); // Tipo de mensaje global
    const textoMensajeGlobalMarca = document.getElementById('texto-mensaje-global-marca'); // Texto del mensaje global
    const botonGuardar = document.getElementById('guardar'); // Botón de guardar

    if (nombreMarca !== '') { // Validación dinámica
        fetch(`validar_marca.php?nombre=${encodeURIComponent(nombreMarca)}`)
            .then(response => response.json())
            .then(data => {
                if (data.existe) {
                    // Mostrar mensaje de error
                    mensajeErrorInputMarca.classList.remove('hidden');
                    mensajeExitoInputMarca.classList.add('hidden');

                    // Configurar mensaje global de error
                    tipoMensajeGlobalMarca.textContent = "Error:";
                    textoMensajeGlobalMarca.textContent = "Esta marca ya existe. Por favor, ingrese otro.";
                    mensajeGlobalMarca.classList.remove('hidden');
                    mensajeGlobalMarca.classList.add('bg-red-100', 'text-red-700');
                    ...
                    // Mostrar mensaje de éxito
                    mensajeErrorInputMarca.classList.add('hidden');
                    mensajeExitoInputMarca.classList.remove('hidden');

                    // Ocultar mensaje global
                    mensajeGlobalMarca.classList.add('hidden');
...
function filterTable() {
    ...
    const rows = document.querySelectorAll('#marcaTableBody .marca-row');
    ...
    rows.forEach(row => {
        ...
        if (marcaName.includes(searchValue)) {
            row.style.display = '';
        } else {
