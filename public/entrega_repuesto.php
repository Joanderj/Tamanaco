...
$sql_solicitudes = "
    ...
    FROM solicitudes so
    JOIN movimiento_repuesto ma ON ma.id_solicitud = so.id_solicitud
            ...
            <tr>
                ...
                <td class="py-2 px-4 border">
                    <button class="bg-green-500 text-white p-2 rounded" 
                            onclick="verDetalles(<?php echo htmlspecialchars($solicitud['id_repuesto']); ?>, <?php echo htmlspecialchars($solicitud['id_solicitud']); ?>)">
                        Ver Entrega
                    </button>
                    
                    <!-- Formulario para actualizar inventario -->
                    <?php if (isset($id_status) && $id_status == 1): // Solo mostrar si el estado es "Entregando" ?>
                    <form action="actualizar_inventario_repuesto_entrega.php" method="POST" class="inline-block">
                        <input type="hidden" name="repuesto" value="<?php echo htmlspecialchars($solicitud['id_repuesto']); ?>">
...
<!-- Botón Volver -->
<div class="mt-4">
    <a href="inventario_repuesto.php" class="bg-gray-500 text-white p-2 rounded">Volver</a>
</div>
...
<script>
function verDetalles(id_repuesto, id_solicitud) {
    // Realizar una petición AJAX para obtener los datos
    fetch(`ver_detalles_entrega_repuesto.php?id_solicitud=${id_solicitud}&id_repuesto=${id_repuesto}`)
