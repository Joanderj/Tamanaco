...
<hr>
 ...
 <div class="flex flex-col items-center mb-6">
    ...
    <p class="text-gray-600 mt-2 text-center">Registra los Almacenes y vincula los Almacenes con la Sede de forma rápida y organizada.</p>
</div>
    <?php
...
<div class="max-w-3xl mx-auto bg-white p-8 shadow-lg rounded-lg">
  <h2 class="text-2xl font-bold text-gray-700 mb-6">Registrar almacen</h2>

  <form id="formRegistraralmacen" action="registrar_almacen.php" method="POST">
    <!-- Nombre de la almacen -->
    <div class="mb-4">
      <label for="almacen" class="block text-gray-700 font-bold mb-2"><i class="fas fa-warehouse text-blue-500 mr-2"></i>Nombre de la almacen:<span class="text-red-600">*</span></label>
      <input type="text" id="almacen" name="almacen"  placeholder="Ingrese el nombre del almacen"
       ...
       <small id="mensaje-error-input" class="text-red-500 hidden">Este Almacen ya existe</small>
            ...
            <small class="text-gray-500">Ejemplo: ALMACEN ELECTRICO</small>
    </div>
...
<div class="mb-4">
      ...
      <div class="flex items-center gap-4">
        ...
        <button type="button" onclick="abrirVentanaEmergente2()"
   ...
   <!-- Vincular más almacenes -->
<div class="mb-4">
  <label for="vincularalmacenes" class="block text-gray-700 font-bold mb-2">
    ¿Deseas vincular esta Almacen a una Sede?
  </label>
  <select 
    id="vincularalmacenes" 
    class="border border-gray-300 rounded p-2 w-full focus:outline-none focus:ring-2 focus:ring-indigo-500"
    onchange="togglealmacenTable()"
  >
...
<!-- Tabla de sede (oculta por defecto) -->
<div id="almacenTable" class="hidden mt-4">
<label for="sucursal_principal" class="block text-gray-700 font-bold mb-2">
      <i class="fas fa-map-marker-alt text-blue-500 mr-2"></i> Seleccione la Sede <span class="text-red-600">*</span> 
      </label>
    <tbody>
    <div class="flex items-center gap-4">
...
<p class="text-gray-500 text-sm mt-4">Todos los campos marcados con <span class="text-red-600">*</span> son obligatorios.</p>
    ...
    <!-- Botón Regresar -->
    <button type="button" onclick="location.href='almacen.php';"
...
<!-- Ventana emergente para nueva almacen -->
<div id="ventanaEmergente" class="fixed inset-0 bg-black bg-opacity-50 hidden flex justify-center items-center">
  <div class="bg-white p-6 rounded-lg shadow-lg max-w-lg w-full">
    <h3 class="text-lg font-bold text-gray-700 mb-4">Registrar Nueva almacen</h3>
    <form id="formRegistraralmacen" action="registrar_almacen.php" method="POST">
      <div class="mb-4">
        <label for="nombre_almacen" class="block text-gray-700 font-bold mb-2">Nombre de la almacen</label>
        <input type="text" id="nombre_almacen" name="nombre_almacen" placeholder="Ingrese el nombre de la almacen"
               class="border border-gray-300 rounded p-2 w-full" required>
      </div>
      <div class="mb-4">
        <label for="direccion_almacen" class="block text-gray-700 font-bold mb-2">Dirección</label>
        <input type="text" id="direccion_almacen" name="direccion_almacen" placeholder="Ingrese la dirección"
...
<!-- Ventana emergente para nueva almacen -->
<div id="ventanaEmergente2" class="fixed inset-0 bg-black bg-opacity-50 hidden flex justify-center items-center">
  <div class="bg-white p-6 rounded-lg shadow-lg max-w-lg w-full">
    <h3 class="text-lg font-bold text-gray-700 mb-4">Registrar Nueva almacen</h3>
    <form id="formRegistraralmacen" action="registrar_almacen.php" method="POST">
      <div class="mb-4">
      ...
      <div class="flex justify-end gap-4">
        <button type="button" onclick="cerrarVentanaEmergente2()"
                class="bg-gray-500 text-white px-4 py-2 rounded hover:bg-gray-600">
          Cancelar
        </button>
        <button type="submit" class="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600">
...
<script>
    function validarNombre() {
        const nombre = document.getElementById('almacen').value;
        ...
        if (nombre.trim() !== '') {
            fetch(`validar_almacen.php?nombre=${encodeURIComponent(nombre)}`)
...
<script>
  ...
  function cerrarVentanaEmergente() {
   ...
   function abrirVentanaEmergente2() {
    document.getElementById('ventanaEmergente2').classList.remove('hidden');
  }

  // Función para cerrar la ventana emergente
  function cerrarVentanaEmergente2() {
    document.getElementById('ventanaEmergente2').classList.add('hidden');
  }

  // Función para alternar la visibilidad de la tabla de sedes
  function togglealmacenTable() {
    const select = document.getElementById('vincularalmacenes');
