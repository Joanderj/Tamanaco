...
$id_marca = $_GET['id_marca'] ?? null; // Obtener el ID de la marca desde la URL
if ($id_marca) {
    // Obtener el nombre de la marca
    $consulta_marca = $conexion->query("SELECT nombre_marca FROM marca WHERE id_marca = $id_marca");
    $marca = $consulta_marca->fetch_assoc();

    // Obtener los modelos asociados con su id_status
    $consulta_modelos = $conexion->query("
        SELECT m.id_modelo, m.nombre_modelo, COALESCE(mm.id_status, 0) AS id_status
        FROM modelo m
        LEFT JOIN marca_modelo mm ON m.id_modelo = mm.id_modelo AND mm.id_marca = $id_marca
    ");
}
?>
...
if ($id_marca) {
    ...
    $consulta_modelos = $conexion->prepare("
        SELECT m.id_modelo, m.nombre_modelo, COALESCE(mm.id_status, 0) AS id_status
        FROM modelo m
        LEFT JOIN marca_modelo mm ON m.id_modelo = mm.id_modelo AND mm.id_marca = ?
        ORDER BY m.nombre_modelo ASC
    ");
    $consulta_modelos->bind_param("i", $id_marca);
    $consulta_modelos->execute();
    $resultado = $consulta_modelos->get_result();
    ...
    $consulta_total = $conexion->prepare("
        ...
        LEFT JOIN marca_modelo mm ON m.id_modelo = mm.id_modelo AND mm.id_marca = ?
    ");
    $consulta_total->bind_param("i", $id_marca);
...
?>
 ...
 <div class="flex flex-col items-center mb-6">
    ...
    <p class="text-gray-600 mt-2 text-center">Registra las marcas y vincula los modelos asociados de forma rápida y organizada.</p>
</div>
    <?php
...
<?php endif; ?>
    ...
    <form action="modificar_marca.php" method="POST" class="space-y-4" id="form-marca">
        ...
        Nombre de la Marca: <span class="text-red-600">*</span>
        </label> 
        <input type="text" id="nombre_marca" name="nombre_marca" 
       value="<?php echo htmlspecialchars($marca['nombre_marca'] ?? ''); ?>" 
       ...
       oninput="convertirMayusculas(this); validarNombreMarca()">
            ...
            <small id="mensaje-error-input" class="text-red-500 hidden">Esta marca ya existe</small>
            <small id="mensaje-exito-input" class="text-green-500 hidden">¡Esta marca está disponible!</small>
            <small class="text-gray-500">Ejemplo: TAMANACO</small>
        </div>
        <button type="button" onclick="abrirVentanaEmergenteModelo()"
...
<div id="vincular_varios">
    ...
    <input type="text" id="searchBox" 
        ...
        placeholder="Buscar modelo..." onkeyup="filterTable()">
...
while ($fila = $resultado->fetch_assoc()) {
    echo "<tr class='model-row'>
            <td class='px-4 py-2 border text-center'>
                <input type='checkbox' name='modelos[" . $fila['id_modelo'] . "]' value='1' 
                " . ($fila['id_status'] == 1 ? 'checked' : '') . ">
            </td>
            <td class='px-4 py-2 border'>" . htmlspecialchars($fila['nombre_modelo'], ENT_QUOTES, 'UTF-8') . "</td>
          </tr>";
}
?>
            </tbody>
...
<p class="text-gray-500 text-sm mt-4">Todos los campos marcados con <span class="text-red-600">*</span> son obligatorios.</p>
        ...
        <div class="flex justify-between mt-4 space-x-4">
            <input type="text" name="id_marca" id="id_marca" value="<?php echo htmlspecialchars($id_marca); ?>" class="hidden">
    <!-- Botón Guardar -->
    ...
    <!-- Botón Regresar -->
    <button type="button" onclick="location.href='marca.php';"
...
<script>
    ...
    function validarNombreMarca() {
        ...
        const idMarca = document.getElementById('id_marca').value; // Assuming there's a hidden input with the current brand ID
        ...
        if (nombreMarca.trim() !== '') {
            fetch(`validar_marca_modificar.php?nombre=${encodeURIComponent(nombreMarca)}&id=${encodeURIComponent(idMarca)}`)
                .then(response => response.json())
                .then(data => {
                    if (data.existe && data.id !== parseInt(idMarca)) {
...
<div id="ventanaEmergenteModelo" class="fixed inset-0 bg-black bg-opacity-50 hidden flex justify-center items-center">
    ...
    <form id="formRegistrarModelo" method="POST" action="guardar_modelo.php">
            <div class="mb-4">
