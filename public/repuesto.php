...
$nombreRepuesto = isset($_GET['nombreRepuesto']) ? trim($_GET['nombreRepuesto']) : '';
...
// Clasificación
$orderBy = 'r.id_repuesto'; // Orden por defecto
if (isset($_GET['sort'])) {
    switch ($_GET['sort']) {
        case 'fecha_asc':
        ...
        case 'fecha_desc':
            ...
            break;
        case 'nombre_asc':
            $orderBy = 'r.nombre_repuesto ASC';
            break;
        case 'nombre_desc':
            $orderBy = 'r.nombre_repuesto DESC';
            break;
        case 'numero_asc':
            $orderBy = 'r.id_repuesto ASC';
            break;
        case 'numero_desc':
            $orderBy = 'r.id_repuesto DESC';
            break;
    }
}

// Consulta total de elementos
$totalQuery = "SELECT COUNT(*) FROM repuesto r WHERE 1=1";
$params = [];

if (!empty($nombreRepuesto)) {
    $totalQuery .= " AND r.nombre_repuesto LIKE ?";
    $params[] = '%' . $nombreRepuesto . '%';
}

if (!empty($idMarca)) {
    $totalQuery .= " AND r.id_marca = ?";
    $params[] = $idMarca;
}

if (!empty($idModelo)) {
    $totalQuery .= " AND r.id_modelo = ?";
    $params[] = $idModelo;
}

if (!empty($status)) {
    $totalQuery .= " AND r.id_status = ?";
    $params[] = $status;
}

if (!empty($fechaInicio)) {
    $totalQuery .= " AND r.date_created >= ?";
    $params[] = $fechaInicio;
}
...
if (!empty($nombreRepuesto)) {
    $query .= " AND r.nombre_repuesto LIKE ?";
}

if (!empty($idMarca)) {
    $query .= " AND r.id_marca = ?";
}

if (!empty($idModelo)) {
    $query .= " AND r.id_modelo = ?";
}

if (!empty($status)) {
    $query .= " AND r.id_status = ?";
}

if (!empty($fechaInicio)) {
    $query .= " AND r.date_created >= ?";
}
...
<!-- Modal para mostrar información del Repuesto -->
<div id="modalRepuesto" class="fixed inset-0 bg-gray-900 bg-opacity-75 hidden justify-center items-center z-50">
    <div class="bg-white rounded-xl shadow-lg w-full max-w-lg overflow-hidden">
        ...
        <div class="px-6 py-4 flex justify-between items-center bg-gradient-to-r from-indigo-500 via-purple-500 to-indigo-500 text-white">
            <h2 class="text-xl font-semibold flex items-center">
                <i class="fas fa-cogs mr-2"></i> Detalles del Repuesto
            </h2>
        ...
        <div class="px-6 py-4">
            ...
            <div class="flex items-center space-x-4">
                ...
                <p class="text-lg font-medium text-gray-800">
                    <strong>Nombre del Repuesto:</strong> <span id="repuestoNombre"></span>
                </p>
            </div>
            <!-- Fecha de creación -->
            <div class="flex items-center space-x-4 mt-4">
                ...
                <p class="text-lg font-medium text-gray-800">
                    <strong>Fecha de Creación:</strong> <span id="repuestoFecha"></span>
                </p>
            </div>

            <!-- Clasificaciones relacionadas -->
            <p class="mt-4 text-lg font-semibold text-gray-700">Clasificaciones Relacionadas:</p>
            <ul id="clasificacionesRelacionadas" class="grid grid-cols-2 gap-4 mt-4">
        ...
        <div class="px-6 py-4 bg-gray-100 flex flex-col items-center space-y-4 rounded-lg shadow-sm">
            <div class="flex flex-row gap-3 justify-center items-center">
                <!-- Animación con diferentes colores y retrasos -->
...
<div class="flex justify-between items-center mb-4">
    ...
    <div class="space-x-2">
        <?php if (in_array(2, $permisos)): ?>
            <button 
                onclick="window.location.href='formulario_guardar_repuesto.php'" 
        ...
        <?php if (in_array(4, $permisos)): ?>
            <button 
                onclick="window.location.href='imprimir_repuesto.php'" 
...
<div id="filterForm" class="hidden mt-2 bg-white border border-gray-300 rounded shadow-lg p-4">
   ...
   <div id="savedFilters" class="mt-4">
    ...
    <div>
        <label for="nombreRepuesto" class="block text-gray-700 mb-2">Nombre del Repuesto</label>
        <input 
            type="text" 
            id="nombreRepuesto" 
    ...
    <!-- Campo: Marca y Modelo Juntos -->
    <div class="grid grid-cols-2 gap-4">
        <!-- Marca -->
        <div>
            <label for="idMarca" class="block text-gray-700 mb-2">Marca</label>
            <select 
                id="idMarca" 
                class="w-full border border-gray-300 rounded p-2 focus:outline-none focus:ring-2 focus:ring-indigo-500"
            >
                ...
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        $idMarca = htmlspecialchars($row["id_marca"]);
                        $nombreMarca = htmlspecialchars($row["nombre_marca"]);
        ...
        <!-- Modelo -->
        <div>
            <label for="idModelo" class="block text-gray-700 mb-2">Modelo</label>
            <select 
                id="idModelo" 
                class="w-full border border-gray-300 rounded p-2 focus:outline-none focus:ring-2 focus:ring-indigo-500"
            >
                ...
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        $idModelo = htmlspecialchars($row["id_modelo"]);
                        $nombreModelo = htmlspecialchars($row["nombre_modelo"]);
...
<div class="border border-gray-300 rounded-lg shadow-md p-4">
    ...
    <div class="flex justify-between items-center mb-4 border-b border-gray-300 pb-2">
        <h1 class="text-xl font-semibold text-gray-700">Repuestos</h1>
        <div class="relative">
    ...
    <tbody id="dataTable" class="bg-white divide-y divide-gray-200">
        ...
        <tr class="hover:bg-gray-100">
            <!-- Imagen -->
            <td class="px-6 py-4">
                <?php if (!empty($repuesto['url'])): ?>
                    <img src="<?php echo htmlspecialchars($repuesto['url']); ?>" alt="<?php echo htmlspecialchars($repuesto['nombre_imagen']); ?>" class="w-32 h-32 object-cover rounded-md">
                <?php else: ?>
                    <span>Sin Imagen</span>
                <?php endif; ?>
            </td>
    ...
    <li class="flex items-center space-x-2">
        ...
        <strong>ID:</strong> <?php echo htmlspecialchars($repuesto['id_repuesto']); ?>
    </li>
    <li class="flex items-center space-x-2">
        <i class="fas fa-tags text-green-500"></i>
        <strong>Nombre:</strong> <?php echo htmlspecialchars($repuesto['nombre_repuesto']); ?>
    </li>
    <li class="flex items-center space-x-2">
        <i class="fas fa-industry text-blue-500"></i>
        <strong>Marca:</strong> <?php echo htmlspecialchars($repuesto['nombre_marca']); ?>
    </li>
    <li class="flex items-center space-x-2">
        <i class="fas fa-cube text-yellow-500"></i>
        <strong>Modelo:</strong> <?php echo htmlspecialchars($repuesto['nombre_modelo']); ?>
    </li>
    <li class="flex items-center space-x-2">
        <i class="fas fa-check-circle text-teal-500"></i>
        <strong>Estado:</strong> <?php echo htmlspecialchars($repuesto['nombre_status']); ?>
    </li>
    <li class="flex items-center space-x-2">
        <i class="fas fa-calendar-alt text-purple-500"></i>
        <strong>Fecha:</strong> <?php echo htmlspecialchars($repuesto['date_created']); ?>
    </li>
</ul>
            </td>
            <!-- Acciones -->
...
$idPerfil = $_SESSION['id_perfil'];
$idSubmenu = 17; // ID del submenú correspondiente
$permisos = obtenerPermisos($idPerfil, $idSubmenu);
?>

<td class="px-6 py-4 flex space-x-2">
    <?php if ($repuesto['nombre_status'] == 'Activo'): ?>
        
        <?php if (in_array(1, $permisos)): ?>
        <button 
            class="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600"
            onclick="showModal(<?php echo $repuesto['id_repuesto']; ?>)">
        ...
        <button 
            ...
            onclick="window.location.href='formulario_modificar_repuesto.php?id_repuesto=<?php echo $repuesto['id_repuesto']; ?>'">
        ...
        <button 
            class="bg-green-500 text-white px-4 py-2 rounded hover:bg-green-600" 
            onclick="window.location.href='imprimir_repuesto_individual.php?id_repuesto=<?php echo $repuesto['id_repuesto']; ?>'">
            <i class="fas fa-edit"></i> Imprimir
        </button>
        <?php endif; ?>
     <?php endif; ?>
</td>
...
function applyFilters() {
    ...
    // Redirigir a la misma página con los filtros aplicados como parámetros
    window.location.href = `?nombreRepuesto=${encodeURIComponent(nombreRepuesto)}&idMarca=${idMarca}&idModelo=${idModelo}&fechaInicio=${fechaInicio}&fechaFinal=${fechaFinal}`;
}
...
if (searchInput) {
    ...
    // Capturar el evento de presionar una tecla en el campo de búsqueda
    searchInput.addEventListener('keypress', function(event) {
        if (event.key === 'Enter') {
            const searchValue = this.value;
            // Redirigir a la página con el parámetro de búsqueda
            window.location.href = window.location.pathname + `?nombreRepuesto=${encodeURIComponent(searchValue)}`;
        }
    });
}
</script>
...
<script>
    // Función para mostrar el modal con los detalles del repuesto y sus clasificaciones
    function showModal(idRepuesto) {
        // Realizar la solicitud para obtener datos del repuesto y sus clasificaciones
        fetch(`getRepuestoDetalles.php?id_repuesto=${encodeURIComponent(idRepuesto)}`) // Codificar el parámetro para evitar errores
            .then(response => {
                if (!response.ok) {
                    throw new Error(`Error HTTP ${response.status}`); // Manejo de errores HTTP
                }
                return response.json();
            })
            .then(data => {
                // Validar que los datos existan antes de usarlos
                if (!data || !data.nombreRepuesto || !data.fechaCreacion || !data.clasificaciones) {
                    throw new Error("Datos incompletos recibidos del servidor");
                }

                // Actualizar el contenido del modal con los datos recibidos
                document.getElementById('repuestoNombre').textContent = data.nombreRepuesto;
                document.getElementById('repuestoFecha').textContent = data.fechaCreacion;

                // Contenedor para las clasificaciones relacionadas del repuesto
                const clasificacionesRelacionadas = document.getElementById('clasificacionesRelacionadas');
                clasificacionesRelacionadas.innerHTML = ''; // Limpiar contenido previo

                // Generar lista de clasificaciones relacionadas
                data.clasificaciones.forEach((clasificacion, index) => {
                    const liClasificacion = document.createElement('li');
                    liClasificacion.className = "flex items-center space-x-2 bg-gray-100 p-2 rounded shadow-md";

                    const numeroClasificacion = document.createElement('span');
                    numeroClasificacion.className = "text-indigo-500 font-bold";
                    numeroClasificacion.textContent = `${index + 1}.`;

                    const iconoClasificacion = document.createElement('i');
                    iconoClasificacion.className = "fas fa-tag text-indigo-500";

                    const detalleClasificacion = document.createElement('span');
                    detalleClasificacion.className = "text-gray-700 font-medium";
                    detalleClasificacion.textContent = `${clasificacion.detalle}: ${clasificacion.valor}`;

                    liClasificacion.appendChild(numeroClasificacion);
                    liClasificacion.appendChild(iconoClasificacion);
                    liClasificacion.appendChild(detalleClasificacion);

                    clasificacionesRelacionadas.appendChild(liClasificacion);
                });

                // Mostrar el modal
                const modal = document.getElementById('modalRepuesto');
                modal.classList.remove('hidden'); // Mostrar el modal
                modal.classList.add('flex'); // Activar diseño flex
            })
            .catch(error => {
                console.error('Error al obtener los datos:', error); // Manejo de errores
                alert(`Ocurrió un error al cargar los datos: ${error.message}`); // Notificar al usuario sobre el error
            });
    }
    ...
    function closeModal() {
        ...
        modal.classList.remove('flex'); // Remover diseño flex
    }
</script>