...
<div class="flex flex-col items-center mb-6">
    <div class="bg-purple-500 text-white w-16 h-16 rounded-full flex items-center justify-center shadow-lg mb-4">
        <i class="fas fa-envelope text-3xl"></i>
    </div>
    <h2 class="text-3xl font-extrabold text-gray-800">Correo</h2>
    <p class="text-gray-600 mt-2 text-center">Gestiona tu dirección de correo electrónico.</p>
</div>
...
<?php endif; ?>
  ...
  <!-- Formulario -->
<form action="editar_correo.php" method="POST" onsubmit="return validarCorreo();">
  <!-- Dirección de Correo -->
  <div class="relative mb-6">
    <label for="correo" class="block font-medium text-gray-600">Correo Electrónico:</label>
    <input type="email" id="correo" name="email" value='<?php echo $perfil['correo_electronico'];?>' placeholder="Ejemplo: usuario@empresa.com" oninput="convertirMayusculas(this);" class="w-full p-3 border border-gray-300 rounded-lg bg-gray-100 focus:outline-none" required disabled />
    <button type="button" class="absolute top-2 right-2 text-gray-500 hover:text-blue-500" onclick="habilitarInput('correo')">
        <i class="fas fa-pencil-alt"></i>
      </button>
    <span id="errorCorreo" class="text-red-500 text-sm hidden">Por favor, introduce un correo electrónico válido.</span>
  </div>
...
function validarCorreo() {
  ...
  const errorMensaje = document.getElementById('errorCorreo');
  const regexCorreo = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
  const dominiosPermitidos = ["GMAIL.COM", "HOTMAIL.COM", "YAHOO.COM", "OUTLOOK.COM"];

  // Limpiar el mensaje de error al inicio
  errorMensaje.classList.add('hidden');
  errorMensaje.textContent = "";

  // Validar si el campo está vacío
  if (correoInput.value.trim() === '') {
    errorMensaje.textContent = "El campo de correo electrónico no puede estar vacío.";
  ...
  if (!regexCorreo.test(correoInput.value)) {
    errorMensaje.textContent = "Por favor, introduce un correo electrónico válido (soport_tamanaco@gmail.com).";
    errorMensaje.classList.remove('hidden');
    return false;
  }

  // Validar que el correo tenga una arroba (@) y un dominio permitido
  const dominio = correoInput.value.split('@')[1]; // Extraer el dominio
  if (!dominiosPermitidos.includes(dominio)) {
    errorMensaje.textContent = `El dominio "${dominio}" no es válido. Usa Gmail, Hotmail, Yahoo, Outlook, etc.`;
    errorMensaje.classList.remove('hidden');
    return false;
  }
