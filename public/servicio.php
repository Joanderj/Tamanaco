...
if (isset($_GET['sort'])) {
    switch ($_GET['sort']) {
        case 'fecha_asc': $orderBy = 'servicio.date_created ASC'; break;
        case 'fecha_desc': $orderBy = 'servicio.date_created DESC'; break;
        case 'nombre_asc': $orderBy = 'servicio.nombre_servicio ASC'; break;
        case 'nombre_desc': $orderBy = 'servicio.nombre_servicio DESC'; break;
    }
}
...
if (!empty($nombreServicio)) {
    $whereClause .= " AND servicio.nombre_servicio LIKE ?";
    $params[] = '%' . $nombreServicio . '%';
    $types .= 's';
}
if (!empty($status)) {
    $whereClause .= " AND servicio.id_status = ?";
    $params[] = $status;
    $types .= 'i';
}
if (!empty($fechaInicio)) {
    $whereClause .= " AND servicio.date_created >= ?";
    $params[] = $fechaInicio;
    $types .= 's';
}
if (!empty($fechaFinal)) {
...
// --- Total de servicios ---
$totalQuery = "SELECT COUNT(*) FROM servicio $whereClause";
...
// --- Consulta principal de servicios con paginación ---
$query = "
    SELECT servicio.id_servicio, servicio.nombre_servicio, servicio.descripcion,
           servicio.date_created, servicio.id_status
    FROM servicio
    $whereClause
    ORDER BY $orderBy
    LIMIT ?, ?
...
// Para el OFFSET y LIMIT, usamos enteros 'i'
$paramsWithLimit = $params;
$typesWithLimit = $types . "ii"; // agregar dos enteros para offset y limit
...
<div id="modalMarca" class="fixed inset-0 bg-gray-900 bg-opacity-75 hidden justify-center items-center z-50">
    <div class="bg-white rounded-xl shadow-lg w-full max-w-lg overflow-hidden">
        ...
        <div class="px-6 py-4 flex justify-between items-center bg-gradient-to-r from-indigo-500 via-purple-500 to-indigo-500 text-white">
            <h2 class="text-xl font-semibold flex items-center">
                <i class="fas fa-tags mr-2"></i> Detalles de la Marca
            </h2>
        ...
        <div class="px-6 py-4">
            <div class="flex items-center space-x-4">
                <i class="fas fa-tag text-indigo-500 text-2xl"></i>
                <p class="text-lg font-medium text-gray-800">
                    <strong>Nombre de la Marca:</strong> <span id="marcaNombre"></span>
                </p>
            </div>
            <p class="mt-4 text-lg font-semibold text-gray-700">
                Modelos Relacionados:
            </p>
            <ul id="modelosRelacionados" class="grid grid-cols-2 gap-2 mt-2">
                <!-- Modelos dinámicos -->
            </ul>
        </div>
        <!-- Pie del modal -->
...
<div class="flex justify-between items-center mb-4">
    ...
    <div class="space-x-2">
        <?php if (in_array(2, $permisos)): ?>
            <button 
                onclick="window.location.href='formulario_guardar_servicio.php'" 
        ...
        <?php if (in_array(4, $permisos)): ?>
            <button 
                onclick="window.location.href='imprimir_servicio.php'" 
...
<div id="filterForm" class="hidden mt-2 bg-white border border-gray-300 rounded shadow-lg p-4">
   ...
   <div id="savedFilters" class="mt-4">
    ...
    <form id="filterOptionsForm" method="get" action="servicio.php" class="space-y-6">
    <!-- Campo: Nombre del Servicio -->
    <div class="mb-4">
        <label for="nombreServicio" class="block text-gray-700 font-semibold mb-2">Nombre del Servicio</label>
        <input 
            type="text" 
            id="nombreServicio" 
            name="nombreServicio"
    ...
    <!-- Campo: Estado y Proveedor Juntos -->
    <div class="grid grid-cols-2 gap-4 mb-4">
        ...
        <!-- Proveedor -->
        <div>
            <label for="idProveedor" class="block text-gray-700 font-semibold mb-2">Proveedor</label>
            <select 
                id="idProveedor" 
                name="idProveedor"
                class="w-full border border-gray-300 rounded p-2 focus:outline-none focus:ring-2 focus:ring-indigo-500"
            >
                ...
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        echo "<option value='" . htmlspecialchars($row['id_proveedor']) . "'>" . htmlspecialchars($row['nombre_proveedor']) . "</option>";
                    }
                } else {
    ...
    <div class="grid grid-cols-2 gap-4 mb-4">
        <!-- Fecha Inicio -->
        <div>
            <label for="fechaInicio" class="block text-gray-700 font-semibold mb-2">Fecha Inicio</label>
            <input 
                type="date" 
                id="fechaInicio" 
                name="fechaInicio"
        ...
        <!-- Fecha Final -->
        <div>
            <label for="fechaFinal" class="block text-gray-700 font-semibold mb-2">Fecha Final</label>
            <input 
                type="date" 
                id="fechaFinal" 
                name="fechaFinal"
    ...
    <div class="flex justify-between mt-4">
        <div class="flex space-x-2">
            ...
            <button 
                type="reset" 
                class="border border-red-500 text-red-500 px-4 py-2 rounded hover:bg-red-50 focus:outline-none"
            >
                Borrar Filtro
            </button>
        </div>
        ...
        <button 
            type="button" 
            class="border border-blue-500 text-blue-500 px-4 py-2 rounded hover:bg-blue-50 focus:outline-none"
...
<div class="border border-gray-300 rounded-lg shadow-md p-4">
    ...
    <div class="flex justify-between items-center mb-4 border-b border-gray-300 pb-2">
        <h1 class="text-xl font-semibold text-gray-700">Servicios</h1>
        <div class="relative">
    ...
    <tbody id="dataTable" class="bg-white divide-y divide-gray-200">
        ...
        <tr class="hover:bg-gray-100">
            <!-- ID del Servicio -->
            <td class="px-6 py-4 text-gray-800 font-medium"><?php echo htmlspecialchars($servicio['id_servicio']); ?></td>
            <!-- Nombre del Servicio -->
            <td class="px-6 py-4 text-gray-800"><?php echo htmlspecialchars($servicio['nombre_servicio']); ?></td>
            <!-- Botones de Acción -->
          <?php
include_once 'includes/permisos.php';

$idPerfil = $_SESSION['id_perfil'];
$idSubmenu = 19; // ID del submenú correspondiente
$permisos = obtenerPermisos($idPerfil, $idSubmenu);
?>

<td class="px-6 py-4 flex space-x-2">
    <?php if ($servicio['id_status'] == 1): ?>
        
        <?php if (in_array(1, $permisos)): ?>
        <button 
            class="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600"
            onclick="showModal(<?php echo $servicio['id_servicio']; ?>)">
        ...
        <button 
            ...
            onclick="window.location.href='formulario_modificar_servicio.php?id_servicio=<?php echo $servicio['id_servicio']; ?>'">
  ...
  <script>
    ...
    function applyFilters() {
        const nombreServicio = document.getElementById('nombreServicio').value; // Nombre del servicio
        const status = document.getElementById('status').value; // Estado
        const idProveedor = document.getElementById('idProveedor').value; // Proveedor seleccionado
        const fechaInicio = document.getElementById('fechaInicio').value; // Fecha inicio
        const fechaFinal = document.getElementById('fechaFinal').value; // Fecha final

        // Validar que se seleccionó al menos un filtro
        if (!nombreServicio && !status && !idProveedor && !fechaInicio && !fechaFinal) {
            alert("Por favor, selecciona al menos un filtro.");
            return;
        }

        // Redirigir a la misma página con los filtros aplicados como parámetros GET
        const queryString = `?nombreServicio=${encodeURIComponent(nombreServicio)}&status=${status}&idProveedor=${idProveedor}&fechaInicio=${fechaInicio}&fechaFinal=${fechaFinal}`;
        window.location.href = window.location.pathname + queryString;
    }

    // Función para filtrar la tabla en tiempo real
    function filterTable() {
        const input = document.getElementById('searchInput'); // Campo de búsqueda
        const filter = input.value.toLowerCase(); // Texto introducido por el usuario
        const table = document.getElementById('dataTable'); // Tabla de servicios
        const rows = table.getElementsByTagName('tr'); // Filas de la tabla

        for (let i = 0; i < rows.length; i++) {
            const cells = rows[i].getElementsByTagName('td'); // Columnas de cada fila
            let rowContainsFilter = false;

            for (let j = 0; j < cells.length; j++) {
                if (cells[j]) {
                    const cellValue = cells[j].textContent || cells[j].innerText; // Valor de la celda
                    if (cellValue.toLowerCase().indexOf(filter) > -1) { // Compara con el filtro
    ...
    // Captura el evento de presionar la tecla "Enter" para aplicar el filtro
    document.getElementById('searchInput').addEventListener('keypress', function(event) {
        if (event.key === 'Enter') {
            const searchValue = this.value; // Valor del campo de búsqueda
            // Redirigir con el parámetro de búsqueda aplicado
            const queryString = `?nombreServicio=${encodeURIComponent(searchValue)}`;
            window.location.href = window.location.pathname + queryString;
        }
    });
</script>
...
<script>
    function showModal(idMarca) {
        fetch(`getMarcaModelos.php?id_marca=${idMarca}`)
            .then(response => response.json())
            .then(data => {
                document.getElementById('marcaNombre').textContent = data.nombreMarca;

                const modelosRelacionados = document.getElementById('modelosRelacionados');
                modelosRelacionados.innerHTML = ''; // Limpiar contenido previo
                ...
                data.modelos.forEach((modelo, index) => {
                    ...
                    liModelo.className = "flex items-center space-x-2 bg-gray-100 p-2 rounded shadow-md";

                    // Agregar número del modelo
                    const numeroModelo = document.createElement('span');
                    numeroModelo.className = "text-indigo-500 font-bold";
                    numeroModelo.textContent = `${index + 1}.`;

                    // Agregar ícono decorativo
                    const iconoModelo = document.createElement('i');
                    iconoModelo.className = "fas fa-cube text-indigo-500";

                    // Agregar nombre del modelo
                    const nombreModelo = document.createElement('span');
                    nombreModelo.className = "text-gray-700 font-medium";
                    nombreModelo.textContent = modelo;

                    // Agregar los elementos al <li>
                    liModelo.appendChild(numeroModelo);
                    liModelo.appendChild(iconoModelo);
                    liModelo.appendChild(nombreModelo);

                    // Agregar el <li> al <ul> de modelos relacionados
                    modelosRelacionados.appendChild(liModelo);
                });
