...
// Submenú actual: MARCA (id_submenu = 11
$submenu_actual = 11;
...
<div class="flex justify-between items-center mb-4">
    ...
    <div class="space-x-2">
        <?php if (in_array(2, $permisos)): ?>
            <button 
                onclick="window.location.href='formulario_guardar_marca.php'" 
        ...
        <?php if (in_array(4, $permisos)): ?>
            <button 
                onclick="window.location.href='imprimir_marcas.php'" 
...
<div id="filterForm" class="hidden mt-2 bg-white border border-gray-300 rounded shadow-lg p-4">
   ...
   <div id="savedFilters" class="mt-4">
    ...
    <div class="mb-4">
        <label for="nombreMarca" class="block text-gray-700 mb-2">Nombre de la Marca</label>
        <input 
          type="text" 
          id="nombreMarca" 
          name="nombreMarca"
    ...
    <!-- Campo: Modelos -->
    <div class="mb-4">
        <label for="modelo" class="block text-gray-700 mb-2">Modelos</label>
        <select 
            id="modelo" 
            name="modelo"
            class="w-full border border-gray-300 rounded p-2 focus:outline-none focus:ring-2 focus:ring-indigo-500"
        
        > <!-- Aplica automáticamente el filtro al cambiar el modelo -->
...
<div class="border border-gray-300 rounded-lg shadow-md p-4">
    ...
    <div class="flex justify-between items-center mb-4 border-b border-gray-300 pb-2">
        <h1 class="text-xl font-semibold text-gray-700">Marcas</h1>
        <div class="relative">
    ...
    <tbody id="dataTable" class="bg-white divide-y divide-gray-200">
        ...
        <tr class="hover:bg-gray-100">
            <td class="px-6 py-4"><?php echo $marca['id_marca']; ?></td>
            <td class="px-6 py-4"><?php echo $marca['nombre_marca']; ?></td>
            <?php
include_once 'includes/permisos.php';

$idPerfil = $_SESSION['id_perfil'];
$idSubmenu = 11; // ID del submenú correspondiente
$permisos = obtenerPermisos($idPerfil, $idSubmenu);
?>

<td class="px-6 py-4 flex space-x-2">
    <?php if ($marca['id_status'] == 1): ?>
        
        <?php if (in_array(1, $permisos)): ?>
        <button 
            class="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600"
            onclick="showModal(<?php echo $marca['id_marca']; ?>)">
        ...
        <button 
            ...
            onclick="window.location.href='formulario_modificar_marca.php?id_marca=<?php echo $marca['id_marca']; ?>'">
  ...
  <script>
    ...
    function applyFilters() {
        ...
        const modelo = document.getElementById('modelo').value; // Modelo seleccionado
        const fechaInicio = document.getElementById('fechaInicio').value;
        const fechaFinal = document.getElementById('fechaFinal').value;

        // Validar que se seleccionó un modelo cuando se intenta filtrar por él
        if (!modelo && nombreMarca === "" && status === "" && fechaInicio === "" && fechaFinal === "") {
            alert("Por favor, selecciona al menos un filtro.");
            return;
        }

        // Redirigir a la misma página con los filtros aplicados
        window.location.href = `?nombreMarca=${encodeURIComponent(nombreMarca)}&status=${status}&modelo=${modelo}&fechaInicio=${fechaInicio}&fechaFinal=${fechaFinal}`;
    }
...
<script>
    ...
    document.getElementById('searchInput').addEventListener('keypress', function(event) {
        ...
        if (event.key === 'Enter') {
            ...
            window.location.href = window.location.pathname + '?nombreMarca=' + encodeURIComponent(searchValue);
        }
    });
</script>
