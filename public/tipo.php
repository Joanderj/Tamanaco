...
// Submenú actual: Sede (id_submenu = 13)
$submenu_actual = 13;
...
$nombreTipo = isset($_GET['nombreTipo']) ? $_GET['nombreTipo'] : ''; // Filtro por nombre del tipo
$clasificacionSeleccionada = isset($_GET['clasificacion']) ? $_GET['clasificacion'] : ''; // Clasificación filtrada
$status = isset($_GET['status']) ? $_GET['status'] : ''; // Estado activo/inactivo
$fechaInicio = isset($_GET['fechaInicio']) ? $_GET['fechaInicio'] : ''; // Fecha inicial
$fechaFinal = isset($_GET['fechaFinal']) ? $_GET['fechaFinal'] : ''; // Fecha final
...
$totalQuery = "
    SELECT COUNT(DISTINCT t.id_tipo) 
    FROM tipo t
...
if (!empty($clasificacionSeleccionada)) {
    $totalQuery .= " AND tc.id_clasificacion = ?";
    $params[] = $clasificacionSeleccionada;
}
if (!empty($nombreTipo)) {
    $totalQuery .= " AND t.nombre_tipo LIKE ?";
    $params[] = '%' . $nombreTipo . '%';
}
if (!empty($status)) {
    $totalQuery .= " AND t.id_status = ?";
    $params[] = $status;
}
if (!empty($fechaInicio)) {
    $totalQuery .= " AND t.fecha_creacion >= ?";
    $params[] = $fechaInicio;
}
if (!empty($fechaFinal)) {
    $totalQuery .= " AND t.fecha_creacion <= ?";
...
// Consulta principal de datos (tipos únicos con paginación)
$query = "
    SELECT DISTINCT t.id_tipo, t.nombre_tipo, t.fecha_creacion, t.id_status
    FROM tipo t
    INNER JOIN tipo_clasificacion tc ON t.id_tipo = tc.id_tipo
    WHERE 1=1
";
...
// Agregar filtros dinámicamente
if (!empty($clasificacionSeleccionada)) {
    $query .= " AND tc.id_clasificacion = ?";
}
if (!empty($nombreTipo)) {
    $query .= " AND t.nombre_tipo LIKE ?";
}
if (!empty($status)) {
...
<div id="modalTipo" class="fixed inset-0 bg-gray-900 bg-opacity-75 hidden justify-center items-center z-50">
    <div class="bg-white rounded-xl shadow-lg w-full max-w-lg overflow-hidden">
        ...
        <div class="px-6 py-4 flex justify-between items-center bg-gradient-to-r from-indigo-500 via-purple-500 to-indigo-500 text-white">
            <h2 class="text-xl font-semibold flex items-center">
                <i class="fas fa-layer-group mr-2"></i> Detalles del Tipo
            </h2>
        ...
        <div class="px-6 py-4">
            <div class="flex items-center space-x-4">
                ...
                <p class="text-lg font-medium text-gray-800">
                    <strong>Nombre del Tipo:</strong> <span id="tipoNombre"></span>
                </p>
            </div>
            <div class="flex items-center space-x-4 mt-4">
                <i class="fas fa-calendar-alt text-indigo-500 text-2xl"></i>
                <p class="text-lg font-medium text-gray-800">
                    <strong>Fecha de Creación:</strong> <span id="tipoFecha"></span>
                </p>
            </div>
...
<div class="flex justify-between items-center mb-4">
    ...
    <div class="space-x-2">
        <?php if (in_array(2, $permisos)): ?>
            <button 
                onclick="window.location.href='formulario_guardar_tipo.php'" 
        ...
        <?php if (in_array(4, $permisos)): ?>
            <button 
                onclick="window.location.href='imprimir_tipos.php'" 
...
<div id="filterForm" class="hidden mt-2 bg-white border border-gray-300 rounded shadow-lg p-4">
   ...
   <div id="savedFilters" class="mt-4">
    ...
    <div class="mb-4">
        <label for="nombreTipo" class="block text-gray-700 mb-2">Nombre del Tipo</label>
        <input 
          type="text" 
          id="nombreTipo" 
          name="nombreTipo"
    ...
    <!-- Campo: Status y Clasificaciones Juntos -->
    <div class="grid grid-cols-2 gap-4 mb-4">
        ...
        <!-- Clasificaciones -->
        <div>
            <label for="clasificacion" class="block text-gray-700 mb-2">Clasificación</label>
            <select 
                id="clasificacion" 
                name="clasificacion"
                class="w-full border border-gray-300 rounded p-2 focus:outline-none focus:ring-2 focus:ring-indigo-500"
            >
                ...
                // Consultar todas las clasificaciones
...
<style>
    /* Animación para el mensaje: Fade In y Slide Up */
@keyframes fadeInUp {
    from {
...
/* Estilo del fondo y el contenedor */
#mensajeError {
...
<div id="mensajeError" class="fixed inset-0 flex items-center justify-center z-50 hidden">
    <div class="bg-white bg-opacity-90 border-l-4 border-red-500 text-red-700 rounded-lg shadow-xl p-6 w-96 animate-fadeInUp">
        <div class="flex items-start space-x-4">
            <i class="fas fa-times-circle text-4xl text-red-500"></i>
            <div>
                <p class="text-lg font-semibold">¡Atención!</p>
                <p class="text-gray-700">Por favor, selecciona al menos un filtro antes de continuar.</p>
            </div>
        </div>
        <button onclick="closeErrorMessage()" class="mt-4 w-full bg-red-500 text-white font-bold px-4 py-2 rounded-lg hover:bg-red-600">
            Entendido
        </button>
    </div>
</div>
...
<div class="border border-gray-300 rounded-lg shadow-md p-4">
    ...
    <div class="flex justify-between items-center mb-4 border-b border-gray-300 pb-2">
        <h1 class="text-xl font-semibold text-gray-700">Tipos</h1>
        <div class="relative">
    ...
    <tbody class="bg-white divide-y divide-gray-200">
        ...
        <tr class="hover:bg-gray-100">
            <td class="px-6 py-4"><?php echo htmlspecialchars($tipo['id_tipo']); ?></td>
            <td class="px-6 py-4"><?php echo htmlspecialchars($tipo['nombre_tipo']); ?></td>
            <td class="px-6 py-4"><?php echo htmlspecialchars($tipo['fecha_creacion']); ?></td>
            <?php
            include_once 'includes/permisos.php';

            $idPerfil = $_SESSION['id_perfil'];
            $idSubmenu = 13; // ID del submenú correspondiente
            $permisos = obtenerPermisos($idPerfil, $idSubmenu);
            ?>

            <td class="px-6 py-4 flex space-x-2">
                <?php if ($tipo['id_status'] == 1): ?>
                    
                    <?php if (in_array(1, $permisos)): ?>
                    <button 
                        class="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600"
                        onclick="showModal(<?php echo $tipo['id_tipo']; ?>)">
                    ...
                    <button 
                        ...
                        onclick="window.location.href='formulario_modificar_tipo.php?id_tipo=<?php echo $tipo['id_tipo']; ?>'">
    ...
    function applyFilters() {
        const nombreTipo = document.getElementById('nombreTipo').value; // Filtro por nombre del tipo
        const clasificacion = document.getElementById('clasificacion').value; // Filtro por clasificación
        const status = document.getElementById('status').value; // Filtro por estado
        const fechaInicio = document.getElementById('fechaInicio').value;
        const fechaFinal = document.getElementById('fechaFinal').value;

        // Mostrar el mensaje si no hay filtros seleccionados
        if (!nombreTipo && !clasificacion && !status && !fechaInicio && !fechaFinal) {
        ...
        window.location.href = `?nombreTipo=${encodeURIComponent(nombreTipo)}&clasificacion=${clasificacion}&status=${status}&fechaInicio=${fechaInicio}&fechaFinal=${fechaFinal}`;
    }

    // Función para filtrar la tabla en tiempo real
    function filterTable() {
        const input = document.getElementById('searchInput'); // Campo de búsqueda en tiempo real
        const filter = input.value.toLowerCase();
        const table = document.getElementById('dataTable'); // Identifica la tabla
    ...
    document.getElementById('searchInput').addEventListener('keypress', function(event) {
        ...
        if (event.key === 'Enter') {
            const searchValue = this.value; // Valor del input
...
<script>
    function showModal(idTipo) {
        fetch(`getTipoClasificaciones.php?id_tipo=${idTipo}`)
            .then(response => response.json())
            .then(data => {
                // Mostrar el nombre del tipo y fecha de creación
                document.getElementById('tipoNombre').textContent = data.nombreTipo;
                document.getElementById('tipoFecha').textContent = data.fechaCreacion;
                ...
                // Mostrar las clasificaciones relacionadas
                data.clasificaciones.forEach((clasificacion, index) => {
                    ...
                    iconoClasificacion.className = "fas fa-layer-group text-indigo-500";

                    const nombreClasificacion = document.createElement('span');
                    nombreClasificacion.className = "text-gray-700 font-medium";
                    nombreClasificacion.textContent = clasificacion;

                    liClasificacion.appendChild(numeroClasificacion);
                    liClasificacion.appendChild(iconoClasificacion);
                    liClasificacion.appendChild(nombreClasificacion);
