...
$totalQuery = "SELECT COUNT(DISTINCT a.id_almacen)
 FROM almacen a
...
if (!empty($sede)) {
    $totalQuery .= " AND a.id_sede = ?";
    $params[] = $sede;
}

// Agregar filtro por fechas
if (!empty($fechaInicio)) {
    $totalQuery .= " AND a.fecha_creacion >= ?";
    $params[] = $fechaInicio;
}
if (!empty($fechaFinal)) {
    $totalQuery .= " AND a.fecha_creacion <= ?";
    ...
    <div class="bg-white rounded-xl shadow-lg w-full max-w-lg overflow-hidden">
        ...
        <div class="px-6 py-4 flex justify-between items-center bg-gradient-to-r from-indigo-500 via-purple-500 to-indigo-500 text-white">
            <h2 class="text-xl font-semibold flex items-center">
                <i class="fas fa-tags mr-2"></i> Detalles del Almacen 
            </h2>
        ...
        <div class="px-6 py-4">
            <div class="flex items-center space-x-4">
                <i class="fas fa-warehouse text-indigo-500 text-2xl"></i>
                <p class="text-lg font-medium text-gray-800">
                    <strong>Nombre del Almacen:</strong> <span id="almacen"></span>
                </p>
            </div>
            <div class="flex items-center space-x-4">
...
<div id="filterForm" class="hidden mt-2 bg-white border border-gray-300 rounded shadow-lg p-4">
   ...
   <div id="savedFilters" class="mt-4">
    ...
    <form id="filterOptionsForm">
        <!-- Campo: Nombre de almacen -->
        <div class="mb-4">
            <label for="nombre" class="block text-gray-700 mb-2">Nombre de Almacen</label>
            <input 
              type="text" 
              id="nombre" 
...
<div class="border border-gray-300 rounded-lg shadow-md p-4">
    ...
    <div class="flex justify-between items-center mb-4 border-b border-gray-300 pb-2">
        <h1 class="text-xl font-semibold text-gray-700">Almacenes</h1>
        <div class="relative">
        ...
        <table class="table-auto w-full border-collapse border border-gray-300 shadow-lg rounded-lg">
            ...
            <tbody id="dataTable" class="bg-white divide-y divide-gray-200">
                ...
                <tr class="hover:bg-gray-100">
                    <td class="px-6 py-4"><?php echo $almace['id_almacen']; ?></td>
                    <td class="px-6 py-4"><?php echo $almace['nombre']; ?></td>
...
function applyFilters() {
    ...
    window.location.href = `?nombre=${encodeURIComponent(nombre)}&status=${status}&sede=${sede}&fechaInicio=${fechaInicio}&fechaFinal=${fechaFinal}`;
}
...
document.getElementById('searchInput').addEventListener('keyup', filterTable);
  ...
  <script>
    function showModal(valor) {
        fetch(`getalmacen_su_se.php?id_almacen=${valor}`)
            .then(response => response.json())
            .then(data => {
                document.getElementById('almacen').textContent = data.nombre;
                ...
                data.sede.forEach((modelo, index) => {
