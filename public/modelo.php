...
// Submenú actual: MODELO (id_submenu = 12)
$submenu_actual = 12;
...
$nombreModelo = isset($_GET['nombreModelo']) ? $_GET['nombreModelo'] : ''; // Filtro por nombre del modelo
$marcaSeleccionada = isset($_GET['marca']) ? $_GET['marca'] : ''; // Marca filtrada
...
$totalQuery = "
    SELECT COUNT(DISTINCT mo.id_modelo) 
    FROM modelo mo
...
if (!empty($marcaSeleccionada)) {
    $totalQuery .= " AND mm.id_marca = ?";
    $params[] = $marcaSeleccionada;
}
if (!empty($nombreModelo)) {
    $totalQuery .= " AND mo.nombre_modelo LIKE ?";
    $params[] = '%' . $nombreModelo . '%';
}
if (!empty($status)) {
    $totalQuery .= " AND mo.id_status = ?";
    $params[] = $status;
}
if (!empty($fechaInicio)) {
    $totalQuery .= " AND mo.fecha_creacion >= ?";
    $params[] = $fechaInicio;
}
if (!empty($fechaFinal)) {
    $totalQuery .= " AND mo.fecha_creacion <= ?";
...
// Consulta principal de datos (modelos únicos con paginación)
$query = "
    SELECT DISTINCT mo.id_modelo, mo.nombre_modelo, mo.año, mo.fecha_creacion, mo.id_status
    FROM modelo mo
    INNER JOIN marca_modelo mm ON mo.id_modelo = mm.id_modelo
    WHERE 1=1
";
...
// Agregar filtros
if (!empty($marcaSeleccionada)) {
    $query .= " AND mm.id_marca = ?";
}
if (!empty($nombreModelo)) {
    $query .= " AND mo.nombre_modelo LIKE ?";
}
if (!empty($status)) {
    $query .= " AND mo.id_status = ?";
}
if (!empty($fechaInicio)) {
    $query .= " AND mo.fecha_creacion >= ?";
}
if (!empty($fechaFinal)) {
    $query .= " AND mo.fecha_creacion <= ?";
}
...
<div id="modalModelo" class="fixed inset-0 bg-gray-900 bg-opacity-75 hidden justify-center items-center z-50">
    <div class="bg-white rounded-xl shadow-lg w-full max-w-lg overflow-hidden">
        <div class="px-6 py-4 flex justify-between items-center bg-gradient-to-r from-indigo-500 via-purple-500 to-indigo-500 text-white">
            <h2 class="text-xl font-semibold flex items-center">
                <i class="fas fa-cube mr-2"></i> Detalles del Modelo
            </h2>
        ...
        <div class="px-6 py-4">
            <div class="flex items-center space-x-4">
                ...
                <p class="text-lg font-medium text-gray-800">
                    <strong>Nombre del Modelo:</strong> <span id="modeloNombre"></span>
                </p>
            </div>
            <div class="flex items-center space-x-4 mt-4">
                <i class="fas fa-calendar-alt text-indigo-500 text-2xl"></i>
                <p class="text-lg font-medium text-gray-800">
                    <strong>Año:</strong> <span id="modeloAño"></span>
                </p>
            </div>

            <p class="mt-4 text-lg font-semibold text-gray-700">Marcas Relacionadas:</p>
            <ul id="marcasRelacionadas" class="grid grid-cols-2 gap-4 mt-4">
                <!-- Marcas dinámicas -->
            </ul>
        </div>
         <!-- Pie del modal -->
...
<div class="flex justify-between items-center mb-4">
    ...
    <div class="space-x-2">
        <?php if (in_array(2, $permisos)): ?>
            <button 
                onclick="window.location.href='formulario_guardar_modelo.php'" 
        ...
        <?php if (in_array(4, $permisos)): ?>
            <button 
                onclick="window.location.href='imprimir_modelos.php'" 
...
<div id="filterForm" class="hidden mt-2 bg-white border border-gray-300 rounded shadow-lg p-4">
   ...
   <div id="savedFilters" class="mt-4">
    ...
    <div class="mb-4">
        <label for="nombreModelo" class="block text-gray-700 mb-2">Nombre del Modelo</label>
        <input 
          type="text" 
          id="nombreModelo" 
          name="nombreModelo"
    ...
    <div class="grid grid-cols-2 gap-4 mb-4">
        ...
        <!-- Marcas -->
        <div>
            <label for="marca" class="block text-gray-700 mb-2">Marcas</label>
            <select 
                id="marca" 
                name="marca"
                class="w-full border border-gray-300 rounded p-2 focus:outline-none focus:ring-2 focus:ring-indigo-500"
...
<div class="border border-gray-300 rounded-lg shadow-md p-4">
    ...
    <div class="flex justify-between items-center mb-4 border-b border-gray-300 pb-2">
        <h1 class="text-xl font-semibold text-gray-700">Modelos</h1>
        <div class="relative">
    ...
    <tbody class="bg-white divide-y divide-gray-200" id="dataTable">
        ...
        <tr class="hover:bg-gray-100">
            <td class="px-6 py-4"><?php echo $modelo['id_modelo']; ?></td>
            <td class="px-6 py-4"><?php echo $modelo['nombre_modelo']; ?></td>
            <td class="px-6 py-4 flex space-x-2">
                <?php if ($modelo['id_status'] == 1): ?>
                    
                    <?php if (in_array(1, $permisos)): ?>
                    <button 
                        class="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600"
                        onclick="showModal(<?php echo $modelo['id_modelo']; ?>)">
                    ...
                    <button 
                        ...
                        onclick="window.location.href='formulario_modificar_modelo.php?id_modelo=<?php echo $modelo['id_modelo']; ?>'">
 ...
 <script>
    ...
    function applyFilters() {
        const nombreModelo = document.getElementById('nombreModelo').value; // Nuevo filtro por nombre de modelo
        const marca = document.getElementById('marca').value; // Filtro por marca
        const status = document.getElementById('status').value; // Filtro por status
        const fechaInicio = document.getElementById('fechaInicio').value;
        const fechaFinal = document.getElementById('fechaFinal').value;

     // Mostrar el mensaje si no hay filtros seleccionados
    if (!nombreModelo && !marca && !status && !fechaInicio && !fechaFinal) {
        ...
        window.location.href = `?nombreModelo=${encodeURIComponent(nombreModelo)}&marca=${marca}&status=${status}&fechaInicio=${fechaInicio}&fechaFinal=${fechaFinal}`;
    }
    ...
    // Si deseas ocultarlo automáticamente después de unos segundos, descomenta esta línea:
...
<script>
    function showModal(idModelo) {
        fetch(`getModeloMarcas.php?id_modelo=${idModelo}`)
            .then(response => response.json())
            .then(data => {
                document.getElementById('modeloNombre').textContent = data.nombreModelo;
                document.getElementById('modeloAño').textContent = data.año; // Mostrar el año del modelo

                const marcasRelacionadas = document.getElementById('marcasRelacionadas');
                marcasRelacionadas.innerHTML = ''; // Limpiar contenido previo

                data.marcas.forEach((marca, index) => {
                    const liMarca = document.createElement('li');
                    liMarca.className = "flex items-center space-x-2 bg-gray-100 p-2 rounded shadow-md";

                    const numeroMarca = document.createElement('span');
                    numeroMarca.className = "text-indigo-500 font-bold";
                    numeroMarca.textContent = `${index + 1}.`;

                    const iconoMarca = document.createElement('i');
                    iconoMarca.className = "fas fa-tags text-indigo-500";

                    const nombreMarca = document.createElement('span');
                    nombreMarca.className = "text-gray-700 font-medium";
                    nombreMarca.textContent = marca;

                    liMarca.appendChild(numeroMarca);
                    liMarca.appendChild(iconoMarca);
                    liMarca.appendChild(nombreMarca);

                    marcasRelacionadas.appendChild(liMarca);
                });
