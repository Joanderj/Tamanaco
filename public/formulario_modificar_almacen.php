...
$id_almacen = $_GET['id_almacen'] ?? null; // Obtener el ID 
if ($id_almacen) {
    $consulta1 = $conexion->query("SELECT nombre FROM almacen WHERE id_almacen = $id_almacen");
    $almacen = $consulta1->fetch_assoc();
}
?>
...
<hr>
 ...
 <div class="flex flex-col items-center mb-6">
    <div class="bg-blue-500 text-white w-16 h-16 rounded-full flex items-center justify-center shadow-lg mb-4">
        <i class="fas fa-warehouse text-3xl"></i> <!-- Ícono de etiquetas -->
    </div>
    <h2 class="text-3xl font-extrabold text-gray-800">Formulario de Almacen</h2>
    <p class="text-gray-600 mt-2 text-center">Edita Los Almacenes y Sus Vinculo  asociado de forma rápida y organizada.</p>
</div>
    <?php
...
<?php endif; ?>
    ...
    <form action="modificar_sucursal.php" method="POST" class="space-y-4" id="form">
        <!-- Nombre  -->
        <div>
        <label for="nombre" class="block font-semibold flex items-center">
        <i class="fas fa-warehouse text-blue-500 mr-2"></i> <!-- Ícono de marca -->
        Nombre del Almacen: <span class="text-red-600">*</span>
        </label> 
       ...
       oninput="convertirMayusculas(this); validarNombre()">
            ...
            <small id="mensaje-error-input" class="text-red-500 hidden">Este Almacen ya existe</small>
            <small id="mensaje-exito-input" class="text-green-500 hidden">¡Este Almacen está disponible!</small>
...
<button type="button" onclick="abrirVentanaEmergente()"
    ...
    Agregar Nueva Sede
      ...
      <label for="sucursal_principal" class="block text-gray-700 font-bold mb-2">
      <i class="fas fa-store text-blue-500 mr-2"></i> Seleccione la sucursal <span class="text-red-600">*</span> 
      </label>
      <div class="flex items-center gap-4">
        <select id="pais" name="sucursal" class="border border-gray-300 rounded p-2 w-full">
          <option value="">Selecciona una sucursal</option>
                        ...
                        $query = "SELECT id_sucursal, nombre FROM sucursal ORDER BY nombre ASC";
                        $result = $conexion->query($query);

                        if ($result->num_rows > 0) {
                            while ($row = $result->fetch_assoc()) {
                                echo "<option value='".htmlspecialchars($row["id_sucursal"])."'>".htmlspecialchars($row["nombre"])."</option>";
                            }
                        }
                        $conexion->close();
                        ?>
            ...
            <!-- select -->
            <div>
                <label for="sede" class="block text-gray-700 mb-2"><i class="fas fa-sun text-blue-500 mr-2"></i>Sede<span class="text-red-600">*</span></label>
                <select id="estado" name="sede" class="border border-gray-300 rounded p-2 w-full">
      <option value="">Selecciona una Sede</option>
                        ...
                        if ($result->num_rows > 0) {
                            while ($row = $result->fetch_assoc()) {
                                echo "<option value='".htmlspecialchars($row["id_sede"])."'>".htmlspecialchars($row["nombre_sede"])."</option>";
                            }
                        }
                        $conexion->close();
                        ?>
...
<p class="text-gray-500 text-sm mt-4">Todos los campos marcados con <span class="text-red-600">*</span> son obligatorios.</p>
        ...
        <div class="flex justify-between mt-4 space-x-4">
            <input type="text" name="id_sede" id="id_sede" value="<?php echo htmlspecialchars($id_almacen); ?>" class="hidden">
    <!-- Botón Guardar -->
...
<script>
    ...
    function validarNombre() {
        ...
        if (nombre.trim() !== '') {
            fetch(`validar_almacen_modificar.php?nombre=${encodeURIComponent(nombre)}&id=${encodeURIComponent(idsede)}`)
                ...
                .then(data => {
                    if (data.existe && data.id !== parseInt(idsede)) {
                        ...
                        textoMensajeGlobal.textContent = "Esta Almacen ya existe. Por favor, ingrese otra.";
...
<div id="ventanaEmergenteModelo" class="fixed inset-0 bg-black bg-opacity-50 hidden flex justify-center items-center">
    <div class="bg-white max-w-4xl mx-auto rounded-lg shadow-lg p-6">
        <div class="flex flex-col items-center mb-6">
            ...
            <h2 class="text-3xl font-extrabold text-gray-800">Formulario Sedr</h2>
            <p class="text-gray-600 mt-2 text-center">Registra una Sede.</p>
        </div>
           <!-- Contenedor global para mensajes de error -->
    ...
    <form id="formRegistrarModelo" method="POST" action="guardar_sede.php">
            <div class="mb-4">
                <label for="nombre2" class="block text-gray-700 font-bold mb-2">Nombre de la Sede <span class="text-red-600">*</span></label>
                <input type="text" id="nombre2" name="nombre2" placeholder="Ingrese el nombre del Sede"
                       class="border border-gray-300 rounded-lg p-2 w-full" required oninput="convertirMayusculas(this); validarNombre2()">
                       <small id="mensaje-error-input-modelo" class="text-red-500 hidden">Esta Sede ya existe</small>
                       <small id="mensaje-exito-input-modelo" class="text-green-500 hidden">¡Esta Sede está disponible!</small>
                <small class="text-gray-500">Ejemplo: Peltan</small>
            </div>
 <!-- Seleccionar sucursal principal -->
 <div class="mb-4">
      <label for="sucursal_principal" class="block text-gray-700 font-bold mb-2">
        Seleccione la sucursal principal de la sede
      </label>
      <div class="flex items-center gap-4">
        <select id="sucursal_principal" name="sucursal_principal" class="border border-gray-300 rounded p-2 w-full">
          <option value="" disabled selected>Seleccione una sucursal</option>
          <?php
          // Conexión a la base de datos
          $conexion = new mysqli('localhost', 'root', '', 'bd_tamanaco');
          $consulta = "SELECT id_sucursal, nombre FROM sucursal WHERE id_status = 1";
...
document.getElementById('formRegistrarModelo').addEventListener('submit', function (event) {
    ...
    fetch('guardar_sucursal_sede.php', {
        method: 'POST',
        body: formData,
    })
        ...
        .then(data => {
            ...
            if (data.success) {
                ...
                row.innerHTML = `
                    <td class='px-4 py-2'>
                        <input type='checkbox' name='sede[]' value='${data.id_sede}' onchange='highlightRow(this)'>
                    </td>
                    <td class='px-4 py-2 text-gray-700'>${data.nombre_sede}</td>
                `;
                tbody.appendChild(row); // Agregar la nueva fila

                // Mostrar mensaje de éxito
                mostrarMensaje('success', '¡Sede se ha guardado exitosamente!');
        ...
        .catch(error => {
            ...
            mostrarMensaje('error', 'Ocurrió un error al intentar guardar el Sede.');
        });
});
...
<script>
   function validarNombre2() {
    ...
    if (nombre2 !== '') { // Validación dinámica
        fetch(`validar_sede.php?nombre=${encodeURIComponent(nombre2)}`)
            .then(response => response.json())
            .then(data => {
                if (data.existe) {
                    // Mostrar mensaje de error
