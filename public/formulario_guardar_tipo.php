...
<?php endif; ?>
    ...
    <form action="guardar_tipo.php" method="POST" class="space-y-4" id="form-tipo">
    <!-- Nombre del Tipo -->
    <div>
...
<script>
    function validarNombreTipo() {
        const nombreTipo = document.getElementById('nombre_tipo').value.trim(); // Limpiar espacios en blanco al inicio y final
        const mensajeErrorInput = document.getElementById('mensaje-error-input'); // Elemento para mensaje de error
        const mensajeExitoInput = document.getElementById('mensaje-exito-input'); // Elemento para mensaje de éxito
        ...
        // Validación del campo "Nombre del Tipo"
        if (nombreTipo !== '') {
            fetch(`validar_tipo.php?nombre=${encodeURIComponent(nombreTipo)}`)
                ...
                .then(data => {
                    if (data.existe) {
                        ...
                        textoMensajeGlobal.textContent = "Esta modelo ya existe. Por favor, ingrese otra.";
            ...
            // Ocultar mensajes y deshabilitar el botón si el campo está vacío
            mensajeErrorInput.classList.add('hidden');
            mensajeExitoInput.classList.add('hidden');
            botonGuardar.disabled = true;
            botonGuardar.classList.add('bg-gray-400', 'cursor-not-allowed');
