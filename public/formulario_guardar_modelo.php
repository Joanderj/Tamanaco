...
<?php endif; ?>
    ...
    <form action="guardar_modelo.php" method="POST" class="space-y-4" id="form-modelo">
        <!-- Nombre del Modelo -->
        <div>
...
<div>
    <label for="vincular_opcion" class="block font-semibold flex items-center">
        ...
        Vincular Marcas:<span class="text-red-600">*</span>
    </label>
    <div class="flex space-x-4">
        <select id="vincular_opcion" name="vincular_opcion"
                class="w-full border border-gray-300 rounded-lg p-2" required
                onchange="toggleMarcaSelection()">
...
<!-- Vincular una Marca -->
<div id="vincular_una" class="hidden">
    <label for="marca_una" class="block font-semibold flex items-center">
        <i class="fas fa-tags text-blue-500 mr-2"></i> <!-- Ícono de marca -->
        Seleccionar una marca <span class="text-red-600">*</span>
    </label>
    <select name="marca_una" id="marca_una" class="w-full border border-gray-300 rounded-lg p-2">
        <?php
        $conexion = new mysqli("localhost", "root", "", "bd_tamanaco");
...
<div id="vincular_varias" class="hidden">
    <label class="block font-semibold"><i class="fas fa-tags text-blue-500 mr-2"></i> <!-- Ícono de marca -->Selecciona las marcas <span class="text-red-600">*</span></label>
        ...
        <tbody>
            ...
            while ($fila = $resultado->fetch_assoc()) {
                ...
                <tr class='hover:bg-blue-50' onclick='toggleSelection(this)'>
                    <td class='px-4 py-2'>
                        <input type='checkbox' name='marcas[]' value='{$fila['id_marca']}' onchange='highlightRow(this)'>
                    </td>
...
<script>
    function validarNombreModelo() {
        ...
        const mensajeErrorInput = document.getElementById('mensaje-error-input'); // Mensaje de error
        const mensajeExitoInput = document.getElementById('mensaje-exito-input'); // Mensaje de éxito
        const mensajeGlobal = document.getElementById('mensaje-global'); // Contenedor de mensaje global
        const tipoMensajeGlobal = document.getElementById('tipo-mensaje-global'); // Tipo de mensaje global
        const textoMensajeGlobal = document.getElementById('texto-mensaje-global'); // Texto de mensaje global
        const botonGuardar = document.getElementById('guardar'); // Botón guardar
        ...
        if (nombreModelo !== '') { // Validación dinámica
            fetch(`validar_modelo.php?nombre=${encodeURIComponent(nombreModelo)}`)
                ...
                .then(data => {
                    if (data.existe) {
                        mensajeErrorInput.classList.remove('hidden'); // Mostrar mensaje de error
                        mensajeExitoInput.classList.add('hidden'); // Ocultar mensaje de éxito
                        ...
                        mensajeErrorInput.classList.add('hidden'); // Ocultar mensaje de error
                        mensajeExitoInput.classList.remove('hidden'); // Mostrar mensaje de éxito
