...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($conexion->connect_error) {
        ...
        header("Location: formulario_inventario_repuesto_traslado.php");
        exit();
    }

    // Capturar datos del formulario y manejar valores nulos
    $id_repuesto = !empty($_POST['repuesto']) ? $_POST['repuesto'] : null; // Asumimos que 'repuesto' es el ID
    ...
    $stmt->bind_param("ss", $id_repuesto, $id_almacen);
    $stmt->execute();
    ...
    if ($row) {
        ...
        if ($cantidad_disponible >= $cantidad) {
            ...
            $update_stmt->bind_param("iss", $nueva_cantidad, $id_repuesto, $id_almacen);
            $update_stmt->execute();

            // Registrar el movimiento
            $id_tipo_movimiento = 3; // Asumiendo que este es el ID para 'traslado'
            $insert_movimiento_query = "INSERT INTO movimiento_repuesto (id_repuesto, id_almacen_origen, id_almacen_destino, cantidad, descripcion, id_tipo_movimiento, id_solicitud) VALUES (?, ?, ?, ?, ?, ?, ?)";
            $insert_movimiento_stmt = $conexion->prepare($insert_movimiento_query);
            $insert_movimiento_stmt->bind_param("ssissii", $id_repuesto, $id_almacen, $id_almacen_destino_traslado, $cantidad, $descripcion, $id_tipo_movimiento, $solicitud);
            $insert_movimiento_stmt->execute();

            $_SESSION['success'] = "Traslado realizado correctamente.";
            // Redireccionar a inventario_repuesto en caso de éxito
        ...
        $_SESSION['error'] = "La repuesto no existe en el almacén especificado.";
    }
