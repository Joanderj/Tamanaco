...
$query = "SELECT h.*, 
          ...
          FROM herramientas h
          LEFT JOIN marca m ON m.id_marca = h.id_marca
          LEFT JOIN modelo mo ON mo.id_modelo = h.id_modelo
          LEFT JOIN tipo t ON t.id_tipo = h.id_tipo
          LEFT JOIN inventario_herramientas inv ON inv.herramienta_id = h.id_herramienta
...
if (!$herramienta) {
    $herramienta = [
        'url' => '/images/default-tool.png',
        'nombre_herramienta' => 'Herramienta no encontrada',
        'nombre_marca' => null,
        'nombre_modelo' => null,
        'nombre_tipo' => null,
        'descripcion' => null,
    ...
    <div class="grid grid-cols-1 md:grid-cols-2">
        <div class="bg-gray-100 p-6 flex justify-center items-center">
            <?php if (!empty($herramienta['url'])): ?>
                <img src="<?= htmlspecialchars($herramienta['url']) ?>" alt="<?= htmlspecialchars($herramienta['nombre_herramienta']) ?>" class="w-64 h-64 object-contain rounded border">
            <?php else: ?>
        ...
        <div class="p-6">
            <h2 class="text-2xl font-bold mb-2">
                ...
                <?= strtoupper(htmlspecialchars($herramienta['nombre_herramienta'])) ?>
            </h2>

            <?php if (!empty($herramienta['descripcion'])): ?>
                <p class="text-gray-600 mb-4 italic"><?= nl2br(htmlspecialchars($herramienta['descripcion'])) ?></p>
            <?php endif; ?>

            <div class="space-y-2 text-sm">
                <p><span class="font-semibold">Marca:</span> <?= htmlspecialchars($herramienta['nombre_marca'] ?? '-') ?></p>
                <p><span class="font-semibold">Modelo:</span> <?= htmlspecialchars($herramienta['nombre_modelo'] ?? '-') ?></p>
                <p><span class="font-semibold">Tipo:</span> <?= htmlspecialchars($herramienta['nombre_tipo'] ?? '-') ?></p>
                <p><span class="font-semibold">Ubicación:</span> <?= htmlspecialchars($herramienta['id_sede'] ?? '-') ?></p>
                <p><span class="font-semibold">Cantidad disponible:</span> <?= $herramienta['cantidad'] ?? '-' ?></p>
                <p><span class="font-semibold">Stock máximo:</span> <?= $herramienta['stock_maximo'] ?? '-' ?></p>
                <p><span class="font-semibold">Punto de reorden:</span> <?= $herramienta['punto_reorden'] ?? '-' ?></p>
                <p><span class="font-semibold">Última reposición:</span> <?= $herramienta['fecha_ultima_reposicion'] ? date('d/m/Y', strtotime($herramienta['fecha_ultima_reposicion'])) : '-' ?></p>
                <p><span class="font-semibold">Estatus:</span> 
                    <?= ($herramienta['id_status'] == 1) ? '<span class="text-green-600 font-semibold">Activo</span>' : '<span class="text-red-600 font-semibold">Inactivo</span>' ?>
                </p>
                <p><span class="font-semibold">Fecha de creación:</span> 
                    <?= $herramienta['date_created'] ? date('d/m/Y H:i', strtotime($herramienta['date_created'])) : '-' ?>
                </p>
            </div>

            <div class="mt-6">
                <a href="javascript:window.close();" class="inline-block bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700">
                    <i class="fas fa-arrow-left mr-1"></i> Cerrar
                </a>
            </div>
        </div>
    </div>
