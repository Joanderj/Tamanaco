...
$totalQuery = "SELECT COUNT(DISTINCT s.id_sede)
FROM sede s
INNER JOIN sucursal sc ON sc.id_sucursal = s.id_sucursal_fija
WHERE 1=1";
$params = [];

if (!empty($nombreSede)) {
    $totalQuery .= " AND s.nombre_sede LIKE ?";
    $params[] = '%' . $nombreSede . '%'; // Agregar wildcard para búsqueda
}

if (!empty($status)) {
    $totalQuery .= " AND s.id_status = ?";
    $params[] = $status;
}

if (!empty($sucursal)) {
    $totalQuery .= " AND s.id_sucursal_fija = ?";
    $params[] = $sucursal;
}

// Agregar filtro por fechas
if (!empty($fechaInicio)) {
    $totalQuery .= " AND s.fecha_creacion >= ?";
    $params[] = $fechaInicio;
}
if (!empty($fechaFinal)) {
    $totalQuery .= " AND s.fecha_creacion <= ?";
...
// Consulta de datos
$query = "SELECT distinct s.id_sede,s.nombre_sede,sc.nombre as sucursal
 FROM sede s 
 INNER JOIN sucursal sc ON sc.id_sucursal = s.id_sucursal_fija
...
if (!empty($status)) {
    $query .= " AND s.id_status = ?";
}

if (!empty($sucursal)) {
    $query .= " AND s.id_sucursal_fija = ?";
}
...
<div id="modalver" class="fixed inset-0 bg-gray-900 bg-opacity-75 hidden justify-center items-center z-50">
    <div class="bg-white rounded-xl shadow-lg w-full max-w-lg overflow-hidden">
        ...
        <div class="px-6 py-4 flex justify-between items-center bg-gradient-to-r from-indigo-500 via-purple-500 to-indigo-500 text-white">
            <h2 class="text-xl font-semibold flex items-center">
                <i class="fas fa-tags mr-2"></i> Detalles de la Sede
            </h2>
        ...
        <div class="px-6 py-4">
            ...
            <div class="flex items-center space-x-4">
                ...
                <p class="text-lg font-medium text-gray-800">
                    <strong>Sucursal Principal:</strong> <span id="fija"></span>
                </p>
            </div>
            <div class="flex items-center space-x-4">
 ...
 <div class="container mx-auto px-4 py-6">
    ...
    <div class="flex justify-between items-center mb-4">
      ...
      <div class="space-x-2">
        <button  onclick="window.location.href='formulario_sede.php'" class="bg-indigo-500 text-white px-4 py-2 rounded hover:bg-indigo-600">
...
<div class="border border-gray-300 rounded-lg shadow-md p-4">
    ...
    <div class="flex justify-between items-center mb-4 border-b border-gray-300 pb-2">
        <h1 class="text-xl font-semibold text-gray-700">Sedes</h1>
        <div class="relative">
        ...
        <table class="table-auto w-full border-collapse border border-gray-300 shadow-lg rounded-lg">
            ...
            <tbody id="dataTable" class="bg-white divide-y divide-gray-200">
                ...
                <tr class="hover:bg-gray-100">
                    <td class="px-6 py-4"><?php echo $sede['id_sede']; ?></td>
                    <td class="px-6 py-4"><?php echo $sede['nombre_sede']; ?></td>
                    <td class="px-6 py-4"><?php echo $sede['sucursal']; ?></td>
                    <td class="px-6 py-4 flex space-x-2">
                    <button 
        class="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600"
        onclick="showModal(<?php echo $sede['id_sede']; ?>)">
        <i class="fas fa-eye"></i> Ver
    </button>
    <button class="bg-yellow-500 text-white px-4 py-2 rounded hover:bg-yellow-600" 
    onclick="window.location.href='formulario_modificar_sede.php?id_sede=<?php echo $sede['id_sede']; ?>'">
...
document.getElementById('searchInput').addEventListener('keyup', filterTable);
  ...
  <script>
    function showModal(valor) {
        fetch(`getSedeSucursal.php?id_sede=${valor}`)
            .then(response => response.json())
            .then(data => {
                document.getElementById('sede').textContent = data.nombre;
                document.getElementById('status').textContent = data.status;
                document.getElementById('fija').textContent = data.fija;
