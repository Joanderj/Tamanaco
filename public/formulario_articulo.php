...
<hr>
 ...
 <div class="flex flex-col items-center mb-6">
    <div class="bg-blue-500 text-white w-16 h-16 rounded-full flex items-center justify-center shadow-lg mb-4">
        <i class="fas fa-question-circle text-3xl"></i> <!-- Ícono de etiquetas -->
    </div>
    <h2 class="text-3xl font-extrabold text-gray-800">Formulario de Articulo</h2>
    <p class="text-gray-600 mt-2 text-center">Registra los Articulos y vincula los Articulos con la Planta de forma rápida y organizada.</p>
</div>
    <?php
...
<?php endif; ?>
  ...
  <form id="formRegistrarArticulo" action="registrar_articulo.php" method="POST">
    <!-- Nombre de la articulo -->
    <div class="mb-4">
      <label for="nombre_articulo" class="block text-gray-700 font-bold mb-2"><i class="fas fa-question-circle text-blue-500 mr-2"></i>Nombre del Articulo<span class="text-red-600">*</span></label>
      <input type="text" id="nombre_articulo" name="nombre_articulo" placeholder="Ingrese el nombre de la Articulo"
       ...
       <small id="mensaje-error-input" class="text-red-500 hidden">Esta Articulo ya existe</small>
            <small id="mensaje-exito-input" class="text-green-500 hidden">¡Esta Articulo está disponible!</small>
            <small class="text-gray-500">Ejemplo: SACO DE BOXEO</small>
    </div>
    <div class="mb-4">
      <label for="descripcion" class="block text-gray-700 font-bold mb-2"><i class="fas fa-asterisk text-blue-500 mr-2"></i>Descripcion<span class="text-red-600">*</span></label>
      <input type="text" id="descripcion" name="descripcion" placeholder="Ingrese una descripcion"
             class="border border-gray-300 rounded p-2 w-full" required oninput="convertirMayusculas(this);">
    </div>
...
<div class="mb-4">
  <label for="vincularSucursales" class="block text-gray-700 font-bold mb-2">
    ¿Deseas vincular esta articulo con una Planta?
  </label>
  <select 
...
<label for="nombre_articulo" class="block text-gray-700 font-bold mb-2"><i class="fas fa-industry text-blue-500 mr-2"></i>Nombre de la Planta<span class="text-red-600">*</span></label>
  <table class="table-auto border-collapse w-full text-left text-gray-700">
    <thead>
      <tr>
        <th class="border-b-2 border-gray-300 px-4 py-2">Seleccionar</th>
    ...
    <tbody>
      ...
      // Conexión a la base de datos
      $conexion = new mysqli('localhost', 'root', '', 'bd_tamanaco');
      $consulta = "SELECT id_planta, nombre FROM planta WHERE id_status = 1";
      $resultado = $conexion->query($consulta);

      while ($fila = $resultado->fetch_assoc()) {
          echo "<tr>
                  <td class='border-b border-gray-300 px-4 py-2'>
                    <input type='checkbox' name='planta[]' value='{$fila['id_planta']}' />
                  </td>
                  <td class='border-b border-gray-300 px-4 py-2'>{$fila['id_planta']}</td>
                  <td class='border-b border-gray-300 px-4 py-2'>{$fila['nombre']}</td>
                </tr>";
      }
      $conexion->close();
      ?>
...
<!-- Ventana emergente para nueva sucursal -->
<div id="ventanaEmergente" class="fixed inset-0 bg-black bg-opacity-50 hidden flex justify-center items-center">
  <div class="bg-white p-6 rounded-lg shadow-lg max-w-lg w-full">
    <h3 class="text-lg font-bold text-gray-700 mb-4">Registrar Nueva Sucursal</h3>
    <form id="formRegistrarSucursal" action="registrar_sucursal.php" method="POST">
      <div class="mb-4">
        <label for="nombre_sucursal" class="block text-gray-700 font-bold mb-2">Nombre de la Sucursal</label>
        <input type="text" id="nombre_sucursal" name="nombre_sucursal" placeholder="Ingrese el nombre de la sucursal"
               class="border border-gray-300 rounded p-2 w-full" required>
      </div>
      <div class="mb-4">
        <label for="direccion_sucursal" class="block text-gray-700 font-bold mb-2">Dirección</label>
        <input type="text" id="direccion_sucursal" name="direccion_sucursal" placeholder="Ingrese la dirección"
...
<script>
    function validarNombre() {
        const nombre_articulo = document.getElementById('nombre_articulo').value;
        ...
        if (nombre_articulo.trim() !== '') {
            fetch(`validar_articulo.php?nombre=${encodeURIComponent(nombre_articulo)}`)
                .then(response => response.json())
                .then(data => {
                    if (data.existe) {
                        ...
                        textoMensajeGlobal.textContent = "Esta Articulo ya existe. Por favor, ingrese otra.";
