...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $face = !empty($_POST['face']) ? $_POST['face'] : null; // faceboock  de la empresa
    $insta = !empty($_POST['insta']) ? $_POST['insta'] : null; // instagram de la empresa
    $twi = !empty($_POST['twi']) ? $_POST['twi'] : null; // twitter de la empresa
    $you = !empty($_POST['you']) ? $_POST['you'] : null; // youtube de la empresa
    $what = !empty($_POST['what']) ? $_POST['what'] : null; // whatsapp de la empresa
    ...
    try {
        ...
        if ($stmt_check->num_rows > 0) {
            ...
            $stmt = $conexion->prepare("UPDATE empresa SET facebook=?, instagram=?, twitter=?,youtube=?,whatsapp=? WHERE id_empresa=?");
            if (!$stmt) {
                throw new Exception("Error al preparar la consulta de actualización: " . $conexion->error);
            }
            $stmt->bind_param("sssssi", $face, $insta, $twi, $you,$what, $id_empresa);
            $action = "actualizada";
        } else {
            // Si no existe, insertar una nueva empresa con id_empresa = 1
            $stmt = $conexion->prepare("INSERT INTO empresa (id_empresa, facebook, instagram, twitter, youtube,whatsapp, status_id_status) VALUES (?,?, ?, ?, ?, ?, ?)");
            if (!$stmt) {
                throw new Exception("Error al preparar la consulta de inserción: " . $conexion->error);
            }
            $stmt->bind_param("isssssi", $id_empresa, $face, $insta, $twi, $you,$what, $id_status);
            $action = "registrada";
        }
        ...
        header("Location: redes_sociales_empresa.php");
