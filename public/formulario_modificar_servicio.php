...
try {
    ...
    // Obtener todas las herramientas disponibles con marca, modelo y tipo
    $stmt = $conn->prepare("
        SELECT 
            h.*, 
    ...
    // Herramientas seleccionadas para el servicio
    $stmt = $conn->prepare("
        SELECT 
            sh.*, 
    ...
    ");
    $stmt->execute([$id_servicio]);
    $herramientas_seleccionadas = $stmt->fetchAll(PDO::FETCH_ASSOC);
    ...
    $stmt = $conn->prepare("
        SELECT 
            sm.*, 
            m.nombre_maquina 
        FROM servicio_maquina sm
        JOIN maquina m ON sm.id_maquina = m.id_maquina
        WHERE sm.id_servicio = ?
    ");
    $stmt->execute([$id_servicio]);
    $maquinas_seleccionadas = $stmt->fetchAll(PDO::FETCH_ASSOC);
...
$sql = "SELECT sp.id_producto, sp.cantidad,
               p.nombre_producto, p.unidad_medida,
               m.nombre_marca, mo.nombre_modelo,
               t.nombre_tipo, c.nombre_clasificacion,
               p.url
...
// Obtener los IDs de proveedores vinculados al servicio actual
$stmt_vinculados = $conn->prepare("
    SELECT id_proveedor 
    FROM proveedor_servicio 
    WHERE id_servicio = ?
");
$stmt_vinculados->execute([$id_servicio]); // Asegúrate de tener $id_servicio definido
$proveedor_ids_vinculados = array_column($stmt_vinculados->fetchAll(PDO::FETCH_ASSOC), 'id_proveedor');
    ...
    ");
    $stmt->execute([$id_servicio]);
    $repuestos_seleccionados = $stmt->fetchAll(PDO::FETCH_ASSOC);
...
<form action="modificar_servicio.php" method="post" enctype="multipart/form-data" id="serviceForm" class="space-y-6">
    ...
    <div>
        ...
        <input value="<?= htmlspecialchars($servicio['nombre_servicio']) ?>" type="text" name="nombre_servicio" id="nombre_servicio" required placeholder="Ej: Mantenimiento preventivo"
    ...
    <div>
        ...
        <input type="hidden" name="descripcion" id="descripcion" value="<?= htmlspecialchars($servicio['descripcion'] ?? '', ENT_QUOTES, 'UTF-8') ?>">
        <div id="editor" class="border border-gray-300 p-2 min-h-[120px]"><?= $servicio['descripcion'] ?? '' ?></div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
            ...
            <input value="<?= $servicio['tiempo_programado'] ?>" type="number" name="tiempo_programado" id="tiempo_programado" min="0" step="0.1"
        ...
        <div>
            ...
            <input value="<?= $servicio['tiempo_paro_maquina'] ?>" type="number" name="tiempo_paro_maquina" id="tiempo_paro_maquina" min="0" step="0.1"
                class="w-full border border-gray-300 rounded-md p-3">
        </div>
        </div>
    <?php
    // Determinar el tipo de servicio y el elemento asociado (máquina o repuesto)
    // Consultar primero si existe una máquina asociada
    $stmt = $conn->prepare("SELECT id_maquina FROM servicio_maquina WHERE id_servicio = ?");
    $stmt->execute([$id_servicio]);
    $row_maquina = $stmt->fetch(PDO::FETCH_ASSOC);

    $stmt = $conn->prepare("SELECT id_repuesto FROM servicio_piezas WHERE id_servicio = ?");
    $stmt->execute([$id_servicio]);
    $row_repuesto = $stmt->fetch(PDO::FETCH_ASSOC);

    if ($row_maquina && $row_maquina['id_maquina']) {
        $tipo_servicio = 'maquina';
        $id_maquina_seleccionada = $row_maquina['id_maquina'];
        $id_repuesto_seleccionado = '';
    } elseif ($row_repuesto && $row_repuesto['id_repuesto']) {
        $tipo_servicio = 'pieza';
        $id_maquina_seleccionada = '';
        $id_repuesto_seleccionado = $row_repuesto['id_repuesto'];
    } else {
        $tipo_servicio = '';
        $id_maquina_seleccionada = '';
        $id_repuesto_seleccionado = '';
    ...
    // Cargar datos del elemento asociado para mostrar en la interfaz
    $nombre_elemento = $marca_elemento = $modelo_elemento = $tipo_elemento = $img_elemento = '';
    if ($tipo_servicio === 'maquina' && $id_maquina_seleccionada) {
        $stmt = $conn->prepare("SELECT m.nombre_maquina, ma.nombre_marca, mo.nombre_modelo, t.nombre_tipo, m.url 
            FROM maquina m
            LEFT JOIN marca ma ON m.id_marca = ma.id_marca
            LEFT JOIN modelo mo ON m.id_modelo = mo.id_modelo
            LEFT JOIN tipo t ON m.id_tipo = t.id_tipo
            WHERE m.id_maquina = ?");
        $stmt->execute([$id_maquina_seleccionada]);
        $maq = $stmt->fetch(PDO::FETCH_ASSOC);
        if ($maq) {
            $nombre_elemento = $maq['nombre_maquina'];
            $marca_elemento = $maq['nombre_marca'] ?? '';
            $modelo_elemento = $maq['nombre_modelo'] ?? '';
            $tipo_elemento = $maq['nombre_tipo'] ?? '';
            $img_elemento = $maq['url'] ?? 'img/maquina_default.png';
        }
    } elseif ($tipo_servicio === 'pieza' && $id_repuesto_seleccionado) {
        $stmt = $conn->prepare("SELECT r.nombre_repuesto, m.nombre_marca, mo.nombre_modelo, t.nombre_tipo, r.url 
            FROM repuesto r
            LEFT JOIN marca m ON r.id_marca = m.id_marca
            LEFT JOIN modelo mo ON r.id_modelo = mo.id_modelo
            LEFT JOIN tipo t ON r.id_tipo = t.id_tipo
            WHERE r.id_repuesto = ?");
        $stmt->execute([$id_repuesto_seleccionado]);
        $rep = $stmt->fetch(PDO::FETCH_ASSOC);
        if ($rep) {
            $nombre_elemento = $rep['nombre_repuesto'];
            $marca_elemento = $rep['nombre_marca'] ?? '';
            $modelo_elemento = $rep['nombre_modelo'] ?? '';
            $tipo_elemento = $rep['nombre_tipo'] ?? '';
            $img_elemento = $rep['url'] ?? 'img/repuesto_default.png';
        }
    }
    ?>

<div class="mt-6">
    <h3 class="text-xl font-semibold mb-4 text-gray-700">Elemento asociado al servicio</h3>

    <!-- BLOQUE PARA MÁQUINA -->
    <?php if ($tipo_servicio === 'maquina' && $id_maquina_seleccionada): ?>
        <div class="bg-white shadow-md rounded-lg p-4 flex items-center space-x-4 border border-blue-300 mb-4">
            <img src="<?= htmlspecialchars($img_elemento) ?>" alt="Imagen de máquina" class="w-24 h-24 object-cover rounded-lg border">
            <div>
                <p class="text-lg font-bold text-blue-700"><?= htmlspecialchars($nombre_elemento) ?></p>
                <p class="text-sm text-gray-600">Marca: <span class="font-medium"><?= htmlspecialchars($marca_elemento) ?></span></p>
                <p class="text-sm text-gray-600">Modelo: <span class="font-medium"><?= htmlspecialchars($modelo_elemento) ?></span></p>
                <p class="text-sm text-gray-600">Tipo: <span class="font-medium"><?= htmlspecialchars($tipo_elemento) ?></span></p>
                <span class="inline-block mt-2 px-3 py-1 text-sm font-semibold text-white bg-blue-500 rounded-full">Servicio a Máquina</span>
            </div>
        </div>
    <?php endif; ?>

    <!-- BLOQUE PARA PIEZA -->
    <?php if ($tipo_servicio === 'pieza' && $id_repuesto_seleccionado): ?>
        <div class="bg-white shadow-md rounded-lg p-4 flex items-center space-x-4 border border-green-300 mb-4">
            <img src="<?= htmlspecialchars($img_elemento) ?>" alt="Imagen de pieza" class="w-24 h-24 object-cover rounded-lg border">
            <div>
                <p class="text-lg font-bold text-green-700"><?= htmlspecialchars($nombre_elemento) ?></p>
                <p class="text-sm text-gray-600">Marca: <span class="font-medium"><?= htmlspecialchars($marca_elemento) ?></span></p>
                <p class="text-sm text-gray-600">Modelo: <span class="font-medium"><?= htmlspecialchars($modelo_elemento) ?></span></p>
                <p class="text-sm text-gray-600">Tipo: <span class="font-medium"><?= htmlspecialchars($tipo_elemento) ?></span></p>
                <span class="inline-block mt-2 px-3 py-1 text-sm font-semibold text-white bg-green-500 rounded-full">Servicio a Pieza</span>
            </div>
        </div>
    <?php endif; ?>

    <!-- BLOQUE DE ADVERTENCIA SI NO HAY ELEMENTO ASOCIADO -->
    <?php if (empty($id_maquina_seleccionada) && empty($id_repuesto_seleccionado)): ?>
        <div class="bg-yellow-100 border-l-4 border-yellow-500 text-yellow-700 p-4 rounded">
            <p class="font-semibold">Atención:</p>
            <p>No se ha especificado aún a quién se dirige el servicio (máquina o pieza).</p>
        </div>
    <?php endif; ?>
</div>
...
// Obtener datos generales con PDO
try {
    $stmtRepuestos = $conn->prepare("
        SELECT r.*, m.nombre_marca AS marca, mo.nombre_modelo AS modelo, t.nombre_tipo AS tipo
        FROM repuesto r
        LEFT JOIN marca m ON r.id_marca = m.id_marca
        LEFT JOIN modelo mo ON r.id_modelo = mo.id_modelo
        LEFT JOIN tipo t ON r.id_tipo = t.id_tipo
    ...
    $stmtProductos = $conn->prepare("
        SELECT p.*, m.nombre_marca AS nombre_marca, mo.nombre_modelo AS nombre_modelo, 
               t.nombre_tipo AS nombre_tipo, c.nombre_clasificacion AS nombre_clasificacion, 
               p.unidad_medida
        FROM producto p
        LEFT JOIN marca m ON p.id_marca = m.id_marca
        LEFT JOIN modelo mo ON p.id_modelo = mo.id_modelo
    ...
    $stmtHerramientas = $conn->prepare("
        SELECT h.*, m.nombre_marca AS marca, mo.nombre_modelo AS modelo, t.nombre_tipo AS tipo
        FROM herramientas h
        LEFT JOIN marca m ON h.id_marca = m.id_marca
        LEFT JOIN modelo mo ON h.id_modelo = mo.id_modelo
        LEFT JOIN tipo t ON h.id_tipo = t.id_tipo
    ...
    $stmtMaquinas = $conn->prepare("
        SELECT m.*, ma.nombre_marca AS marca, mo.nombre_modelo AS modelo, t.nombre_tipo AS tipo
        FROM maquina m
        LEFT JOIN marca ma ON m.id_marca = ma.id_marca
        LEFT JOIN modelo mo ON m.id_modelo = mo.id_modelo
        LEFT JOIN tipo t ON m.id_tipo = t.id_tipo
    ...
    $maquinas = $stmtMaquinas->fetchAll();
} catch (PDOException $e) {
    die("Error al obtener datos: " . $e->getMessage());
}

// Verificar que el servicio exista
$stmt = $conexion->prepare("SELECT * FROM servicio WHERE id_servicio = ?");
...
// Datos seleccionados (mysqli)
$repuestos_seleccionados = [];
$result = $conexion->query("
    SELECT sr.*, r.nombre_repuesto, r.url, m.nombre_marca AS marca, 
           mo.nombre_modelo AS modelo, t.nombre_tipo AS tipo
    FROM servicio_repuesto sr
    JOIN repuesto r ON sr.id_repuesto = r.id_repuesto
    LEFT JOIN marca m ON r.id_marca = m.id_marca
    LEFT JOIN modelo mo ON r.id_modelo = mo.id_modelo
...
while ($row = $result->fetch_assoc()) {
    $repuestos_seleccionados[] = $row;
}
?>

<!-- Interfaz de repuestos -->
<div class="relative">
    <label class="font-semibold block mb-2">Repuestos que serán reemplazados</label>
    <div id="repuestosSeleccionados" class="space-y-2 mb-2">
        <?php foreach ($repuestos_seleccionados as $sel): ?>
            <div class="flex items-center space-x-3 border rounded p-2">
                <input type="hidden" name="repuestos[<?= $sel['id_repuesto'] ?>][id]" value="<?= $sel['id_repuesto'] ?>">
                <img src="<?= htmlspecialchars($sel['url'] ?? 'img/repuesto_default.png') ?>" class="w-10 h-10 object-contain rounded border">
                <div class="text-sm flex-grow">
                    <div class="font-semibold"><?= htmlspecialchars($sel['nombre_repuesto']) ?></div>
                    <div class="text-gray-500 text-xs"><?= $sel['marca'] ?? '---' ?> | <?= $sel['modelo'] ?? '---' ?> | <?= $sel['tipo'] ?? '---' ?></div>
                </div>
                <input type="number" name="repuestos[<?= $sel['id_repuesto'] ?>][cantidad]" min="1" value="<?= $sel['cantidad'] ?>" placeholder="Cantidad"
                    class="border border-gray-300 rounded p-1 w-20 text-sm">
                <button type="button" onclick="this.parentElement.remove(); repuestoIds.delete('<?= $sel['id_repuesto'] ?>')" 
...
<script>
    ...
    const repuestoIds = new Set(<?= json_encode(array_column($repuestos_seleccionados, 'id_repuesto')) ?>);
...
<div class="relative">
    ...
    <div id="productosSeleccionados" class="space-y-2 mb-2">
        <?php foreach ($productos_seleccionados as $p): ?>
            <div class="flex items-center space-x-3 border rounded p-2">
                <input type="hidden" name="productos[<?= $p['id_producto'] ?>][id_producto]" value="<?= $p['id_producto'] ?>">
                <img src="<?= htmlspecialchars($p['url'] ?? 'img/producto_default.png') ?>" class="w-10 h-10 object-contain rounded border">
                <div class="text-sm flex-grow">
                    ...
                    <div class="text-gray-500 text-xs">
                        <?= htmlspecialchars($p['nombre_clasificacion']) ?> | <?= htmlspecialchars($p['unidad_medida']) ?>
                    </div>
                </div>
                <input type="number" name="productos[<?= $p['id_producto'] ?>][cantidad]" min="1" value="<?= $p['cantidad'] ?>" class="border border-gray-300 rounded p-1 w-20 text-sm">
                <button type="button" onclick="this.parentElement.remove(); productoIds.delete('<?= $p['id_producto'] ?>')" class="text-red-500 text-sm ml-2 hover:text-red-700">Quitar</button>
            </div>
        <?php endforeach; ?>
    </div>
    ...
    <div id="productosList" class="absolute z-10 bg-white border border-gray-300 rounded-md mt-1 w-full max-h-64 overflow-y-auto hidden shadow-lg">
        <?php foreach($productos as $p): ?>
            ...
            >
                ...
                <div class="text-sm">
                    <div class="font-semibold"><?= htmlspecialchars($p['nombre_producto']) ?></div>
                    <div class="text-gray-500 text-xs"><?= htmlspecialchars($p['nombre_marca']) ?> | <?= htmlspecialchars($p['nombre_modelo']) ?> | <?= htmlspecialchars($p['nombre_tipo']) ?></div>
                    <div class="text-gray-500 text-xs">Clasificación: <?= htmlspecialchars($p['nombre_clasificacion']) ?> | Unidad: <?= htmlspecialchars($p['unidad_medida']) ?></div>
                </div>
            </div>
        <?php endforeach; ?>
    </div>
...
<script>
    ...
    // Cargar productos ya seleccionados (desde PHP)
    <?php foreach ($productos_seleccionados as $p): ?>
        productoIds.add('<?= $p['id_producto'] ?>');
    <?php endforeach; ?>
...
<div class="relative">
    ...
    <!-- Herramientas ya seleccionadas -->
    <div id="herramientasSeleccionadas" class="space-y-2 mb-2">
        <?php foreach ($herramientas_seleccionadas as $h): ?>
            <div class="flex items-center space-x-3 border rounded p-2">
                <input type="hidden" name="herramientas[<?= $h['id_herramienta'] ?>][id_servicio]" value="<?= $h['id_herramienta'] ?>">
                <img src="<?= htmlspecialchars($h['url'] ?? 'img/herramienta_default.png') ?>" class="w-10 h-10 object-contain rounded border">
                <div class="text-sm flex-grow">
                    <div class="font-semibold"><?= htmlspecialchars($h['nombre_herramienta']) ?></div>
                    <div class="text-gray-500 text-xs"><?= htmlspecialchars($h['marca'] ?? '---') ?> | <?= htmlspecialchars($h['modelo'] ?? '---') ?> | <?= htmlspecialchars($h['tipo'] ?? '---') ?></div>
                </div>
                <input type="number" name="herramientas[<?= $h['id_herramienta'] ?>][cantidad]" min="1" value="<?= $h['cantidad'] ?>" class="border border-gray-300 rounded p-1 w-20 text-sm">
                <button type="button" onclick="this.parentElement.remove(); herramientaIds.delete('<?= $h['id_herramienta'] ?>')" class="text-red-500 text-sm ml-2 hover:text-red-700">Quitar</button>
            </div>
        <?php endforeach; ?>
    </div>
...
<script>
    ...
    // Cargar IDs de herramientas ya seleccionadas (desde PHP)
    <?php foreach ($herramientas_seleccionadas as $h): ?>
        herramientaIds.add('<?= $h['id_herramienta'] ?>');
    <?php endforeach; ?>
...
<div id="contenedorProveedores" class="mt-4">
    ...
    <div id="listaProveedores" class="space-y-2 max-h-64 overflow-y-auto border border-gray-200 rounded p-2">
        <?php foreach ($proveedores as $prov): 
            $seleccionado = in_array($prov['id_proveedor'], $proveedor_ids_vinculados);
        ?>
            <div class="proveedor-item flex justify-between items-center bg-white p-2 rounded border cursor-pointer <?= $seleccionado ? 'bg-blue-100 border-blue-400' : '' ?>" data-nombre="<?= strtolower($prov['nombre_proveedor']) ?>" data-id="<?= $prov['id_proveedor'] ?>">
                <span class="font-medium"><?= htmlspecialchars($prov['nombre_proveedor']) ?></span>
                <span class="check <?= $seleccionado ? '' : 'hidden' ?> text-green-600 font-bold">✔️</span>
                <input type="checkbox" name="proveedor_id[]" value="<?= $prov['id_proveedor'] ?>" class="hidden" <?= $seleccionado ? 'checked' : '' ?>>
            </div>
        <?php endforeach; ?>
    </div>
...
<script>
    ...
    window.addEventListener('DOMContentLoaded', () => {
        ...
        // Refrescar estado visual de seleccionados (si recargó)
