...
// Asegúrate de que $perfil['id_usuario'] esté definido
if (isset($perfil['id_usuario'])) {
    $usuario = htmlspecialchars($perfil['id_usuario']);
} else {
    // Manejo de error si id_usuario no está definido
    die("Error: ID de usuario no definido.");
}

// Consulta para contar los inicios de sesión por mes y año
$sql_estadisticas = "
    SELECT DATE_FORMAT(fecha, '%Y-%m') AS mes, COUNT(*) AS cantidad
    FROM registro_actividades
    WHERE accion = 'Inicio de sesión' AND id_usuario = ? AND fecha BETWEEN ? AND ?
    GROUP BY mes
    ORDER BY mes DESC
";

$stmt_estadisticas = $conexion->prepare($sql_estadisticas);
$stmt_estadisticas->bind_param("iss", $usuario, $fechaDesde, $fechaHasta);
...
// Recoger los resultados en arrays
while ($row = $result_estadisticas->fetch_assoc()) {
    $meses[] = $row['mes'];
    $cantidades[] = $row['cantidad'];
}

// Asegúrate de manejar el caso donde no hay resultados
if (empty($meses)) {
    echo "No se encontraron inicios de sesión en el rango de fechas especificado.";
} else {
    // Aquí puedes procesar los arrays $meses y $cantidades según sea necesario
}
...
<div class="bg-white rounded-lg shadow-md p-6 mb-6 flex items-center justify-between">
    <div class="flex items-center space-x-4">
        <?php 
        $imagenUrl = htmlspecialchars($perfil['url']);
        ?>
        <img src="<?php echo $imagenUrl; ?>" alt="<?php htmlspecialchars($perfil['nombre_imagen']);?>" class="w-24 h-24 rounded-full">
        <div>
            <h2 class="text-xl font-bold">Nombre y Apellido: <?php echo htmlspecialchars($perfil['primer_nombre']); ?> <?php echo htmlspecialchars($perfil['segundo_nombre']); ?> <?php echo htmlspecialchars($perfil['primer_apellido']); ?> <?php echo htmlspecialchars($perfil['segundo_apellido']); ?></h2>
            <p class="text-gray-500">Correo: <?php echo htmlspecialchars($perfil['correo_electronico']); ?></p>
            <p class="text-gray-500">Teléfono: <?php echo htmlspecialchars($perfil['telefono']); ?></p>
        </div>
    </div>
...
<style>
    .pelotas {
        ...
        margin-left: 20px; /* Espacio entre los datos y las pelotas */
    ...
    .pelota {
        width: 100px;
        height: 100px;
        background-image: url('servidor_img/css/pelota.jpeg'); /* Reemplaza con la ruta de tu imagen de pelota */
    ...
    @keyframes rebote {
        ...
        50% {
            transform: translateY(-30px); /* Altura del rebote */
        }
    }
</style>
...
<div class="p-6">
    <form method="POST" class="flex space-x-4 mb-6">
        ...
        <input type="date" name="fechaDesde" value="<?php echo $fechaDesde; ?>" class="border rounded p-2">
        <label for="hasta">Hasta</label>
        <input type="date" name="fechaHasta" value="<?php echo $fechaHasta; ?>" class="border rounded p-2">
...
<div class="mt-6 bg-white rounded-lg shadow-md p-6">
    <h2 class="text-lg font-bold mb-4">Estadísticas de Inicios de Sesión por Mes</h2>
    <canvas id="userStatsChart" class="w-full h-64"></canvas>
</div>

<!-- Librerías JS -->
<script>
    // Gráfico de Inicios de Sesión por Mes
    const ctx = document.getElementById('userStatsChart').getContext('2d');
    const userStatsChart = new Chart(ctx, {
        type: 'bar', // Gráfica de barras
        data: {
            labels: <?php echo json_encode($meses); ?>, // Meses de inicio de sesión
            datasets: [{
                label: 'Inicios de Sesión',
                data: <?php echo json_encode($cantidades); ?>, // Cantidad de inicios de sesión
                backgroundColor: '#10B981', // Color de las barras
                borderColor: '#059669', // Color del borde
                borderWidth: 1
            }]
        },
        options: {
