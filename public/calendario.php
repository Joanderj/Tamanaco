...
<div class="container mx-auto px-4 py-6">
  ...
  <main class="p-6">
    <div class="mb-6">
      <p class="text-gray-700 font-semibold text-xl text-center">Calendario de Planes</p>
    </div>
    <div id="calendar" class="p-4 bg-white shadow border border-gray-200 rounded-lg overflow-auto max-h-[600px]"></div>
  </main>
<script>
document.addEventListener('DOMContentLoaded', function () {
    ...
    var calendar = new FullCalendar.Calendar(calendarEl, {
        ...
        eventClick: function(info) {
            ...
            document.getElementById('modal-maquina').textContent = info.event.extendedProps.maquina || 'Sin máquina';
            document.getElementById('modal-descripcion').textContent = info.event.extendedProps.descripcion || 'Sin descripción';
            document.getElementById('modal-fechas').textContent = "Desde " + info.event.start.toLocaleString() + " hasta " + info.event.end.toLocaleString();
            document.getElementById('modal-btn').href = 'formulario_guardar_actividad.php?id=' + info.event.id;

            document.getElementById('tarea-modal').classList.remove('hidden');
        },
        editable: false,
        eventDisplay: 'block'
    });
...
function cerrarModal() {
    document.getElementById('tarea-modal').classList.add('hidden');
}
</script>
<!-- Modal -->
<div id="tarea-modal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 hidden">
  <div class="bg-white rounded-xl shadow-xl max-w-md w-full p-6 relative">
    <button onclick="cerrarModal()" class="absolute top-2 right-2 text-gray-500 hover:text-red-500 text-xl">&times;</button>
    <h2 id="modal-title" class="text-2xl font-bold mb-2 text-indigo-700">Título de la Tarea</h2>
    <p class="text-sm text-gray-500 mb-1">Máquina: <span id="modal-maquina" class="font-medium text-gray-800"></span></p>
    <p class="text-sm text-gray-500 mb-1">Descripción:</p>
    <p id="modal-descripcion" class="text-gray-700 text-sm mb-3"></p>
    <p id="modal-fechas" class="text-sm text-gray-500 mb-4"></p>
    <a id="modal-btn" href="#" class="block w-full text-center bg-blue-600 hover:bg-blue-700 text-white py-2 rounded-lg font-semibold">Ir a la tarea</a>
  </div>
</div>
