...
?>
<form action="modificar_producto.php" method="POST" enctype="multipart/form-data" class="space-y-6">
    <!-- ID oculto -->
 ...
 <div class="text-center mb-6">
    <label for="imagen" class="relative w-64 h-64 mx-auto border-2 border-dashed border-blue-500 rounded-lg flex justify-center items-center cursor-pointer group overflow-hidden">
        ...
        <img id="imagen-preview"
            src="<?= !empty($producto['url']) ? $producto['url'] : '' ?>"
            alt="Imagen actual"
            class="absolute inset-0 w-full h-full object-cover rounded-lg <?= !empty($producto['url']) ? '' : 'hidden' ?>" />
...
<script>
    ...
    <!-- Nombre -->
    <div>
        <label class="block font-semibold">Nombre del Producto <span class="text-red-600">*</span></label>
        <input type="text" name="nombre_producto" required value="<?= $producto['nombre_producto'] ?>" class="w-full border border-gray-300 rounded-lg p-2">
    </div>

    <!-- Marca -->
    <div>
        <label class="block font-semibold">Marca <span class="text-red-600">*</span></label>
          <div class="flex items-center gap-4">
        <select name="id_marca" required class="w-full border border-gray-300 rounded-lg p-2">
            <?php
            $marcas = $conexion->query("SELECT id_marca, nombre_marca FROM marca");
            while ($fila = $marcas->fetch_assoc()) {
                $selected = $producto['id_marca'] == $fila['id_marca'] ? 'selected' : '';
    ...
    <div>
        <label class="block font-semibold">Modelo</label>
          <div class="flex items-center gap-4">
        <select name="id_modelo" class="w-full border border-gray-300 rounded-lg p-2">
            <?php
            $modelos = $conexion->query("SELECT id_modelo, nombre_modelo FROM modelo");
            while ($fila = $modelos->fetch_assoc()) {
                $selected = $producto['id_modelo'] == $fila['id_modelo'] ? 'selected' : '';
    ...
    <!-- Tipo -->
    <div>
        <label class="block font-semibold">Tipo <span class="text-red-600">*</span></label>
        <div class="flex items-center gap-4">
        <select name="id_tipo" id="tipo" required class="w-full border border-gray-300 rounded-lg p-2" onchange="cargarClasificaciones()">
            <?php
            $tipos = $conexion->query("SELECT id_tipo, nombre_tipo FROM tipo WHERE id_status = 1");
            while ($fila = $tipos->fetch_assoc()) {
                $selected = $producto['id_tipo'] == $fila['id_tipo'] ? 'selected' : '';
...
<div class="flex items-center w-full border border-gray-300 rounded-lg overflow-hidden shadow-md bg-white p-3">
    ...
    <div class="relative">
        <select id="clasificacion" name="id_clasificacion"
            ...
            <option disabled>Seleccione una clasificación</option>
                <?php
                $clasificaciones = $conexion->query("SELECT c.id_clasificacion, c.nombre_clasificacion FROM tipo_clasificacion tc JOIN clasificacion c ON tc.id_clasificacion = c.id_clasificacion WHERE tc.id_tipo = {$producto['id_tipo']} AND tc.id_status = 1");
                while ($fila = $clasificaciones->fetch_assoc()) {
                    $selected = $producto['id_clasificacion'] == $fila['id_clasificacion'] ? 'selected' : '';
                    echo "<option value='{$fila['id_clasificacion']}' $selected>{$fila['nombre_clasificacion']}</option>";
                }
                ?>
        </select>
    ...
    <div class="relative flex-1 ml-4">
        <input type="text" id="unidad_medida" name="unidad_medida" value="<?= $producto['unidad_medida'] ?>"
    ...
    <div class="flex justify-between mt-6">
        ...
        <a href="producto.php" class="bg-gray-300 text-black px-6 py-2 rounded-lg hover:bg-gray-400">
            Cancelar
        </a>
    </div>
</form>
...
<script>
    document.getElementById('marca').addEventListener('change', function() {
        const marcaId = this.value; // Obtener el ID de la marca seleccionada
        const modeloSelect = document.getElementById('modelo'); // Select del modelo

        // Limpiar el contenido del select de modelos
        modeloSelect.innerHTML = '<option value="">Seleccionar</option>';

        if (marcaId) {
            // Hacer la llamada AJAX para obtener los modelos relacionados
            fetch(`obtener_modelos_producto.php?marca=${marcaId}`)
                .then(response => response.json())
                .then(data => {
                    data.forEach(modelo => {
