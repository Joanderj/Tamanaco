...
// Clasificación
$orderBy = 'id_inventario_producto';
if (isset($_GET['sort'])) {
    switch ($_GET['sort']) {
        case 'fecha_asc':
...
if (!empty($nombre)) {
    $conditions .= " AND p.nombre_producto LIKE ?";
    $params[] = '%' . $nombre . '%';
}
...
// Calcular si hay producto con stock bajo o valores nulos
$hayStockBajo = false;
$cantidadStockBajo = 0;
$cantidadNulos = 0;
...
<div class="space-x-2">
    <button onclick="location.href='entrega_producto.php'" class="bg-blue-500 hover:bg-blue-600 text-white px-3 py-2 rounded-md shadow text-sm">
...
<div id="filterForm" class="hidden mt-2 bg-white border border-gray-300 rounded shadow-lg p-4">
   ...
   <div id="savedFilters" class="mt-4">
    ...
    <form id="filterOptionsForm">
        <!-- Campo: Nombre -->
        <div class="mb-4">
            <label for="nombre" class="block text-gray-700 mb-2">Nombre del producto</label>
            <input 
              type="text" 
              id="nombre"
              name="nombre" 
...
<div id="lowStockAlert" class="<?= $hayStockBajo || $cantidadNulos > 0 ? 'block' : 'hidden' ?> fixed bottom-4 right-4 bg-red-100 border-l-4 border-red-500 text-red-700 p-4 rounded-lg shadow-lg z-50" style="max-width: 400px;">
    <div class="flex items-start">
        ...
        <div class="ml-3">
            ...
            <p class="text-sm">Tienes <?= $cantidadStockBajo ?> producto(s) con stock por debajo del mínimo y <?= $cantidadNulos ?> producto(s) con valores nulos en stock mínimo o máximo.</p>
            <div class="mt-2">
...
<div id="lowReorderPointAlert" class="<?= $hayPuntoReordenBajo ? 'block' : 'hidden' ?> fixed bottom-16 left-4 bg-yellow-100 border-l-4 border-yellow-500 text-yellow-700 p-4 rounded-lg shadow-lg z-50" style="max-width: 400px; cursor: move;" onmousedown="dragStart(event, 'lowReorderPointAlert')">
    <div class="flex items-start">
        ...
        <div class="ml-3">
            <p class="font-bold">¡Atención!</p>
            <p class="text-sm">Tienes <?= $cantidadPuntoReordenBajo ?> producto(s) con punto de reorden menor que la cantidad actual.</p>
            <div class="mt-2">
...
function showLowReorderPointItems() {
    ...
    showFilterAppliedMessage('Mostrando solo producto con punto de reorden bajo');
}
...
<div class="border border-gray-300 rounded-lg shadow-md p-4">
    ...
    <div class="flex justify-between items-center mb-4 border-b border-gray-300 pb-2">
        <h1 class="text-xl font-semibold text-gray-700">Lista de producto en el Inventario</h1>
        <div class="relative">
            <select id="clasificacion" 
                ...
                <option value="nombre_asc">Por producto (A-Z)</option>
                <option value="nombre_desc">Por producto (Z-A)</option>
...
<div class="overflow-x-auto">
    ...
    <tr class="hover:bg-gray-100 <?= ($item['cantidad'] < $item['stock_minimo'] || is_null($item['stock_minimo']) || is_null($item['stock_maximo'])) ? 'bg-red-50' : '' ?>">
        <td class="px-6 py-4"><?= htmlspecialchars($item['id_inventario_producto']) ?></td>
        <td class="px-6 py-4"><?= htmlspecialchars($item['nombre_producto']) ?></td>
        <td class="px-6 py-4 whitespace-nowrap"><?= $item['cantidad'] ?></td>
        <td class="px-6 py-4 whitespace-nowrap editable-cell" 
            data-id="<?= $item['id_inventario_producto'] ?>"
        ...
        <td class="px-6 py-4 whitespace-nowrap editable-cell" 
            data-id2="<?= $item['id_inventario_producto'] ?>"
...
<td class="px-6 py-4 flex space-x-2">
    <button onclick="verproducto(<?= $item['id_producto'] ?>)" 
...
document.getElementById('searchInput').addEventListener('keyup', filterTable);
  ...
  <script>
   // Mostrar solo producto con stock bajo o valores nulos
function showLowStockItems() {
    const rows = document.querySelectorAll('#dataTable tr');
    rows.forEach(row => {
        const cantidadCell = row.querySelector('td:nth-child(3)');
    ...
    // Mostrar mensaje de filtro aplicado
    showFilterAppliedMessage('Mostrando solo producto con stock bajo o valores nulos');
}

// Mostrar todos los producto nuevamente
function showAllItems() {
    const rows = document.querySelectorAll('#dataTable tr');
    rows.forEach(row => {
        row.style.display = '';
