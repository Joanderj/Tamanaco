...
while ($submenu = $result_submenus_tipo_1->fetch_assoc()) {
  ...
  // Variables para paginación
    ...
    $nombrePlan = isset($_GET['nombrePlan']) ? trim($_GET['nombrePlan']) : '';
    $idMaquina = isset($_GET['idMaquina']) ? (int)$_GET['idMaquina'] : '';
    $status = isset($_GET['status']) ? (int)$_GET['status'] : '';
    $fechaInicio = isset($_GET['fechaInicio']) ? $_GET['fechaInicio'] : '';
    $fechaFinal = isset($_GET['fechaFinal']) ? $_GET['fechaFinal'] : '';

    // Clasificación
    $orderBy = 'p.id_plan ASC'; // Orden por defecto
    if (isset($_GET['sort'])) {
        switch ($_GET['sort']) {
            case 'fecha_asc':
            ...
            case 'fecha_desc':
                ...
                break;
            case 'nombre_asc':
                $orderBy = 'p.nombre_plan ASC';
                break;
            case 'nombre_desc':
                $orderBy = 'p.nombre_plan DESC';
                break;
            case 'numero_asc':
                $orderBy = 'p.id_plan ASC';
                break;
            case 'numero_desc':
                $orderBy = 'p.id_plan DESC';
                break;
            default:
                $orderBy = 'p.id_plan ASC';
        }
    ...
    FROM planes p
    LEFT JOIN tareas t ON t.id_plan = p.id_plan AND t.status_id = 1
    ...
    // Filtros
    if (!empty($nombrePlan)) {
        $totalQuery .= " AND p.nombre_plan LIKE ?";
        $params[] = '%' . $nombrePlan . '%';
        $types .= "s";
    }
    if (!empty($idMaquina)) {
        $totalQuery .= " AND m.id_maquina = ?";
        $params[] = $idMaquina;
        $types .= "i";
    }
    if (!empty($status)) {
    ...
    $dataTypes = "";
    if (!empty($nombrePlan)) {
        $query .= " AND p.nombre_plan LIKE ?";
        $dataParams[] = '%' . $nombrePlan . '%';
        $dataTypes .= "s";
    }
    if (!empty($idMaquina)) {
        $query .= " AND m.id_maquina = ?";
        $dataParams[] = $idMaquina;
        $dataTypes .= "i";
    }
    if (!empty($status)) {
    ...
    if (!empty($fechaInicio)) {
        ...
        $dataParams[] = $fechaInicio;
        $dataTypes .= "s";
    }
    if (!empty($fechaFinal)) {
        $query .= " AND t.fecha_inicio <= ?";
        $dataParams[] = $fechaFinal;
        $dataTypes .= "s";
    }
...
<div class="flex justify-start items-center space-x-4 mb-4">
 ...
 <div class="container mx-auto px-4 py-6">
    ...
    <div class="flex justify-between items-center mb-4">
      <button onclick="window.location.href='mantenimiento.php'" class="bg-gray-200 text-gray-700 px-4 py-2 rounded hover:bg-gray-300">
        <i class="fas fa-arrow-left"></i> Regresar
      </button>
      <div class="space-x-2">
        <button  onclick="window.location.href='formulario_guardar_plan.php'" class="bg-indigo-500 text-white px-4 py-2 rounded hover:bg-indigo-600">
...
<div class="border border-gray-300 rounded-lg shadow-md p-4">
    ...
    <div class="flex justify-between items-center mb-4 border-b border-gray-300 pb-2">
        <h1 class="text-xl font-semibold text-gray-700">Tareas</h1>
        <div class="relative">
    ...
    ?>
      ...
      <table class="w-full table-auto border border-gray-300 shadow rounded-lg overflow-hidden">
    <thead class="bg-indigo-600 text-white">
        <th class="px-4 py-3 text-left">
            <i class="fas fa-hashtag text-indigo-300 mr-1"></i> 
        </th>
        <th class="px-4 py-3 text-left">
            <i class="fas fa-tasks text-indigo-300 mr-1"></i> Plan de mantenimiento
        </th>
        <th class="px-4 py-3 text-left">
            <i class="fas fa-industry text-indigo-300 mr-1"></i> Máquina
        </th>
        <th class="px-4 py-3 text-left">
            <i class="fas fa-bolt text-yellow-400 mr-1"></i> Trigger
        </th>
        <th class="px-4 py-3 text-left">
            <i class="fas fa-calendar-alt text-green-400 mr-1"></i> Próxima tarea
        </th>
        <th class="px-4 py-3 text-left">
            <i class="fas fa-cogs text-gray-400 mr-1"></i> Acciones
        </th>
        </tr>
    </thead>
    <tbody class="bg-white divide-y divide-gray-200">
        ...
        <tr class="hover:bg-gray-50">
            ...
            <td class="px-4 py-4 font-medium text-indigo-600">
                <?= htmlspecialchars($plan['id_plan']) ?>
            </td>

            <!-- Frecuencia y Nombre del Plan -->
            <td class="px-4 py-4 text-gray-700">
                <div class="flex items-center space-x-2">
                    <i class="fas fa-sync-alt text-indigo-500"></i>
                    <span>
                        Cada <?= htmlspecialchars($plan['frecuencia']) ?> <?= htmlspecialchars($plan['tipo_frecuencia']) ?>
                    </span>
                </div>
                <div class="text-xs text-gray-500 ml-6">Duración: <?= htmlspecialchars($plan['duracion']) ?> días</div>
                <div class="mt-2 font-semibold text-indigo-700"><?= htmlspecialchars($plan['nombre_plan']) ?></div>
            </td>

            <!-- Máquina con imagen -->
            <td class="px-4 py-4 text-gray-700">
                <div class="flex items-center space-x-4">
                    <?php if (!empty($plan['imagen_maquina'])): ?>
                        <img src="<?= htmlspecialchars($plan['imagen_maquina']) ?>" alt="img" class="w-14 h-14 rounded object-cover border">
                    <?php else: ?>
                        <div class="w-14 h-14 bg-gray-300 rounded flex items-center justify-center text-white text-xs">Sin img</div>
                    <?php endif; ?>
                    <div>
                        <div class="font-semibold"><?= htmlspecialchars($plan['nombre_maquina']) ?></div>
                        <div class="text-sm text-gray-500">
                            <?= htmlspecialchars($plan['nombre_marca']) ?> / <?= htmlspecialchars($plan['nombre_modelo']) ?>
                        </div>
                        <div class="text-xs text-gray-400">Código: <?= htmlspecialchars($plan['codigo_maquina']) ?></div>
                    </div>
                </div>
            </td>
            ...
            <td class="px-4 py-4 text-gray-700">
                <div class="flex items-center space-x-2">
                    ...
                    <span><?= ucfirst(htmlspecialchars($plan['trigger_opcion'])) ?></span>
                </div>
            </td>

            <!-- Próxima Tarea -->
            <td class="px-4 py-4 text-gray-700">
                <?php if (!empty($plan['id_tarea'])): ?>
                    <a href="formulario_guardar_actividad.php?id=<?= htmlspecialchars($plan['id_tarea']) ?>"
                       class="inline-block bg-green-100 text-green-800 px-3 py-1 rounded-full text-sm font-medium hover:bg-green-200 transition">
                       <i class="fas fa-calendar-alt mr-1"></i> <?= date('j \d\e F \d\e Y', strtotime($plan['fecha_inicio'])) ?>
                    </a>
                <?php else: ?>
                    <span class="text-sm italic text-gray-400">Sin tarea asignada</span>
                <?php endif; ?>
            </td>
            <td class="px-4 py-4">
                <div class="flex items-center justify-center space-x-6">
                    <!-- Ver (Ojo) -->
                    <a href="ver_plan.php?id=<?= htmlspecialchars($plan['id_plan']) ?>"
                       class="text-blue-600 hover:text-blue-800 text-2xl transition-transform transform hover:scale-125"
                       title="Ver plan">
                        <i class="fas fa-eye"></i>
                    </a>
                    <!-- Imprimir -->
                    <a href="imprimir_plan.php?id=<?= htmlspecialchars($plan['id_plan']) ?>"
                       class="text-green-600 hover:text-green-800 text-2xl transition-transform transform hover:scale-125"
                       title="Imprimir plan" target="_blank">
                        <i class="fas fa-print"></i>
                    </a>
                    <!-- Copiar (Replicar) -->
                    <a href="copiar_plan.php?id=<?= htmlspecialchars($plan['id_plan']) ?>"
                       class="text-gray-600 hover:text-gray-800 text-2xl transition-transform transform hover:scale-125"
                       title="Copiar plan">
