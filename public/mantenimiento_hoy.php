...
$fecha_fin_semana = $domingo->format('Y-m-d');

// Total de tareas esta semana
$countQuery = "
    SELECT COUNT(DISTINCT t.id_tarea) AS total
    FROM tareas t
    LEFT JOIN maquina_unica mu ON t.id_maquina_unica = mu.id_maquina_unica
    WHERE t.fecha_inicio BETWEEN :inicio AND :fin
";
$stmt = $conn->prepare($countQuery);
$stmt->bindValue(':inicio', $fecha_inicio_semana);
$stmt->bindValue(':fin', $fecha_fin_semana);
$stmt->execute();
$totalItems = $stmt->fetchColumn();
$totalPages = ceil($totalItems / $itemsPerPage);
...
$query = "
    SELECT 
        ...
        ma.nombre_marca, mo.nombre_modelo,
        s.nombre_sede AS ubicacion_sede,
        e.id_status, e.nombre_status AS estado_tarea,
        GROUP_CONCAT(CONCAT(p.primer_nombre, ' ', p.primer_apellido) SEPARATOR ', ') AS responsables_asignados
    FROM tareas t
    LEFT JOIN tipo_mantenimiento tm ON t.tipo_mantenimiento_id = tm.id_tipo
    LEFT JOIN maquina_unica mu ON t.id_maquina_unica = mu.id_maquina_unica
...
<table class="table-auto w-full border border-gray-300 shadow-lg rounded-lg">
    ...
    <?php foreach ($tareas as $fila): 
        ...
        $color = $estado === 'Pendiente' ? 'bg-red-500' : ($estado === 'Planificado' ? 'bg-yellow-500' : 'bg-green-500');
    ?>
        <tr class="hover:bg-gray-100">
            ...
            <td class="px-6 py-4">
                <div class="flex flex-col items-start">
                    <span class="text-lg font-bold pb-1"><?= htmlspecialchars($fila['titulo_tarea']) ?></span>
                    <span class="text-sm"><i class="fas fa-clock text-blue-600 mr-2"></i><?= htmlspecialchars($fila['tiempo_programado']) ?></span>
                    <span class="text-sm"><i class="fas fa-calendar text-blue-600 mr-2"></i><?= htmlspecialchars($fila['fecha_inicio']) ?></span>
                    <span class="text-sm"><i class="fas fa-tag text-blue-600 mr-2"></i><?= htmlspecialchars($fila['tipo_mantenimiento']) ?></span>
                </div>
            </td>
            <td class="px-6 py-4 text-center flex flex-col items-center">
                <?php if (!empty($fila['url_maquina'])): ?>
                    <img src="<?= htmlspecialchars($fila['url_maquina']) ?>" alt="imagen" class="w-20 h-20 object-cover rounded mb-2">
                <?php else: ?>
                    <span class="text-gray-500">Sin Imagen</span>
                <?php endif; ?>
                <div class="text-sm text-gray-700"><?= htmlspecialchars($fila['nombre_maquina']) ?></div>
                <div class="text-xs text-gray-500">
                    <?= htmlspecialchars($fila['nombre_marca']) ?> | <?= htmlspecialchars($fila['nombre_modelo']) ?> | <?= htmlspecialchars($fila['codigo_maquina']) ?>
                </div>
            </td>
            <td class="px-6 py-4  items-center text-sm">
                <i class="fas fa-map-marker-alt text-green-600 mr-2"></i><?= htmlspecialchars($fila['ubicacion_sede']) ?>
            </td>
            <td class="px-6 py-4  items-center text-sm">
                <i class="fas fa-user-friends text-purple-600 mr-2"></i><?= htmlspecialchars($fila['responsables_asignados']) ?>
            </td>
            ...
            <td class="px-6 py-4 text-center flex flex-col gap-2">
                <button 
                    ...
                    onclick="window.location.href='formulario_guardar_actividad.php?id=<?= htmlspecialchars($fila['id_tarea']) ?>'">
                    <i class="fas fa-eye"></i> Ver
                </button>
                <?php if ($fila['id_status'] == 1): ?>
                    <button 
                        class="bg-yellow-500 text-white px-4 py-2 rounded hover:bg-yellow-600 iniciar-btn"
                ...
                <?php elseif ($fila['id_status'] == 7): ?>
                    <button 
                        class="bg-purple-600 text-white px-4 py-2 rounded hover:bg-purple-700"
                        style="background-color:#7c3aed;" 
                        onclick="window.location.href='imprimir_mantenimiento_realizado.php?id=<?= htmlspecialchars($fila['id_tarea']) ?>'">
...
<div class="mt-4 flex items-center justify-between">
    ...
    <div class="flex space-x-2">
        <a href="?page=<?= max(1, $currentPage - 1) ?>&itemsPerPage=<?= $itemsPerPage ?>" class="border px-4 py-2 rounded hover:bg-gray-200">Anterior</a>
        <?php
        for ($i = 1; $i <= $totalPages; $i++) {
            if ($i == $currentPage) {
                echo "<span class='bg-indigo-500 text-white px-4 py-2 rounded'>$i</span>";
            } elseif ($i <= 3 || $i > $totalPages - 3 || abs($currentPage - $i) < 2) {
                echo "<a href='?page=$i&itemsPerPage=$itemsPerPage' class='border px-4 py-2 rounded hover:bg-gray-200'>$i</a>";
            } elseif ($i == 4 && $currentPage > 5) {
                echo "<span class='text-gray-500'>...</span>";
            }
        }
        ?>
        <a href="?page=<?= min($totalPages, $currentPage + 1) ?>&itemsPerPage=<?= $itemsPerPage ?>" class="border px-4 py-2 rounded hover:bg-gray-200">Siguiente</a>
    </div>
    <div class="flex items-center space-x-2">
        <label for="itemsPerPage" class="text-gray-600">Mostrar:</label>
        <select id="itemsPerPage" onchange="window.location.href='?itemsPerPage=' + this.value + '&page=1'" class="border px-3 py-1 rounded">
...
document.addEventListener("DOMContentLoaded", () => {
    document.querySelectorAll(".iniciar-btn").forEach(button => {
        button.addEventListener("click", () => {
            ...
            .then(data => {
                if (data.trim() === "ok") {
                    mostrarMensajeExito("Has iniciado el mantenimiento correctamente.");
                    setTimeout(() => location.reload(), 3000);
                } else {
                    mostrarMensajeError("No se pudo iniciar: " + data);
                }
            })
            .catch(error => {
