...
$sql_solicitudes = "
    SELECT so.id_tipo_solicitud, so.fecha_solicitud,ts.nombre_tipo, ma.*
    FROM solicitudes so
    JOIN movimiento_maquina ma ON ma.id_solicitud = so.id_solicitud
   ...
   <section class="p-4">
    ...
    <form method="GET" class="mb-4">
        ...
        <select name="id_status" id="id_status" class="border rounded p-2">
            <option value="1" <?php echo $id_status == 1 ? 'selected' : ''; ?>>Entregando</option>
            <option value="2" <?php echo $id_status == 2 ? 'selected' : ''; ?>>Entregado</option>
        </select>
    ...
    <table class="min-w-full bg-white border border-gray-300">
        <thead>
            <tr class="bg-gray-200">
                <th class="py-2 px-4 border">ID Tipo Solicitud</th>
                <th class="py-2 px-4 border">Fecha de Entrega</th>
        ...
        <tbody>
            <?php foreach ($solicitudes as $solicitud): ?>
                <tr>
                    ...
                    <td class="py-2 px-4 border"><?php echo htmlspecialchars($solicitud['fecha_solicitud']); ?></td>
                    ...
                    <td class="py-2 px-4 border">
                       <button class="bg-green-500 text-white p-2 rounded" 
                            onclick="verDetalles(<?php echo htmlspecialchars($solicitud['id_maquina']); ?>, <?php echo htmlspecialchars($solicitud['id_solicitud']); ?>)">
                        Ver Entrega
                    </button>
                        
                        <!-- Formulario para actualizar inventario -->
                         <?php if (isset($id_status) && $id_status == 1): // Solo mostrar si el estado es "Entregando" ?>
                        <form action="actualizar_inventario_maquina_entrega.php" method="POST" class="inline-block">
                            <input type="hidden" name="maquina" value="<?php echo htmlspecialchars($solicitud['id_maquina']); ?>">
                            <input type="hidden" name="solicitud" value="<?php echo htmlspecialchars($solicitud['id_solicitud']); ?>">
                            <input type="hidden" name="sede" value="<?php echo htmlspecialchars($solicitud['id_almacen_destino']); ?>">
                            <button type="submit" class="ml-2 bg-green-500 text-white p-2 rounded">Entregado</button>
                        </form>
                        <?php endif; ?>
                    </td>
                </tr>
...
<!-- Botón Volver -->
<div class="mt-4">
    <a href="inventario_maquina.php" class="bg-gray-500 text-white p-2 rounded">Volver</a>
</div>
...
<script>
function verDetalles(id_maquina, id_solicitud) {
    // Realizar una petición AJAX para obtener los datos
    fetch(`ver_detalles_entrega_maquina.php?id_solicitud=${id_solicitud}&id_maquina=${id_maquina}`)
        ...
        .then(data => {
            ...
            if (data.length > 0) {
                const contenido = `
                    ...
                    <p>Nombre de herramienta: ${data[0].nombre_repuesto}</p>
