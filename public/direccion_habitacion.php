...
$sql = "SELECT * FROM usuarios u JOIN personas p ON u.id_persona = p.id_persona WHERE u.usuario = ?";
...
<div class="flex flex-col items-center mb-6">
  ...
  <h2 class="text-3xl font-extrabold text-gray-800">Dirección de Habitación</h2>
  <p class="text-gray-600 mt-2 text-center">Gestiona y actualiza la información de tu dirección de habitación.</p>
</div>
...
<form action="editar_direccion_habitacion.php" method="POST">
    <!-- País y Estado -->
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        <div class="relative">
            <label for="pais" class="block font-medium text-gray-600">País:</label>
            <select id="pais" name='pais' class="w-full p-3 border border-gray-300 rounded-lg bg-gray-100 focus:outline-none" disabled>
                ...
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        ...
                        echo "<option value='$id' " . ($perfil['pais_id'] == $id ? 'selected' : '') . ">$nombre</option>";
                    }
                } else {
            ...
            <button type="button" class="absolute top-2 right-2 text-gray-500 hover:text-blue-500" onclick="habilitarInput('pais')">
        ...
        <div class="relative">
                ...
                if (isset($perfil['pais_id'])) {
                    ...
                    if ($result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                            ...
                            echo "<option value='$idestado' " . ($perfil['estado_id'] == $idestado ? 'selected' : '') . ">$nombreestado</option>";
                        }
                    } else {
            ...
            <button type="button" class="absolute top-2 right-2 text-gray-500 hover:text-blue-500" onclick="habilitarInput('estado')">
                <i class="fas fa-pencil-alt"></i>
            </button>
        </div>
    </div>
    ...
    <div class="relative mb-6">
        <label for="direccion" class="block font-medium text-gray-600">Direccion:</label>
        <textarea id="direccion" rows="4" name='direccion' placeholder="Ejemplo: Detallar referencias, puntos cercanos o indicaciones adicionales..." class="w-full p-3 border border-gray-300 rounded-lg bg-gray-100 focus:outline-none" oninput="convertirMayusculas(this);" disabled><?php echo htmlspecialchars($perfil['direccion']); ?></textarea>
        <button type="button" class="absolute top-2 right-2 text-gray-500 hover:text-blue-500" onclick="habilitarInput('direccion')">
            <i class="fas fa-pencil-alt"></i>
        </button>
    </div>

    <input type="hidden" id="persona" name='id_persona' value='<?php echo htmlspecialchars($perfil['id_persona']); ?>'/>
    <input type="hidden" id="cedula" name='cedula' value='<?php echo htmlspecialchars($perfil['cedula']); ?>'/>
...
function habilitarInput(id) {
    ...
    inputElement.classList.remove('bg-gray-100'); // Eliminar clase de fondo gris
    inputElement.classList.add('bg-white'); // Agregar clase de fondo blanco
}
</script>
