...
<div class="bg-blue-500 text-white w-16 h-16 rounded-full flex items-center justify-center shadow-lg mb-4">
    ...
    <h2 class="text-3xl font-extrabold text-gray-800">Blog de la Empresa</h2>
    <p class="text-gray-600 mt-2 text-center">Explora y gestiona las publicaciones del blog de la empresa.</p>
</div>
<script src="https://cdn.jsdelivr.net/npm/animejs@3.2.1/lib/anime.min.js"></script>
<!-- Contenedor principal -->
<div class="container mx-auto max-w-4xl p-8 bg-gradient-to-br from-white to-gray-100 rounded-3xl shadow-2xl relative overflow-hidden group">

  <!-- Botón mostrar formulario -->
  <button id="openPostForm" class="w-full py-4 px-6 bg-gradient-to-r from-blue-600 to-indigo-600 text-white text-lg font-semibold rounded-xl shadow-xl hover:shadow-2xl hover:scale-105 transition-transform duration-300 flex items-center justify-center gap-2">
    <i class="fas fa-edit text-xl"></i> Crear una nueva publicación
  </button>

  <!-- FORMULARIO -->
   <form action="guardar_publicacion.php" method="POST" enctype="multipart/form-data" class="">
  <div id="postForm" class="hidden opacity-0 translate-y-6 transition-all duration-700 bg-white/30 backdrop-blur-lg rounded-2xl p-8 md:p-10 shadow-2xl mt-8 border border-white/40 space-y-8 relative">

    <!-- Encabezado animado -->
    <div class="text-center">
      <h2 class="text-3xl font-bold text-blue-700 tracking-wide mb-2">Nueva Publicación</h2>
      <p class="text-gray-700">Haz que tu voz se escuche. Inspira. Conecta. Crea impacto.</p>
    </div>

   <!-- FOTO DE PORTADA MEJORADA -->
<div class="relative">
  <label class="block text-gray-800 text-lg font-semibold mb-4">
    <i class="fas fa-image text-blue-600 mr-2"></i> Foto de portada
  </label>

  <!-- Zona de carga personalizada -->
  <div id="dropZone" class="flex flex-col items-center justify-center border-4 border-dashed border-blue-400 rounded-2xl p-8 bg-white/50 hover:bg-white/70 transition-all duration-300 cursor-pointer shadow-inner relative group">
    <i class="fas fa-cloud-upload-alt text-5xl text-blue-500 group-hover:scale-110 transition-transform duration-300"></i>
    <p class="mt-4 text-blue-800 font-semibold">Haz clic o arrastra una imagen aquí</p>
    <input name="imagen" id="fileInput" type="file" accept="image/*,video/*" class="absolute inset-0 opacity-0 cursor-pointer z-10" />
  </div>

  <!-- Preview de imagen -->
  <div id="imagePreview" class="mt-4 hidden">
    <p class="text-sm text-gray-600 mb-1">Vista previa:</p>
    <img id="previewImg" src="" alt="Vista previa" class="rounded-xl w-full h-60 object-cover shadow-md border border-gray-300" />
  </div>
</div>
<script>
...
<!-- Título mejorado -->
<div class="relative">
  <label class="block text-gray-800 text-xl font-bold mb-3 tracking-wide">
    <i class="fas fa-heading text-blue-600 mr-2"></i> Título del blog
  </label>
  <div class="relative">
    <input name="titulo" type="text" placeholder="Escribe un título épico aquí..." class="peer w-full bg-white/70 backdrop-blur-xl px-6 py-4 rounded-2xl border border-blue-300 shadow-xl focus:ring-4 focus:ring-blue-400 focus:outline-none text-gray-800 placeholder-gray-400 transition-all duration-300" />
    <i class="fas fa-pen-nib absolute top-1/2 right-5 text-blue-500 transform -translate-y-1/2 pointer-events-none"></i>
  </div>
  <p class="text-sm text-gray-500 mt-2">Haz que el título llame la atención del lector.</p>
</div>
...
<div class="relative mt-8">
  ...
  <div class="relative">
    <textarea name="descripcion" rows="5" placeholder="Escribe una descripción convincente..." class="peer w-full bg-white/70 backdrop-blur-xl px-6 py-4 rounded-2xl border border-blue-300 shadow-xl resize-none focus:ring-4 focus:ring-blue-400 focus:outline-none text-gray-800 placeholder-gray-400 transition-all duration-300"></textarea>
    <i class="fas fa-feather-alt absolute top-4 right-5 text-blue-500 pointer-events-none"></i>
  </div>
  <p class="text-sm text-gray-500 mt-2">Tu historia, experiencia o ideas pueden inspirar a otros.</p>
</div>
<div class="text-center pt-10 relative">
  <button id="publishPost" class="group relative inline-flex items-center justify-center gap-3 px-10 py-4 bg-gradient-to-br from-purple-600 via-indigo-600 to-blue-600 text-white text-lg font-bold rounded-full shadow-2xl hover:shadow-blue-600/50 transition-all duration-500 overflow-hidden">
    <span class="absolute inset-0 bg-white/10 blur-xl rounded-full group-hover:scale-125 transition-transform duration-700 ease-out"></span>
    <i class="fas fa-rocket animate-bounce-slow text-white text-xl z-10"></i>
    <span class="z-10">Publicar entrada</span>
  </button>
</div>
</form>
  </div>
...
<!-- Título con estilo elegante y animación -->
<h2 id="tituloPublicaciones" class="text-4xl md:text-5xl font-extrabold text-center text-indigo-700 tracking-wide drop-shadow-lg mb-10">
...
<script>
  ...
  .add({
    targets: '#postCard .glow-ring',
    opacity: [0, 1],
    scale: [0.5, 1],
    duration: 600
  }, '-=800'); // empieza antes para que solape
</script>
