...
<div class="container mx-auto px-4 py-6">
    <div class="max-w-5xl mx-auto bg-white rounded-lg shadow-lg p-6">
        <div class="flex flex-col items-center mb-6">
            ...
            <p class="text-gray-600 mt-2 text-center">Realiza movimientos de Traslado</p>
        </div>
...
?>
      ...
      <!-- Formulario Mejorado -->
<form id="formRegistrarmaquina" action="actualizar_maquina_traslado.php" method="POST" class="max-w-2xl mx-auto bg-white p-6 rounded-xl shadow-md">
    <h2 class="text-2xl font-semibold text-gray-800 mb-6 flex items-center">
    </h2>

    <!-- Selección de Máquina -->
    <div class="mb-5">
        <label for="maquina" class="block text-gray-700 font-medium mb-2">
            <i class="fas fa-box text-blue-500 mr-1"></i>Máquina<span class="text-red-600">*</span>
        </label>
        <select 
            id="maquina" 
            name="maquina"
    ...
    <!-- Selección de Sede -->
    <div class="mb-5">
        <label for="sede" class="block text-gray-700 font-medium mb-2">
            <i class="fas fa-map-marker-alt text-blue-500 mr-1"></i>Sede Salida<span class="text-red-600">*</span>
        </label>
        <select 
            id="sede" 
            name="sede"
        ...
        >
            ...
            $result = $conexion->query($query);
            while ($row = $result->fetch_assoc()) {
                $id = htmlspecialchars($row["id_sede"]);
        ...
        <!-- Advertencia de sedes iguales -->
<p id="errorSedes" class="text-red-600 font-medium mt-2 hidden">
    ⚠️ La sede de salida y la sede de entrada no pueden ser la misma.
</p>
    ...
    <!-- Código de Máquina -->
    <div class="mb-5">
        <label for="codigo_maquina" class="block text-gray-700 font-medium mb-2">
            <i class="fas fa-barcode text-blue-500 mr-1"></i>Código de Máquina
        </label>
        <select 
            id="codigo_maquina" 
            name="codigo_maquina"
     ...
     <!-- Selección de Sede -->
    <div class="mb-5">
        <label for="sede_entrada" class="block text-gray-700 font-medium mb-2">
            <i class="fas fa-map-marker-alt text-blue-500 mr-1"></i>Sede Entrada<span class="text-red-600">*</span>
        </label>
        <select 
            id="sede_entrada" 
            name="sede_entrada"
...
<p id="errorSedes" class="text-red-600 font-medium mt-2 hidden">
    ...
    <div>
            <label for="tiempo" class="block font-semibold">
                <i class="fas fa-sun text-blue-500 mr-2"></i>Día de Enntrega Estimada
            </label>
            <input type="date" name="tiempo" class="w-full border border-gray-300 rounded-lg p-2"
            min="<?php echo date('Y-m-d'); ?>"
                value="<?php echo date('Y-m-d'); ?>">
        </div>
    <div>
        <label for="descripcion" class="block font-semibold">
            <i class="fas fa-pencil-alt text-blue-500 mr-2"></i>Descripción del motivo
        </label>
    ...
    <div class="mb-6">
        <label class="block text-gray-700 font-medium mb-2">
            <i class="fas fa-cogs text-blue-500 mr-1"></i>Repuestos Asociados
        </label>
        <ul id="repuesto_info" class="list-disc list-inside text-gray-700 space-y-1 bg-gray-50 p-3 rounded-lg border border-gray-200">
    ...
    <div class="text-right">
        <button type="submit" id="guardar"
            class="bg-green-600 text-white px-6 py-2 rounded-lg hover:bg-green-700 transition duration-300 focus:outline-none focus:ring-2 focus:ring-green-300 disabled:bg-gray-400 disabled:text-gray-200 disabled:cursor-not-allowed">
...
<!-- Script Mejorado -->
<script>
// Llenar automáticamente códigos al cargar
...
// Actualiza códigos según selección de máquina y sede
function updateCodigo() {
    const maquina = document.getElementById("maquina").value;
    const sede = document.getElementById("sede").value;
    const selectCodigo = document.getElementById("codigo_maquina");
    selectCodigo.innerHTML = '<option value="">Cargando...</option>';

    if (maquina && sede) {
        fetch(`obtener_codigos_traslado.php?maquina=${maquina}&sede=${sede}`)
            .then(res => res.json())
            .then(data => {
                selectCodigo.innerHTML = '<option value="">Seleccione un código</option>';
                data.forEach(item => {
                    const option = document.createElement("option");
                    option.value = item.id_maquina_unica;
                    option.textContent = item.codigounico;
        ...
        selectCodigo.innerHTML = '<option value="">Seleccione primero máquina y sede</option>';
    }
}

// Mostrar repuestos al seleccionar código
document.getElementById("codigo_maquina").addEventListener("change", function () {
    const idMaquinaUnica = this.value;
    const repuestoList = document.getElementById("repuesto_info");
    repuestoList.innerHTML = '';

    if (idMaquinaUnica) {
        fetch(`obtener_repuestos.php?id_maquina_unica=${idMaquinaUnica}`)
            .then(res => res.json())
            .then(data => {
                if (data.length > 0) {
                    data.forEach(repuesto => {
                        const li = document.createElement("li");
                        li.textContent = repuesto.nombre_repuesto;
                        repuestoList.appendChild(li);
                    });
                } else {
                    repuestoList.innerHTML = '<li class="text-gray-500 italic">Sin repuestos asociados.</li>';
                }
            })
            .catch(err => {
                console.error("Error al obtener repuestos:", err);
                repuestoList.innerHTML = '<li class="text-red-500">Error al cargar repuestos.</li>';
            });
    }
});

// Validar que la sede de entrada y salida no sean la misma
document.getElementById("formRegistrarmaquina").addEventListener("submit", function (e) {
    const sedeSalida = document.getElementById("sede").value;
    const sedeEntrada = document.getElementById("sede_entrada").value;

    if (sedeSalida === sedeEntrada) {
        e.preventDefault();
        alert("La sede de salida y la sede de entrada no pueden ser la misma.");
    }
});

function validarSedesIguales() {
    const sedeSalida = document.getElementById("sede").value;
    const sedeEntrada = document.getElementById("sede_entrada").value;
    const botonGuardar = document.getElementById("guardar");
    const mensajeError = document.getElementById("errorSedes");

    if (sedeSalida && sedeEntrada && sedeSalida === sedeEntrada) {
...
document.getElementById("sede").addEventListener("change", validarSedesIguales);
document.getElementById("sede_entrada").addEventListener("change", validarSedesIguales);
