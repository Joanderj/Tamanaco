...
$nombreMaquina = isset($_GET['nombreMaquina']) ? trim($_GET['nombreMaquina']) : '';
...
// Clasificación
$orderBy = 'm.id_maquina'; // Orden por defecto
if (isset($_GET['sort'])) {
    switch ($_GET['sort']) {
        case 'fecha_asc':
        ...
        case 'fecha_desc':
            ...
            break;
        case 'nombre_asc':
            $orderBy = 'm.nombre_maquina ASC';
            break;
        case 'nombre_desc':
            $orderBy = 'm.nombre_maquina DESC';
            break;
        case 'numero_asc':
            $orderBy = 'm.id_maquina ASC';
            break;
        case 'numero_desc':
            $orderBy = 'm.id_maquina DESC';
            break;
    }
}

// Consulta total de elementos
$totalQuery = "SELECT COUNT(*) FROM maquina m WHERE 1=1";
$params = [];

if (!empty($nombreMaquina)) {
    $totalQuery .= " AND m.nombre_maquina LIKE ?";
    $params[] = '%' . $nombreMaquina . '%';
}

if (!empty($idMarca)) {
    $totalQuery .= " AND m.id_marca = ?";
    $params[] = $idMarca;
}

if (!empty($idModelo)) {
    $totalQuery .= " AND m.id_modelo = ?";
    $params[] = $idModelo;
}

if (!empty($idTipo)) {
    $totalQuery .= " AND m.id_tipo = ?";
    $params[] = $idTipo;
}
...
if (!empty($nombreMaquina)) {
    $query .= " AND m.nombre_maquina LIKE ?";
}
...
<div id="modalMaquina" class="fixed inset-0 bg-black bg-opacity-50 hidden items-center justify-center z-50">
    <div class="bg-white p-6 rounded-lg w-11/12 max-w-2xl">
        <h2 class="text-xl font-bold mb-4">Detalles de Máquina</h2>
        <p><strong>Nombre:</strong> <span id="maquinaNombre"></span></p>
        <p><strong>Tipo:</strong> <span id="maquinaTipo"></span></p>
        <p><strong>Marca:</strong> <span id="maquinaMarca"></span></p>
        <p><strong>Modelo:</strong> <span id="maquinaModelo"></span></p>
        <p><strong>Elaborada por:</strong> <span id="maquinaElaboradaPor"></span></p>
        <p><strong>Fecha de creación:</strong> <span id="maquinaFecha"></span></p>
        <p><strong>Descripción:</strong> <span id="maquinaDescripcion"></span></p>

        <h3 class="font-semibold mt-4">Especificaciones:</h3>
        <ul id="listaEspecificaciones" class="list-disc pl-5 text-gray-700"></ul>

        <h3 class="font-semibold mt-4">Características:</h3>
        <ul id="listaCaracteristicas" class="list-disc pl-5 text-gray-700"></ul>

        <button onclick="closeMaquinaModal()" class="mt-4 bg-red-500 text-white px-4 py-2 rounded">Cerrar</button>
    </div>
</div>
...
<div class="flex justify-between items-center mb-4">
    ...
    <div class="space-x-2">
        <?php if (in_array(2, $permisos)): ?>
            <button 
                onclick="window.location.href='formulario_guardar_maquina.php'" 
        ...
        <?php if (in_array(4, $permisos)): ?>
            <button 
                onclick="window.location.href='imprimir_maquina.php'" 
...
<div class="border border-gray-300 rounded-lg shadow-md p-4">
    ...
    <div class="flex justify-between items-center mb-4 border-b border-gray-300 pb-2">
        <h1 class="text-xl font-semibold text-gray-700">Maquinas</h1>
        <div class="relative">
    ...
    <tbody id="dataTable" class="bg-white divide-y divide-gray-200">
        ...
        <tr class="hover:bg-gray-100">
            <!-- Imagen -->
            <td class="px-6 py-4">
                <?php if (!empty($maquina['url'])): ?>
                    <img src="<?php echo htmlspecialchars($maquina['url']); ?>" alt="<?php echo htmlspecialchars($maquina['nombre_imagen']); ?>" class="w-32 h-32 object-cover rounded-md">
                <?php else: ?>
                    <span>Sin Imagen</span>
                <?php endif; ?>
            </td>
            ...
            <td class="px-6 py-4">
                <ul>
                    <li class="flex items-center space-x-2">
                        ...
                        <strong>ID:</strong> <?php echo htmlspecialchars($maquina['id_maquina']); ?>
                    </li>
                    <li class="flex items-center space-x-2">
                        <i class="fas fa-tags text-green-500"></i>
                        <strong>Nombre:</strong> <?php echo htmlspecialchars($maquina['nombre_maquina']); ?>
                    </li>
                    <li class="flex items-center space-x-2">
                        <i class="fas fa-user-tie text-orange-500"></i>
                        <strong>Elaborada por:</strong> <?php echo htmlspecialchars($maquina['elaborada_por']); ?>
                    </li>
                    <li class="flex items-center space-x-2">
                        <i class="fas fa-industry text-blue-500"></i>
                        <strong>Marca:</strong> <?php echo htmlspecialchars($maquina['nombre_marca']); ?>
                    </li>
                    <li class="flex items-center space-x-2">
                        <i class="fas fa-cube text-yellow-500"></i>
                        <strong>Modelo:</strong> <?php echo htmlspecialchars($maquina['nombre_modelo']); ?>
                    </li>
                    <li class="flex items-center space-x-2">
                        <i class="fas fa-layer-group text-teal-500"></i>
                        <strong>Tipo:</strong> <?php echo htmlspecialchars($maquina['nombre_tipo']); ?>
                    </li>
                    <li class="flex items-center space-x-2">
                        <i class="fas fa-check-circle text-teal-500"></i>
                        <strong>Estado:</strong> <?php echo htmlspecialchars($maquina['nombre_status']); ?>
                    </li>
                    <li class="flex items-center space-x-2">
                        <i class="fas fa-calendar-alt text-purple-500"></i>
                        <strong>Fecha:</strong> <?php echo htmlspecialchars($maquina['date_created']); ?>
                    </li>
                </ul>
            </td>
             <?php
...
$idPerfil = $_SESSION['id_perfil'];
$idSubmenu = 16; // ID del submenú correspondiente
$permisos = obtenerPermisos($idPerfil, $idSubmenu);
?>

<td class="px-6 py-4 flex space-x-2">
    <?php if ($maquina['nombre_status'] == 'Activo'): ?>
        
        <?php if (in_array(1, $permisos)): ?>
        <button 
            class="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600"
            onclick="showMaquinaModal(<?php echo $maquina['id_maquina']; ?>)">
        ...
        <button 
            ...
            onclick="window.location.href='formulario_modificar_maquina.php?id_maquina=<?php echo $maquina['id_maquina']; ?>'">
            <i class="fas fa-edit"></i> Modificar
        </button>
        <?php endif; ?>
        <?php if (in_array(4, $permisos)): ?>
            <button 
               onclick="window.location.href='imprimir_maquina_individual.php?id_maquina=<?php echo $maquina['id_maquina']; ?>'" 
                class="bg-green-500 text-white px-4 py-2 rounded hover:bg-green-600">
                <i class="fas fa-print"></i> Imprimir
            </button>
        <?php endif; ?>
...
<script>
function showMaquinaModal(idMaquina) {
    fetch(`getMaquinaDetalles.php?id_maquina=${encodeURIComponent(idMaquina)}`)
        .then(response => {
        ...
        .then(data => {
            ...
            document.getElementById('maquinaNombre').textContent = data.nombreMaquina;
            document.getElementById('maquinaTipo').textContent = data.tipo;
            document.getElementById('maquinaMarca').textContent = data.marca;
            document.getElementById('maquinaModelo').textContent = data.modelo;
            document.getElementById('maquinaDescripcion').textContent = data.descripcion;
            document.getElementById('maquinaElaboradaPor').textContent = data.elaboradaPor;
            document.getElementById('maquinaFecha').textContent = data.fechaCreacion;

            // Limpiar y agregar especificaciones
            const contEsp = document.getElementById('listaEspecificaciones');
            contEsp.innerHTML = '';
            data.especificaciones.forEach((esp, i) => {
                const li = document.createElement('li');
                li.innerHTML = `<strong>${i + 1}.</strong> ${esp.nombre_especificacion}: ${esp.descripcion_especificacion}`;
                contEsp.appendChild(li);
            });

            // Limpiar y agregar características
            const contCar = document.getElementById('listaCaracteristicas');
            contCar.innerHTML = '';
            data.caracteristicas.forEach((car, i) => {
                const li = document.createElement('li');
                li.innerHTML = `<strong>${i + 1}.</strong> ${car.nombre_caracteristica}: ${car.descripcion_caracteristica}`;
                contCar.appendChild(li);
            });
